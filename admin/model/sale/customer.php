	...
	public function addTransaction($customer_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($customer_info) {
			...
			$message .= sprintf($this->language->get('text_transaction_total'), $this->currency->format($this->getTransactionTotal($customer_id)));
                        ...
                        $subject = sprintf($this->language->get('text_transaction_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
                        
                        $emailTemplate = $this->emailTemplate(13);
                        if($emailTemplate){
                            $message =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                            $message = str_replace(array('[FIRST-NAME]', '[CREDIT-POINTS]', '[TOTAL-CERDIT-POINTS]'), array(
                                $customer_info['firstname'], $this->currency->format($amount, $this->config->get('config_currency')),  $this->currency->format($this->getTransactionTotal($customer_id))
                            ), $message);
	...
	public function addReward($customer_id, $description = '', $points = '', $order_id = 0) {
		...
		if ($customer_info) {
			...
			$message .= sprintf($this->language->get('text_reward_total'), $this->getRewardTotal($customer_id));
                        ...
                        $subject = sprintf($this->language->get('text_reward_subject'), html_entity_decode($store_name, ENT_QUOTES, 'UTF-8'));
                        
                        $emailTemplate = $this->emailTemplate(12);
                        if($emailTemplate){
                            $message =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                            $message = str_replace(array('[FIRST-NAME]', '[REWARD-POINTS]', '[TOTAL-REWARD-POINTS]'), array(
                                $customer_info['firstname'], $points, $this->getRewardTotal($customer_id)
                            ), $message);
                            $subject = $emailTemplate['email_subject'];
                        }
