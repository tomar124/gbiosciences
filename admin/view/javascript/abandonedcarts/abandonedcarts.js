...
$(document).ready(function() {
    ...
    scheduleTypeDisplayOption($('select[name="' + moduleName + '[ScheduleType]"]').val());
    
    $('select[name="' + moduleName + '[ScheduleType]"]').on('change', function(e){
        scheduleTypeDisplayOption($(this).val());
    });
            
});

// Merging two independent condition check, for scheduletype select option.
    function scheduleTypeDisplayOption(scheduleTypeValue) {
        if(scheduleTypeValue == 'P') {
           $('#FixedDateOptions').hide();
           $('#CustomOptions').hide();
           $('#PeriodicOptions').show(200);	
        } else if(scheduleTypeValue == 'F') {
            $('#PeriodicOptions').hide();
            $('#CustomOptions').hide();
            $('#FixedDateOptions').show(200);
        } else if(scheduleTypeValue == 'C') {
            $('#FixedDateOptions').hide();
            $('#PeriodicOptions').hide();
            $('#CustomOptions').show(200);			
	}
    }
...
// CUSTOM CRON    
    $('.btn.cstmCronVerify').on('click', function(e){
        e.preventDefault();
        if($('#CustomDate').val() && $('#CustomTemplate').val() ) {
            
            var returnVal = includes(cronHours, $('#CustomDate').val());
            if(!returnVal) {
                    cronHours.push($('#CustomDate').val());
                    $('.scrollbox.CstmDateList').append('<div id="date' + $('#CustomDate').val() + '">' + $('#CustomDate').val() + ' Hour /' + $("#CustomTemplate option:selected").text() + '<i class="fa fa-minus-circle removeIcon"></i>\n\ \n\ <input type="hidden" name="' + moduleName + '[CustomDates][]" value="' + $('#CustomDate').val() + '/' + $('#CustomTemplate').val() + '" /></div>');
                    $('#CustomDate').val('');
                    $('#CustomTemplate').val('');            
            } else {
                alert('Cron for hour '+$('#CustomDate').val() + ' already exist. Please select another hour');
            }
        } else {
            alert(alert_custom_date);
        }
        refreshRemoveButtonForTheCronJobsCustom();
    });
    ...
    function refreshRemoveButtonForTheCronJobsCustom() {
            $('.scrollbox.CstmDateList div .removeIcon').click(function() {
                $(this).parent().remove();
                resetCustomCronHour();
            });
    }
        // on page load it will fill cronHours.
        
    function resetCustomCronHour() {        
       cronHours = [];
        $(".scrollbox.CstmDateList input[type=hidden]").each(function() {
         var tmp = $(this).val(); 
         res = tmp.split("/");
         cronHours.push(res[0]);
        });
    }
    
    function includes(cronHours, value) {
            var returnvalue = false;
            var count=cronHours.length;
                for(var i=0;i<count;i++)
                {
                    if(cronHours[i] == value.trim()){
                        returnvalue = true;
                    }
                }
                return returnvalue;
    ...
    // verify cron type for fixed/periodic mail template    
    function cronTypeCstm(val,resultid) {
      if(val) {
        $.ajax({
            url: "index.php?route=" + modulePath + "/cronTypeCstm&" + token_addon + "&verifyTemplate=" + val,
            type: 'get',
            dataType: 'html',
            success: function(data) {
                $("#mailtemplateresponse"+resultid).html(data);
            }
        }); 
      }
    };
