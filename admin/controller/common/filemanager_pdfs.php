<?php
class ControllerCommonFileManagerPdfs extends Controller {
        private $ROOT_DIRECTORY = 'pdfs/';
	...
	public function index() {
		...
		$this->load->model('tool/image');
                ...
                if ($this->s3->getObject($directory, '', '', true)) {
                        // Get data
                        ...
                        foreach ($images as $image) {
                                if (in_array($image, $directories)) {
                                        ...
                                        $data['images'][] = array(
                                                ...
                                                'href'  => $this->url->link('common/filemanager_pdfs', 'user_token=' . $this->session->data['user_token'] . '&directory=' . urlencode(str_replace($this->ROOT_DIRECTORY, '', $directory) . $image) . $url, true)
                                        );
		...
		if (isset($this->request->get['directory'])) {
			$pos = strrpos(rtrim($this->request->get['directory'], '/'), '/');
		...
		$data['parent'] = $this->url->link('common/filemanager_pdfs', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['refresh'] = $this->url->link('common/filemanager_pdfs', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('common/filemanager_pdfs', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$this->response->setOutput($this->load->view('common/filemanager_pdfs', $data));
	}
