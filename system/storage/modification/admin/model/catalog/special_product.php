...
class ModelCatalogSpecialProduct extends Model {
	...
	public function addProduct($data) {
		...
		if (isset($data['special_product_reward'])) {
            ...
            if (isset($data['special_product_filter'])) {
                    ...
                    foreach ($data['special_product_filter'] as $special_product_filter) {
                            ...
                            if (!$this->isSpecialProductFilterExists($product_id, $special_product_filter['special_product_filter_group_id'], $special_product_filter['special_product_filter_id'])) {
                                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_special_product_filter SET product_id = '" . (int)$product_id . "', special_product_filter_group_id = '" . (int)$special_product_filter['special_product_filter_group_id'] . "', special_product_filter_id = '" . (int)$special_product_filter['special_product_filter_id'] . "'");
                            }
                    }
	...
	public function editProduct($product_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', is_ground_hazmat = '" . $data['is_ground_hazmat'] . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', size = '" .$this->db->escape($data['size']) . "', shipping_code = '" . $data['shipping_code'] . "', hazardous = '" . (int)$data['hazardous'] . "', cart_comment = '" .$this->db->escape($data['cart_comment']) . "', special_product = 1, date_modified = NOW(), images_processed = 0 WHERE product_id = '" . (int)$product_id . "'");
		...
		$this->cache->delete('special_product');
            ...
            public function getSpecialProductFilterData($product_id) {
                    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_special_product_filter WHERE product_id = '" . (int)$product_id . "'");

                    return $query->rows;
            }

            public function getSpecialProductFilterDetail($special_product_filter_group_id) {
                    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "special_product_filter_group spfg LEFT JOIN " . DB_PREFIX . "special_product_filter_group_description spfgd ON (spfg.special_product_filter_group_id = spfgd.special_product_filter_group_id) WHERE spfg.special_product_filter_group_id = '" . (int)$special_product_filter_group_id . "'");

                    return $query->row;
            }
                        
            public function getSpecialProductFilterOption($special_product_filter_group_id) {
                    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "special_product_filter spf LEFT JOIN " . DB_PREFIX . "special_product_filter_description spfd ON (spf.special_product_filter_id = spfd.special_product_filter_id) WHERE spf.special_product_filter_group_id = '" . (int)$special_product_filter_group_id . "' AND spfd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY spf.sort_order");
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
            ...
            $data['special_product_filter'] = $this->getSpecialProductFilterData($product_id);
...
//		// get the product discounts
	...
	protected function uploadProductOptionValues( &$reader, $incremental, &$available_product_ids ) {
		...
		( "product_id", "catalog", "description", "meta_title", "meta_description", "meta_keywords", "categories", "gene", "image","alt_text","caption", "shipping", "price", "hazardous", "is_ground_hazmat", "size", "cart_comment", "weight", "weight_unit", "length", "width", "height", "length_unit", "seo_keyword", "status", "points", "date_added", "date_modified", "sort_order" );
	...
	protected function validateProductOptionValues( &$reader ) {
		...
		$worksheets = array( 'Protocol', 'Msds', 'Rewards', 'Images', 'Discounts', 'Specials', 'ProductOptions', 'ProductOptionValues' );
            ...
            $worksheets = array( 'Protocol', 'Msds', 'Rewards', 'Images', 'Discounts', 'Specials', 'Filters', 'ProductOptions', 'ProductOptionValues' );
