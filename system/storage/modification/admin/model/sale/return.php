            ...
            if ($notify) {
                    ...
                    if ($return_query->num_rows) {
                            ...
                            if ($comment) {
                                    ...
                                    $comment = $this->language->get('text_comment') . "\n\n" . strip_tags(html_entity_decode($comment, ENT_QUOTES, 'UTF-8'));
                            }else{
                                $comment = '';
                            }
                            ...
                            $emailTemplate = $this->emailTemplate(6);
                            if($emailTemplate){
                                $body =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                                $body = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[RETURN-ID]', '[RETURN-DATE]', '[RETURN-STATUS]', '[RETURN-COMMENT]'), array($return_query->row['firstname'], $return_query->row['lastname'], $return_id, date($this->language->get('date_format_short'), strtotime($return_query->row['date_added'])), $return_query->row['status'], $comment), $body);
                                $subject = str_replace('[RETURN-ID]', $return_id, $emailTemplate['email_subject']);
                            }
