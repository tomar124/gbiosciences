...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
            ...
            'review_notification_count' => $this->model_sale_order->getReveiewNotificationCount($result['order_id']),
	...
	public function info() {
		...
		if ($order_info) {
            ...
            /* CUSTOM FEDEX LABEL CODE */
            $this->load->model('tool/image');

            $data['pdf_icon'] = $this->model_tool_image->resize('pdf.png', 25, 25);

            $data['label'] = $this->url->link('sale/order/label', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);

            if(isset($order_info['shipping_code'])){
                if(strstr($order_info['shipping_code'], 'fedex', 0)){
                    $data['has_shipping_label'] = TRUE;

                    if(isset($order_info['shipping_label']) && !empty($order_info['shipping_label'])){
                        $data['shipping_label'] = HTTP_CATALOG . 'image/ShipmentLabel/' . $order_info['shipping_label'];
                        $data['shipping_transaction_id'] = $order_info['shipping_transaction_id'];
                        $data['shipping_tracking_id'] = $order_info['shipping_tracking_id'];
                    }else{
                        $data['shipping_label'] = '';
                        $data['shipping_transaction_id'] = '';
                        $data['shipping_tracking_id'] = '';                        
                        $data['show_aes_citation_number_input'] = ($order_info['total'] > 2500)? TRUE : FALSE;
                    }
                }else{
                    $data['has_shipping_label'] = FALSE;
                }
            }
            /* CUSTOM FEDEX LABEL CODE */
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
            ...
            $data['text_order']        = sprintf($this->language->get('text_order'), $order_info['order_id']);
            ...
            'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
            'list_price' => $this->currency->format($product['list_price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
            'is_quote' => $product['is_quote']
		...
		$this->response->setOutput($this->load->view('sale/order_invoice', $data));
            ...
            public function label() {
                    ...
                    if (isset($this->request->get['order_id'])) {
                        ...
                        $json['error'] = $this->language->get('error_label');
                    }
                    ...
                    if(!$json){
                        ...
                        if ($order_info) {
                                $shipping_service = strtoupper(explode('.', $order_info['shipping_code'])[1]);
                                ...
                                foreach ($products as $product) {
                                    ...
                                    if (empty($product_info)) {
                                            $product_info = $this->model_catalog_special_product->getProduct($product['product_id']);
                                    }
                                    ...
                                    for($i = 1; $i <= $product['quantity']; $i++){
                                        ...
                                        $weight += $this->weight->convert($product_info['weight'], $this->config->get('config_weight_class_id'), $this->config->get('shipping_fedex_weight_class_id'));
                                    }

                                    if ($shipping_service == 'INTERNATIONAL_FIRST' || $shipping_service == 'INTERNATIONAL_ECONOMY' || $shipping_service == 'INTERNATIONAL_PRIORITY') {
                                        $commodities[] = array(
                                            'NumberOfPieces' => $product['quantity'],
                                            'Description' => $product['name'],
                                            'CountryOfManufacture' => 'US',
                                            'Weight' => array(
                                                'Units' => strtoupper($this->weight->getUnit($this->config->get('fedex_weight_class_id'))),
                                                'Value' => $product_info['weight'] / $product['quantity']
                                            ),
                                            'Quantity' => $product['quantity'],
                                            'QuantityUnits' => 'EA',
                                            'UnitPrice' => array(
                                            ...
                                            'CustomsValue' => array(
                                                'Currency' => $this->config->get('config_currency'),
                                                'Amount' => $product['price'] * $product['quantity'] //It should equal the commodity unit quantity times commodity unit value
                                            )
                                        );
                                    }
                                }
                                ...
                                $data = array(                                
                                    'shipping_address' => array(
                                        'Contact' => array(
                                            'PersonName' => $order_info['shipping_firstname'] . ' ' . $order_info['shipping_lastname'],
                                            'CompanyName' => $order_info['shipping_company'],
                                            'PhoneNumber' => $order_info['telephone']
                                        ),
                                        'Address' => array(
                                            'StreetLines' => array($order_info['shipping_address_1'], $order_info['shipping_address_2']),
                                            'City' => $order_info['shipping_city'],
                                            'StateOrProvinceCode' => $order_info['shipping_zone_code'],
                                    ...
                                    'aes' => isset($this->request->post['aes']) && $this->request->post['aes'] != 'undefined' ? $this->request->post['aes']: ''
                                );

                                $this->load->library('fedex');

                                $response = $this->fedex->createShipment($data);

                                if($response){
                                    if ($response->HighestSeverity != 'FAILURE' && $response->HighestSeverity != 'ERROR') {
                                        if (isset($response->CompletedShipmentDetail->CompletedPackageDetails->TrackingIds->TrackingNumber) && !empty($response->CompletedShipmentDetail->CompletedPackageDetails->TrackingIds->TrackingNumber)) {
                                            $TrackingNumber = $response->CompletedShipmentDetail->CompletedPackageDetails->TrackingIds->TrackingNumber;
                                        }else{
                                            $TrackingNumber = '';
                                        }

                                        if (isset($response->TransactionDetail->CustomerTransactionId) && !empty($response->TransactionDetail->CustomerTransactionId)) {
                                            $ShippingTransactionId = $response->TransactionDetail->CustomerTransactionId;
                                        }else{
                                            $ShippingTransactionId = '';
                                        }

                                        $fileName = strtotime(date('c')) . '_' . $shipping_service . '.pdf';

                                        if (!file_exists(DIR_IMAGE . 'ShipmentLabel')) {
                                                mkdir(DIR_IMAGE . 'ShipmentLabel', 0755, true);
                                        }
                
                                        $label = fopen(DIR_IMAGE . 'ShipmentLabel/' . $fileName, 'wb');
                                        fwrite($label, ($response->CompletedShipmentDetail->CompletedPackageDetails->Label->Parts->Image));
                                        fclose($label);

                                        $this->db->query("UPDATE " . DB_PREFIX . "order SET shipping_label = '" . $fileName . "', shipping_transaction_id = '" . $ShippingTransactionId . "', shipping_tracking_id = '" . $TrackingNumber . "' WHERE order_id = '".$order_id."'");
                                        
                                        $json['success'] = array(
                                            'text' => "Fedex ".ucfirst(strtolower($response->HighestSeverity)).": Label generated successfully",
                                            'shipping_label' => HTTP_CATALOG . 'image/ShipmentLabel/' . $fileName,
                                            'shipping_tracking_id' => $TrackingNumber,
                                            'shipping_transaction_id' => $ShippingTransactionId
                                        );
                                    }else{
                                        $json['error'] = "Fedex ".ucfirst(strtolower($response->HighestSeverity))."[{$response->Notifications->Code}]: {$response->Notifications->Message}";
                                    }
                                }
                        }
                    }
