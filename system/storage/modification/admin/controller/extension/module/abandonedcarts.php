...
class ControllerExtensionModuleAbandonedcarts extends Controller {
	...
	public function index() {
        ...
        $this->data['customerGroups']           = $this->model_customer_customer_group->getCustomerGroups();
            ...
            // mapping array of cron hour with template id
                foreach($this->data['moduleData']['MailTemplate'] as $mailTempl) {
                    $this->data['HourTemplates'][$mailTempl['id']] = $mailTempl['Name'];
                }
            
		$this->data['cronPhpPath'] = '0 0 * * * ';
        if (is_callable('shell_exec') && false === stripos(ini_get('disable_functions'), 'shell_exec') && trim(shell_exec('echo EXEC')) == 'EXEC') {
        ...
        $this->data['encodedLicense'] = !empty($this->data['moduleData']['LicensedOn']) ? base64_encode(json_encode($this->data['moduleData']['License'])) : "";
        ...
        $this->data['ticketOpenLink'] = "http://isenselabs.com/tickets/open/{" . base64_encode('Support Request') . "/" . base64_encode('117') . "/" . base64_encode($_SERVER['SERVER_NAME']);
	...
	// Validation against users with no permissions	
                ...
                protected function validateForm() {
                        ...
                        /* More than one template for fixed/periodic validation */
                        $fixed = array();
                        $periodic = array();
                        $emptyTemplateValidFor = array();
                        foreach($this->request->post[$this->moduleName]['MailTemplate'] as $mailtemplate) {
                            if($mailtemplate['Enabled'] == 'yes') {
                                if($mailtemplate['TemplateValidFor'] == 'F') {
                                    $fixed[] = $mailtemplate['Name'];
                                } else if($mailtemplate['TemplateValidFor'] == 'P') {
                                    $periodic[] = $mailtemplate['Name'];
                                } else if(empty($mailtemplate['TemplateValidFor'])) {
                                    $emptyTemplateValidFor[] = $mailtemplate['Name'];
                                }
                            }
                        }
                        
                        if(!empty($emptyTemplateValidFor)) {
                            $this->error['warning'] = "<span class='text-danger'> Please select mail template type for <strong>".implode(",",$emptyTemplateValidFor)."</strong></span>";
                        }
                
                        if( count($fixed) > 1  || count($periodic) > 1) {
                            if(count($fixed) > 1 ) {
                                $error = implode(",",$fixed);
                            } else {
                                $error = implode(",",$periodic);
                            }
                            $this->error['warning'] = "<span class='text-danger'> Periodic and Fixed cron type can have only one template, Please disable/change cron type in these templates <strong>" .$error. "</strong></span>";
                        }
	...
	private function editCron($data=array(), $store_id) {
        ...
        if (isset($data[$this->moduleName]["ScheduleType"]) && $data[$this->moduleName]["ScheduleType"] == 'P') {
            ...
            if (isset($data[$this->moduleName]["ScheduleType"]) && $data[$this->moduleName]["ScheduleType"] == 'C') {
            	if (isset($data[$this->moduleName]["CustomDates"])) {
            		$cronCommands[]   = '0 * * * * '.$phpPath.' ' . $cronFolder . 'sendReminder.php';
            	}
            }
	...
	public function install() {
        ...
        $this->model_setting_event->addEvent($this->eventGroup, "catalog/model/checkout/order/*/before", $eventsPath . "/onBeforePaymentConfirm");
    }
    ...
    public function injectAdminMenuItem($eventRoute, &$data) {
        if ($this->user->hasPermission('access', $this->modulePath)) {
            ...
            if (isset($abandonedCartsSettings['abandonedcarts']['Enabled']) && $abandonedCartsSettings['abandonedcarts']['Enabled']=='yes' && isset($abandonedCartsSettings['abandonedcarts']['MenuWidget']) && $abandonedCartsSettings['abandonedcarts']['MenuWidget']=='yes') { 
                ...
                public function cronTypeCstm() {
                       $verifyTemplate   = $this->request->get['verifyTemplate'];
                        ...
                        if (isset($moduleSettings['MailTemplate']) && sizeof($moduleSettings['MailTemplate'])>0) {
                            foreach ($moduleSettings['MailTemplate'] as $template) {
                                    if (isset($template['Enabled']) && $template['Enabled']=='yes' && ($template['TemplateValidFor'] == $verifyTemplate)) {
                                        $count[] = $template['Name'];
                                    }
                        }
                            if(!empty($count)){
                                echo "<span class='text-danger'> Please disable/change cron type for template <strong>" .implode(",",$count). "</strong></span>";
                            } else {
                                echo '';
                            }
                        }
	...
	public function sendcustomemail() {
		...
		if (isset($this->request->post) && isset($this->request->post['ABcart_id']) && isset($this->request->post['AB_template_id'])) {
			...
			foreach ($result['cart'] as $product) { 
                ...
                if (isset($product_info["quantity"]) && (int)$product_info["quantity"] <= 0 && $skipOutOfStock) {
                    continue;
                }
                ...
                $parentId = $this->model_catalog_product->getparent_id($product['product_id']);
                if ((int)$product['product_id'] !== $parentId) {
                        $parentProductInfo = $this->model_catalog_product->getProduct($parentId);
                        if ($parentProductInfo && $parentProductInfo['image']) {
                                $image_thumb = $this->model_tool_image->resize($parentProductInfo['image'], $width, $height);
                        } else {
                                $image_thumb = $this->model_tool_image->resize('placeholder.png', $width, $height);
                        }
                } else {
                        $image_thumb = $this->model_tool_image->resize('placeholder.png', $width, $height);
                }
				...
				$CartProducts .='<tr>';
				$CartProducts .='<td><div style="float:left;padding-right:3px;"><a href="'.$catalog_link.'index.php?route=product/product&product_id='. $this->model_catalog_product->getparent_id($product['product_id']).'" target="_blank"><img src="'.str_replace(' ', '%20', $image_thumb).'" /></a></div> <a href="'.$catalog_link.'index.php?route=product/product&product_id='. $this->model_catalog_product->getparent_id($product['product_id']).'" target="_blank">'.$product['name'].'</a><br />';
			...
			if ($settingTemplate['DiscountType']=='N') {
                ...
                $CouponData['hazmat'] = $settingTemplate['DiscountHazmat'];
                $CouponData['handling'] = $settingTemplate['DiscountHandling'];
