...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('catalog/product/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
            ...
            $data['add_gp_grouped'] = $this->url->link('catalog/product/add', 'user_token=' . $this->session->data['user_token'] . '&is_gp=grouped', true);
            $data['export'] = $this->url->link('catalog/product/export', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
	...
	protected function getForm() {
                ...
                foreach ($gps as $child_id => $child) {
                    ...
                    if ($child_info) {
                        ...
                        $data['gps'][] = array(
                            ...
                            'href' => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $child_id, 'SSL'),
		...
		} elseif (!empty($product_info) && !empty($product_info['image']) && $this->s3->getObject('', $product_info['image'], '', true)) {
		...
		$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
            ...
            if (isset($this->request->post['product_references'])) {
                    ...
                    $product_references = $this->model_catalog_product->getProductReferences($this->request->get['product_id']);
            } else {
		...
		foreach ($product_images as $product_image) {
            ...
            if(isset($product_info)){
                $sds = $this->model_catalog_product->getProductsSds($product_info['product_id']); 
            }

            $languageTechnical = $this->model_catalog_product->getLanguageTechnicalForProducts();
            ...
            if(isset($product_info)){
                $technicals = $this->model_catalog_product->getProductsTechnicals($product_info['product_id']);
            }
            ...
            if(isset($product_info)){
                $coas = $this->model_catalog_product->getProductsCoa($product_info['product_id']);
            }

            if (isset($this->request->post['coa']) && $this->request->post['coa']) {
                    foreach($this->request->post['coa'] as $coa){
                        if($this->s3->getObject($coa['pdf'], '', '', true)){
                            $data['coas'][] = array(
                                'thumb' => $this->model_tool_image->resize('pdf_icon.png', 100, 100),
                                'pdf' => $coa['pdf'],
            ...
            //Protocol
            if(isset($product_info)){
                $protocol = $this->model_catalog_product->getProductsProtocol($product_info['product_id']);
            }
            ...
            } else if (!empty($protocol) && $this->s3->getObject($protocol['pdf'], '', '', true)) {
                    $data['thumb_protocol'] = $this->model_tool_image->resize('pdf_icon.png', 100, 100);
            } else {
