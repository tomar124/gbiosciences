...
class ControllerCustomerCustomer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->hubspotData($this->request->post, 'add');
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $email_id = $this->model_customer_customer->editCustomer($this->request->get['customer_id'], $this->request->post);

            //Transffering data to hubspot
            $this->hubspotData($this->request->post, 'edit', $this->request->get['customer_id'], $email_id);
	...
	public function customfield() {
		...
		$this->response->setOutput(json_encode($json));
            ...
            public function hubspotData($data, $type, $customer_id = '', $email = '') {
                    if($email == ''){
                            $email = $data['email'];
                    }
                    ...
                    $hubspot_data = array(
                            'properties' => array(
                                    ...
                                    array(
                                            'property' => 'fax',
                                            'value' => isset($data['input-custom-field3']) ? $data['input-custom-field3'] : ''
                                    )
                            )
                    );

                    if (isset($data['address']) && count($data['address']) > 1) {
                            foreach ($data['address'] as $address) {
                                    if (isset($address['default']) && $address['default'] != 0) {
                    ...
                    switch ($type){
                            case 'add' : $this->hubspot->_create_update(json_encode($hubspot_data), $data['email']);
                                    break;
                            case 'edit': $this->hubspot->_update_by_email(json_encode($hubspot_data), $email);
                                    break;
                            default : break;
                    }
            }
