...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			try{
				if(isset($GLOBALS['astraContainer'])){
					$astraContainer = $GLOBALS['astraContainer'];
					$pathLoginModule = DIR_APPLICATION . "/controller/extension/astra/plugins/LoginProtectionPlugin/LoginProtectionPlugin.php";
					if(!class_exists('LoginProtectionPlugin') && file_exists($pathLoginModule)){
						include_once $pathLoginModule;
					}
					
					if(class_exists('LoginProtectionPlugin')){
						$astraLoginModule = new LoginProtectionPlugin($astraContainer);
						// Start Astra Login capture
						$user['user_login'] = (!empty($this->request->post['username']))? $this->request->post['username'] : '';
						$astraData['success'] = true;
						$astraData['username'] = $user['user_login'];
						//$astraData['email'] = $user['user_login'];
						$astraData['displayName'] = $user['user_login'];
						if($astraContainer->get('options')->get('accessToken')){
							$astraLoginModule->login($astraData);
						}
			...
			try{
				if(isset($GLOBALS['astraContainer'])){
					...
					if(class_exists('LoginProtectionPlugin')){
						...
						$astraData['success'] = false;
						$astraData['username'] = $username;
						if($astraContainer->get('options')->get('accessToken')){
							$astraLoginModule->login($astraData);
						}
						// End Astra Login capture
