...
class ControllerExtensionModuleAbandonedcarts extends Controller
{
    ...
    public function checkCustomerGroup($templateSettings = array(), $email) {
        if (empty($templateSettings['customerGroups'])) {
            return true;
        } else if (!empty($templateSettings['customerGroups'])) {
            $group_query = $this->db->query("SELECT `customer_group_id` FROM `" . DB_PREFIX . "customer` WHERE `email` = '" . $this->db->escape($email) . "' LIMIT 1");
            
            if ($group_query->num_rows) {
                $customer_group_id = $group_query->row['customer_group_id'];
            } else {
                $customer_group_id = 0;
            }
            
            $customer_group_ids = array_keys($templateSettings['customerGroups']);

            return in_array($customer_group_id, $customer_group_ids);
        }
	}
    ...
    public function sendReminder()  {
	    ...
	    if (!empty($moduleData['Enabled']) && $moduleData['Enabled'] == 'yes' && isset($moduleData['ScheduleEnabled']) && $moduleData['ScheduleEnabled']='yes' && isset($moduleData['MailTemplate']) && $moduleData['ScheduleType'] !='C') {
            $tempValidFor = $moduleData['ScheduleType'];
	    ...
	    if ($mailtemplate['Enabled']=='yes' && $mailtemplate['TemplateValidFor'] == $tempValidFor) {
						...
						foreach ($results as $result) {
							...
							if (isset($result['customer_info']['email']) && isset($result['customer_info']['firstname']) && isset($result['customer_info']['lastname'])) {
                                ...
                                if (!in_array($result['customer_info']['email'], $usedEmails, true) && $this->checkCustomerGroup($mailtemplate, $result['customer_info']['email'])) {
                ...
                if ((int)$product['product_id'] !== $parentId) {
                        ...
                        $image_thumb = $this->model_tool_image->resize('placeholder.png', $width, $height);
										...
										$CartProducts .='<tr>';
                                        $CartProducts .='<td class="name"><div id="picture" style="float:left;padding-right:3px;"><a href="'.$catalog_link.'index.php?route=product/product&product_id='. $this->model_catalog_product->getparent_id($product['product_id']).'" target="_blank"><img src="'.$image_thumb.'" /></a></div> <a href="'.$catalog_link.'index.php?route=product/product&product_id='. $this->model_catalog_product->getparent_id($product['product_id']).'" target="_blank">'.$product['name'].'</a><br />';
                ...
                $CouponData['hazmat'] = $mailtemplate['DiscountHazmat'];
                $CouponData['handling'] = $mailtemplate['DiscountHandling'];
                ...
                // Sort cron hours in ascending order
                $CustomDatesAsc = array();
                $CustomDatesAsc = (array_values(array_unique($moduleData['CustomDates'])));
                sort($CustomDatesAsc);

                foreach($CustomDatesAsc as $customDate) {
                    $Buffer = explode('/', $customDate);
                    $BufferHour[] = $Buffer[0];
                    $BufferTemplateId[] = $Buffer[1];
                }  
               
                $usedEmails = array();
                $store_data = $this->moduleModel->getStore($store['store_id']);

                for($k = 0; $k < count($CustomDatesAsc); $k++) {
                       $mailtemplate = array();
                       $cronHour = $BufferHour[$k];
                       $mailtemplate = $moduleData['MailTemplate'][$BufferTemplateId[$k]];

                    if ($mailtemplate['Enabled'] == 'yes' && $mailtemplate['TemplateValidFor'] == 'C') {
                        $results = $this->moduleModel->getCustomCarts($cronHour, $store['store_id']);
                        foreach ($results as $result) {
                            $result['customer_info'] = json_decode($result['customer_info'], true);
                            ...
                            if (isset($result['customer_info']['email']) && isset($result['customer_info']['firstname']) && isset($result['customer_info']['lastname'])) {
                                ...
                                if (!in_array($result['customer_info']['email'], $usedEmails, true) && $this->checkCustomerGroup($mailtemplate, $result['customer_info']['email'])) {
                                    ...
                                    if ($mailtemplate['DiscountType'] == 'N') {
                                        ...
                                        if ($mailtemplate['DiscountApply'] == 'all_products') {
                                            $DiscountCode = $this->moduleModel->generateuniquerandomcouponcode();
                                            $TimeEnd = strtotime($result['date_created']) + ((isset($BufferHour[$k+1]) ? $BufferHour[$k+1] : ($mailtemplate['DiscountValidity'] * 24) )  / 24) * 24 * 60 * 60;
                                            $CouponData = array('name' => 'AbCart [' . $result['customer_info']['email'] . ']',
                                                'code' => $DiscountCode,
                                                'discount' => $mailtemplate['Discount'],
                                                'type' => $mailtemplate['DiscountType'],
                                    ...
                                    if (!in_array($result['customer_info']['email'], $usedEmails, true)) {
                                      ...
                                      // log cron info to db
                                      $customerInfo = json_encode($result['customer_info']);
                                      $cartInfo = json_encode($result['cart']);
                                      $this->db->query("INSERT INTO `" . DB_PREFIX . "abandoned_cart_custom_cron_log` SET `cron_interval`='".$this->db->escape($cronHour)."', `customer_info`='".$this->db->escape($customerInfo)."', `cart_info`='".$this->db->escape($cartInfo)."', `date_created`=NOW(), `date_modified`=NOW()");
                                    }
                                    $run_query = $this->db->query("UPDATE `" . DB_PREFIX . "".$this->moduleName."` SET notified = (notified + 1) WHERE `id`=".(int)$result['id']);
                               ...
                               $result = "Cron was executed successfully! A total of <strong>".$counter."</strong> emails were sent to the customers from the module AbandonedCarts.<br />";
                                ...
                                if (isset($moduleData['NotifyEmail']) && $moduleData['NotifyEmail']=='yes' && (date('H') >= 22 && date('H') < 23)) {
                                   $MailData = array(
                                       'email' =>  $this->config->get('config_email'),
                                       'message' => $result, 
                                       'subject' => 'AbandonedCarts Cron Task',
