...
class ControllerExtensionModuleAbandonedcartsEventHandlers extends Controller {
    ...
    public function processAbandonedCarts() {
        ...
        if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && !empty($this->request->server["HTTP_REFERER"])) {
            ...
            if (!empty($url_parts)) {
                $lastpage = !empty($url_parts["path"]) ? $url_parts["path"] : '';
	    ...
	    // validate the abandoned cart data which already exist, Abandoned cart fix for new entry on new browser login.
                $exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . "abandonedcarts` WHERE `cart` = '".$this->db->escape($cart)."' AND `customer_info` = '".$this->db->escape($customer)."' AND `store_id`='".$store_id."'");
