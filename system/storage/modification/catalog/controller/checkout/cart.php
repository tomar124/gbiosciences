...
class ControllerCheckoutCart extends Controller {
	public function index() {
        ...
        // 'option'    => $option_data,
            ...
            'is_custom' => isset($this->session->data['custom_price']) && array_key_exists($product['key'], $this->session->data['custom_price']) ? true : false
            ...
            $data['quote'] = $this->url->link('account/wishlist/addCartToQuote', '', true);
        ...
        if ($product_info['price'] == 0) {
                if (!empty($request_quote)) {
                         $request_quote .= ", <b>" . $product_info['name'] . "</b>";                                                 
                } else {
                        $request_quote = "Send a quote request for catalogs <b>" . $product_info['name'] . "</b>";
                }

                $json['error']['request_quote'] = $request_quote;
        }    
        ...
        $this->cart->add($this->request->post['products'][$i]['product_id'], $quantity, $option, $recurring_id);
        

				
        $json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['products'][$i]['product_id']), $product_info['name'], $this->url->link('checkout/cart'));
        ...
        $parent_id = $this->model_catalog_product->getparent_id($this->request->post['products'][$i]['product_id']);
	$json['redirect'] = str_replace('&amp;', '&', $this->url->link('product/product', 'product_id=' . $parent_id));
            ...
            foreach ($this->request->post['quantity'] as $key => $value) {
                    if (in_array($key, $cart_ids)) {
                            if (isset($products[array_search($key, $cart_ids)]['min_qty'])) {
                                    if ($value >= $products[array_search($key, $cart_ids)]['min_qty']) {
