...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['payment_method'] = $order_info['payment_method'];
            ...
            $data['po_number'] = isset($this->session->data['po_number']) && !empty($this->session->data['po_number']) ? $this->session->data['po_number'] : '';
		...
		foreach ($order_totals as $order_total) {
            ...
            'value'  => $this->currency->format($order_total['value'], $order_info['currency_code'], $order_info['currency_value']),1
		...
		if (!$from) {
            ...
            $emailTemplate = $this->emailTemplate(2);     
            if ($emailTemplate) {
                $order_body =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                $url_link = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_info['order_id'];
                $order_body = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[ORDER-INFO-URL]'), array($order_info['payment_firstname'], $order_info['payment_lastname'], $url_link), $order_body);

                $order_template_detail = $this->load->view('mail/order_detail', $data);
                $order_template_cart_detail = $this->load->view('mail/order_cart_detail', $data);
                $order_template_address_detail = $this->load->view('mail/order_address_detail', $data);
                $order_template_comment_detail = $this->load->view('mail/order_comment_detail', $data);
                $order_body = str_replace(array('[ORDER-DETAIL-SECTION]', '[ORDER-CART-SECTION]', '[ORDER-ADDRESS-SECTION]', '[ORDER-COMMENT-SECTION]'), array($order_template_detail, $order_template_cart_detail, $order_template_address_detail, $order_template_comment_detail), $order_body);
	...
	public function edit($order_info, $order_status_id, $comment) {
		...
		if (!$from) {
            ...
            $emailTemplate = $this->emailTemplate(5);     
            if($emailTemplate){
                    $order_body =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                    $order_body = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[ORDER-INFO-URL]', '[ORDER-ID]', '[ORDER-DATE-TIME]', '[ORDER-STATUS]', '[ORDER-UPDATE-COMMENT]'), array($order_info['payment_firstname'], $order_info['payment_lastname'], $data['link'], $data['order_id'], $data['date_added'], $data['order_status'], $data['comment']), $order_body);
                    $subject = str_replace('[ORDER-ID]', $data['order_id'], $emailTemplate['email_subject']);
            }
		...
		$mail->send();
            ...
            public function emailTemplate($emailTemplateID){
                    return $this->db->query("SELECT id, description, email_subject FROM email_template where id=$emailTemplateID and status=1")->row;
            }
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
            ...
            $emailTemplate = $this->emailTemplate(4);
                        
            if($emailTemplate){
                $order_body =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                $url_link = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id;
                $order_body = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[ORDER-INFO-URL]'), array($order_info['payment_firstname'], $order_info['payment_lastname'], $url_link), $order_body);
                ...
                $order_body = str_replace(array('[ORDER-DETAIL-SECTION]', '[ORDER-CART-SECTION]', '[ORDER-ADDRESS-SECTION]', '[ORDER-COMMENT-SECTION]'), array($order_template_detail, $order_template_cart_detail, $order_template_address_detail, $order_template_comment_detail), $order_body);
                $subject = str_replace('[ORDER-ID]', $order_id, $emailTemplate['email_subject']);
            }
