...
if (isset($this->request->get['product_id'])) {
			...
			$data['hasquestions'] = $this->model_extension_tmdquestionans->getQuestionByProductId($product_id);
			$hasTotalquestions = $data['hasquestions'];

			$this->settings['items'][]=array(
				'title'=>'FAQs ('.$hasTotalquestions .')',
		...
		if ($this->settings['display'] === 'accordion') {
			...
			if (isset($this->request->get['product_id'])) {
				...
				$this->settings['items'][]=array(
					...
					'contentType'=>'',
					'classes'=>array('in'),
					'panel_classes'=>array('panel-collapse collapse'),
				);
			}
