		...
		switch ($data['display']) {
			...
			if(!empty($tmdquestions_status)){
				...
				if(!empty($questions_qatab[$language_id]['qatab'])){
					$tabquestionanswer = $questions_qatab[$language_id]['qatab'];
				}else{
					$tabquestionanswer = $this->language->get('tab_questionanswer');
				}
			}
			
			$product_id=$this->request->get['product_id'];
			$this->load->model('extension/tmdquestionans');
			$hasquestions = $this->model_extension_tmdquestionans->getQuestionByProductId($product_id);
			$hasTotalquestions = $hasquestions;

			switch ($data['display']) {
				case 'tabs':
				$data['items'][]=array(
					'title'=>$tabquestionanswer.' ('.$hasTotalquestions .') ',
					'content'=>$this->load->controller('extension/tmdquestionans'),
					'display'=>'tab',
					'id'=>'qans',
					'sort'=>'4',
					'expandButton'=>'',
					...
					'contentType'=>'',
					'tab_classes'=>array(),
					'classes'=>array('0'=>'qans','tab-pane'=>'1'),
					'panel_classes'=>array('panel-collapse','tab-pane')
				);
			}
			// TMD Question Ans Module XML Code
