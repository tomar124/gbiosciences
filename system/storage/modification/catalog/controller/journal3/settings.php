...
class ControllerJournal3Settings extends Controller {
	...
	public function index() {
		...
		if ($this->journal3->settings->get('logo')) {
            ...
            if ($this->s3->getObject($logo, '', '', true)) {
                    list ($width, $height) = getimagesize(CDN_SERVER . ltrim($logo, '/'));

                    $this->journal3->settings->set('logo_width', $width);
                    $this->journal3->settings->set('logo_height', $height);
                    $this->journal3->settings->set('logo_src', CDN_SERVER . ltrim($logo, '/'));

                    $logo2x = $this->journal3->settings->get('logo2x');

                    if (is_file(DIR_IMAGE . $logo2x)) {
                            $this->journal3->settings->set('logo2x_src', CDN_SERVER . ltrim($logo2x, '/'));
                    } else {
                    ...
                    if ($this->journal3->document->isMobile()) {
                            ...
                            if ($this->s3->getObject($logoMobile2x, '', '', true)) {
                                    $this->journal3->settings->set('logo2x_src', CDN_SERVER . ltrim($logoMobile2x, '/'));
                            }
                    }
            } else {
