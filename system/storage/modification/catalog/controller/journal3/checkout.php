...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
		if (!$this->customer->isLogged() && $this->config->get('config_customer_price')) {
            ...
            if (isset($this->session->data['order_id']) && !empty($this->session->data['order_id'])) {
                    if($this->model_journal3_checkout->checkPayAndOrderDateDifference($this->session->data['order_id'])) {					
		...
		$data['payment_method_block'] = $this->renderView('journal3/checkout/payment_method', array(
            ...
            if($this->customer->isLogged()){
                    ...
                    $data['current_points'] = $this->model_account_customer->getRewardTotal($customer_id);
            }
            ...
            $data['coupon_voucher_reward_block'] = $this->renderView('journal3/checkout/coupon_voucher_reward', array(
                    ...
                    'alert_reward'   => sprintf($this->language->get('alert_reward'),$data['current_points'],$data['checkout_data']['order_data']['points']),
            ));
            }else{
                $data['coupon_voucher_reward_block'] = $this->renderView('journal3/checkout/coupon_voucher_reward', array(
                    'text_loading'   => $this->language->get('text_loading'),
	...
	public function cart_update() {
		...
		$qty = Arr::get($this->request->post, 'quantity');
            ...
            $cart_ids = array_column($this->cart->getProducts(), 'cart_id');

            $success = true;

            if (in_array($key, $cart_ids)) {
                    if (isset($products[array_search($key, $cart_ids)]['min_qty'])) {
                            if ($qty >= $products[array_search($key, $cart_ids)]['min_qty']) {
                                    ...
                                    $this->session->data['error'] = $this->language->get('error_minimum_quote');

                                    $json['redirect'] = $this->model_journal3_links->url('checkout/checkout', '', true);
	...
	private function products() {
        ...
        'href'       => str_replace('&amp;', '&', $this->url->link('product/product', 'product_id=' . $parent_id)),
        
				'thumb'      => $thumb,
				'thumb2x'    => $thumb2x,

            'cart_comment'    => isset($product['cart_comment']) ? $product['cart_comment'] : '',
	...
	private function registerGuest($order_data) {
		...
		if (Arr::get($this->request->post, 'newsletter') === 'true') {
            ...
            //Transffering data to hubspot
            $sendToHubspot = array(
                    'customer_info' => $account_data,
                    'address_info' => $shipping_address ? $shipping_address : $shipping_address
            );

            $this->hubspotData(0, $sendToHubspot);
	...
	private function registerAccount($order_data) {
		...
		if ($customer_group_info && !$customer_group_info['approval']) {
            ...
            //Transffering data to hubspot
            $this->hubspotData($customer_id);
            
		return null;
	}


            public function hubspotData($customer_id, $customData = array()) {
                    if ($customer_id && $customer_id !== 0) {
                            $customer_info = $this->model_account_customer->getCustomer($customer_id);
                    } else {
                            $customer_info = $customData['customer_info'];
                    }
                    ...
                    $hubspot_data = array(
                            'properties' => array(
                                    array(
                                            'property' => 'firstname',
                                            'value' => $customer_info['firstname']
                                    ),
                                    array(
                                            'property' => 'lastname',
                                            'value' => $customer_info['lastname']
                                    ),
                                    array(
                                            'property' => 'email',
                                    ...
                                    array(
                                            'property' => 'fax',
                                            'value' => isset($customer_info['input-custom-field3']) ? $customer_info['input-custom-field3'] : ''
                                    ),
                                    array(
                                            'property' => 'company',
                                            'value' => ''
                    ...
                    if ($customer_id && $customer_id !== 0) {
                            $address_info = $this->model_account_customer->getAddress($customer_info['address_id']);
                    } else {
                            $address_info = $customData['address_info'];
                    }
                    ...
                    if($address_info){
                            ...
                            //Hubspot
                            $hubspot_data['properties'][5]['value'] = $address_info['company'];
                            $hubspot_data['properties'][6]['value'] = $address_info['address_1'] . ' ' . $address_info['address_2'];
                            $hubspot_data['properties'][7]['value'] = $address_info['city'];
                            $hubspot_data['properties'][8]['value'] = $zone_info['name'] ?? $address_info['zone'];
                            $hubspot_data['properties'][9]['value'] = $address_info['postcode'];
                            $hubspot_data['properties'][10]['value'] = $country_info['name'] ?? $address_info['country'];
                    }

                    $this->load->library('hubspot');
                    $this->hubspot->_create_update(json_encode($hubspot_data), $customer_info['email']);
            }
