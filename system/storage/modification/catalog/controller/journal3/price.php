	...
	public function index() {
		...
		if ($product_info) {
			...
			$product_option_values = $this->model_journal3_product->getProductOptionValues($product_id, Arr::get($this->request->post, 'option', array()));
                ...
                $product_options = $this->model_catalog_product->getProductOptions($product_id);
                        ...
                        foreach ($product_options as $index => $product_option) {
                                $product_option = $product_option['product_option_value'];

                                for ($i = 0; $i < count($product_option); $i++) {
                                                $product_options_new[$product_option[$i]['option_value_id']] = $product_option[$i]['name'];
                                }
                        }
                

			
                foreach ($product_option_values as $index => $product_option_value) {
                $product_option_values[$index]['value'] = $product_options_new[$product_option_value['option_value_id']];
			...
			foreach ($this->model_catalog_product->getProductDiscounts($product_id) as $discount) {
                ...
                $data['catalog-modal'] = $this->cart->formatProductName($product_info['name'], $product_option_values);
