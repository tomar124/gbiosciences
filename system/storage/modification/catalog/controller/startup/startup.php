...
class ControllerStartupStartup extends Controller {
	public function index() {
		...
		if (!isset($this->request->cookie['language']) || $this->request->cookie['language'] != $code) {
			setcookie('language', $code, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST'], ini_get('session.cookie_secure'), ini_get('session.cookie_httponly'));
		}
		...
		if (!isset($this->request->cookie['currency']) || $this->request->cookie['currency'] != $code) {
			setcookie('currency', $code, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST'], ini_get('session.cookie_secure'), ini_get('session.cookie_httponly'));
		}		
