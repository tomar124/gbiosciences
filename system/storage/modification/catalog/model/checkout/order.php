...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', list_price = '" . (isset($product['list_price']) ? (float)$product['list_price'] : 0) . "', list_price_total = '" . (isset($product['list_price_total']) ? (float)$product['list_price_total'] : 0) . "', is_quote = '" . (isset($product['is_quote']) ? (int)$product['is_quote'] : 0) . "'");
	...
	public function getOrderTotals($order_id) {
		...
		return $query->rows;
            ...
            public function hubspotData($email) {
                    ...
                    $this->hubspot->_update_by_email(json_encode($hubspot_data), $email);
            }
            

            public function updatePONumber($order_id, $po) {
                    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET po_number = '" . $this->db->escape($po) . "' WHERE order_id = '" . (int)$order_id . "'");
            }
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
            ...
            //Update customer to hubspot
            $this->hubspotData($order_info['email']);
            ...
            // If order status in the complete range assign reward values to customer account.
            if (in_array($order_status_id, $this->config->get('config_complete_status'))) {
                    //Reward automatically transfer
                    $rewardQuery = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_reward WHERE order_id = '" . (int)$order_info['order_id'] . "' AND points > 0");

                    if(!$rewardQuery ->num_rows){
                            $rewardSubQuery = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_info['order_id'] . "'");

                            if($rewardSubQuery ->num_rows){
                                    $reward = 0;

                                    foreach($rewardSubQuery ->rows as $product){
                                        $reward += $product['reward'];
                                    }

                                    if($reward && $order_info['customer_id'] != 0){
                                            $this->db->query("INSERT INTO " . DB_PREFIX . "customer_reward SET customer_id = '" . (int)$order_info['customer_id'] . "', order_id = '" . (int)$order_info['order_id'] . "', description = '" . $this->db->escape(sprintf('Order ID: #%s', (int)$order_info['order_id'])) . "', points = '" . (float)$reward . "', date_added = NOW()");
                                    }
                            }
                    }
            } else {
