...
class ModelAccountReturn extends Model {
	public function addReturn($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return` SET order_id = '" . (int)$data['order_id'] . "', customer_id = '" . (int)$this->customer->getId() . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', product = '" . $this->db->escape($data['product']) . "', model = '" . $this->db->escape($data['model']) . "', quantity = '" . (int)$data['quantity'] . "', opened = '" . (int)$data['opened'] . "', return_reason_id = '" . (int)$data['return_reason_id'] . "', return_status_id = '" . (int)$this->config->get('config_return_status_id') . "', comment = '" . $this->db->escape($data['comment']) . "', date_ordered = '" . $this->db->escape($data['date_ordered']) . "', date_added = NOW(), date_modified = NOW()");
            ...
            $emailTemplate = $this->emailTemplate(14);
            if($emailTemplate){
                $returnReason = $this->getReturnReason($data['return_reason_id']);
                $body =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                $body = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[EMAIL]', '[TELEPHONE]', '[ORDER-ID]', '[ORDER-DATE-TIME]', '[RETURN-PRODUCT]', '[RETURN-PRODUCT-DESCRIPTION]', '[RETURN-OPENED]', '[RETURN-REASON]', '[RETURN-COMMENT]'), array($data['firstname'], $data['lastname'], $data['email'], $data['telephone'], $data['order_id'], date($this->language->get('date_format_short'), strtotime($data['date_ordered'])), $data['product'], htmlentities($data['model']), ($data['opened'] ? 'Opened' : 'Unopened'), ($returnReason ? $returnReason['name'] : ''), $data['comment']), $body);
                $subject = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[RETURN-ID]'), array($this->db->escape($data['firstname']), $this->db->escape($data['lastname']), $return_id), $emailTemplate['email_subject']);
                ...
                if ($this->config->get('config_return_email') && !empty($this->config->get('config_email'))) {
                    $mail->setTo($this->config->get('config_return_email'));
                    $mail->setFrom($this->config->get('config_email'));
                    $mail->setSender(html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
                    $mail->setSubject($subject);
                    $mail->setHtml($body);
            ...
            $emailTemplate = $this->emailTemplate(15);
            if($emailTemplate){
                $returnReason = $this->getReturnReason($data['return_reason_id']);
                $body =  html_entity_decode($emailTemplate['description'], ENT_QUOTES, "UTF-8");
                $body = str_replace(array('[FIRST-NAME]', '[RETURN-ID]'), array($data['firstname'], $return_id), $body);
                $subject = str_replace(array('[FIRST-NAME]', '[LAST-NAME]', '[RETURN-ID]'), array($this->db->escape($data['firstname']), $this->db->escape($data['lastname']), $return_id), $emailTemplate['email_subject']);
