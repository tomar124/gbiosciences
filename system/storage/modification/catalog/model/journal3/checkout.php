...
class ModelJournal3Checkout extends Model {
	...
	public function __construct($registry) {
		...
		if ($this->journal3->isOC2()) {
            ...
            public function checkPayAndOrderDateDifference($order_id){
                    return $this->db->query("SELECT date_added FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "' AND ( DATE(NOW()) - DATE(date_added) > 1)")->num_rows;
            }
	...
	private function products() {
		...
		foreach ($this->cart->getProducts() as $product) {
            ...
            'list_price' => isset($product['list_price']) ? $product['list_price'] : $product['price'],
            'list_price_total' => isset($product['list_price_total']) ? $product['list_price_total'] : $product['price'] * $product['quantity'],
            'is_quote' => isset($product['is_quote']) ? $product['is_quote'] : 0
