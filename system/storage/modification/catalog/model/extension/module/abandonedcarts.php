...
class ModelExtensionModuleAbandonedcarts extends Model {
	...
	public function getCarts($dayLimit, $store_id = 0) {
	    ...
	    public function getCustomCarts($cronHour, $store_id = 0) {
                $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "abandonedcarts` WHERE (HOUR(TIMEDIFF(NOW(), date_created)) >= '".$cronHour."' AND HOUR(TIMEDIFF(NOW(), date_created)) < '". ($cronHour + 1) ."' ) AND `ordered`=0 AND `store_id`=".$this->db->escape($store_id)." ");
                    return $query->rows;
            }
