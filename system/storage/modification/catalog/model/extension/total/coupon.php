...
class ModelExtensionTotalCoupon extends Model {
	...
	public function getTotal($total) {
		if (isset($this->session->data['coupon'])) {
            ...
            $hazmat[] = $product['hazardous'];
            $handling[] = $product['shipping_code'];
            ...
            //Functionality to remove hazmat fee if admin sets so
            if ($coupon_info['hazmat'] && $hazmat) {
                    if(in_array(1, $hazmat)){
                        $discount_total += $this->config->get('config_shipping_method_hazardous_accessible') ? $this->config->get('config_shipping_method_hazardous_accessible') : 0;
                    }

                    if(in_array(2, $hazmat)){
                        $discount_total += $this->config->get('config_shipping_method_hazardous_inaccessible') ? $this->config->get('config_shipping_method_hazardous_inaccessible') : 0;
                    }
            }

            //Functionality to remove handling fee if admin sets so
            if ($coupon_info['handling'] && $handling) {
                    if(in_array('2DAY', $handling) || in_array('AMBIENT', $handling)){
                            $discount_total += $this->config->get('config_shipping_method_ambient') ? $this->config->get('config_shipping_method_ambient') : 0;
                    }

                    if(in_array('BLUE', $handling)){
                            $discount_total += $this->config->get('config_shipping_method_blue_ice') ? $this->config->get('config_shipping_method_blue_ice') : 0;
                    }

                    if(in_array('DRY', $handling)){
                            $discount_total += $this->config->get('config_shipping_method_dry_ice') ? $this->config->get('config_shipping_method_dry_ice') : 0;
                    }

                    if(in_array('STANDARD', $handling)){
                            $discount_total += $this->config->get('config_shipping_method_standard') ? $this->config->get('config_shipping_method_standard') : 0;
                    }

                    foreach($total_data as $handling){
                            if($handling['code'] == 'handling'){
                                    ...
                                    if(isset($this->session->data['shipping_address']['country_id']) && $this->config->get('config_country_id') != $this->session->data['shipping_address']['country_id']){
                                            $discount_total += $this->config->get('handling_fee_international');
                                    } else {
                                            $discount_total += $this->config->get('handling_fee');
                                    }
                            }
                    }
            }
