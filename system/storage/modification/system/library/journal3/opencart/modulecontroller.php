...
abstract class ModuleController extends Controller {
	...
	public function index($args) {
		...
		if ($this->_cache === false) {
			...
			$parser = new Parser('module/' . $this->module_type . '/general', Arr::get($this->module_data, 'general'), null, array($this->module_id), $this->s3);
			...
			if ($parser->getSetting('status') !== false && Arr::get($this->settings, 'items') === null) {
				...
				foreach ($items as $item) {
					...
					$parser = new Parser('module/' . $this->module_type . '/item', $item, null, array($this->module_id, $this->item_id), $this->s3);
					...
					if (Arr::get($this->settings['items'][$this->item_id], 'items') === null) {
						...
						foreach ($subitems as $subitem) {
							...
							$parser = new Parser('module/' . $this->module_type . '/subitem', $subitem, null, array($this->module_id, $this->item_id, $this->subitem_id), $this->s3);
