...
class ControllerExtensionPaymentPPPro extends Controller {
	...
	public function send() {
		...
		$this->load->model('custom/paymentpppro');
		$tmp = $this->model_custom_paymentpppro->pymntppprovalidation();
	    $this->log->write("check validation pp_pro". $tmp);
	
		if($this->model_custom_paymentpppro->pymntppprovalidation()) 
		{
			$this->load->language('custom/paymentpppro');
			$json = array();
			...
			if ($this->cart->hasShipping()) {
				...
				if (!$response || curl_errno($curl)) {
					...
					// save error_response if have
					$this->model_custom_paymentpppro->save_response($response, $this->session->data['order_id']);
				}
				...
				if (($response_info['ACK'] == 'Success') || ($response_info['ACK'] == 'SuccessWithWarning')) {
					...
					// if order is success make the attempt = 2, so that in loop order can't be triggered even if cart is cleared.
					$this->model_custom_paymentpppro->save_response(json_encode($response), $this->session->data['order_id'], 2);
