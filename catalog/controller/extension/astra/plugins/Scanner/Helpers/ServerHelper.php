...
class ServerHelper
{
    ...
    public static function fileTooBig($file)
    {
        ...
        $tooBig = \false;
        try {
            if (0 === @\fseek($fh, $offset, \SEEK_SET)) {
                if (1 === \strlen(\fread($fh, 1))) {
                    $tooBig = \true;
                }
            }
    ...
    {
        self::mbstring_binary_safe_encoding(\true);
    }
    ...
    public static function potentialBinaryStringToHTML($string, $inline = \false, $allowmb4 = \false)
    {
        ...
        for ($i = 0; $i < self::strlen($string); ++$i) {
            ...
            if ($b < 0x20) {
                ...
                $brokenUTF8 = $i + $bytes > self::strlen($string) || 1 == $bytes;
                if (!$brokenUTF8) {
                    //Make sure we have all the bytes
                    for ($n = 1; $n < $bytes; ++$n) {
                        $c2 = $string[$i + $n];
                        $b2 = \ord($c2);
                        if (($b2 & 0xc0) != 0x80) {
                ...
                if (!$brokenUTF8) {
                    //Ensure the byte sequences are within the accepted ranges: https://tools.ietf.org/html/rfc3629
                    /*
                    ...
                    $regex = '/^(?:' . '[\\xc2-\\xdf][\\x80-\\xbf]' . '|' . '\\xe0[\\xa0-\\xbf][\\x80-\\xbf]' . '|' . '[\\xe1-\\xec][\\x80-\\xbf]{2}' . '|' . '\\xed[\\x80-\\x9f][\\x80-\\xbf]' . '|' . '[\\xee-\\xef][\\x80-\\xbf]{2}';
                    if ($allowmb4) {
