...
{
    public static $test_files = ['/webservice/dispatcher.php', '/bootstrap.php', '/vendor/autoload.php'];
    /**
    ...
    {
        $tests = ['upload' => \false, 'cache_dir' => 'var/cache', 'log_dir' => 'var/logs', 'plugins_dir' => 'plugins'];
        if (!\defined('_PS_HOST_MODE_')) {
            $tests = \array_merge($tests, ['system' => ['fopen', 'fclose', 'fread', 'fwrite', 'rename', 'file_exists', 'unlink', 'rmdir', 'mkdir', 'getcwd', 'chdir', 'chmod'], 'phpversion' => \false, 'apache_mod_rewrite' => \false, 'curl' => \false, 'json' => \false, 'pdo_mysql' => \false, 'config_dir' => 'config', 'openssl' => 'false', 'simplexml' => \false, 'zip' => \false, 'fileinfo' => \false]);
        }
        return $tests;
    }
    /**
    ...
    {
        return ['new_phpversion' => \false, 'gz' => \false, 'mbstring' => \false, 'dom' => \false, 'pdo_mysql' => \false, 'fopen' => \false, 'intl' => \false];
    }
    /**
    ...
    {
        if (\call_user_func(['ConfigCheckHelper', 'test_' . $ptr], $arg)) {
            return 'ok';
        }
        return 'fail';
    }
    ...
    {
        if (isset($_SERVER['SERVER_SOFTWARE']) && \false === \strpos(\strtolower($_SERVER['SERVER_SOFTWARE']), 'apache') || !\function_exists('apache_get_modules')) {
            return \true;
        }
    ...
    {
        $dir = \rtrim(ASTRAROOT, '\\/') . \DIRECTORY_SEPARATOR . \trim($relative_dir, '\\/');
        if (!\file_exists($dir) || !($dh = @\opendir($dir))) {
            $full_report = \sprintf('Directory %s does not exist or is not writable', $dir);
    ...
    {
        return ConfigCheckHelper::test_dir($dir, \true);
    }
    ...
    {
        $absoluteDir = \rtrim(ASTRAROOT, '\\/') . \DIRECTORY_SEPARATOR . \trim($dir, '\\/');
