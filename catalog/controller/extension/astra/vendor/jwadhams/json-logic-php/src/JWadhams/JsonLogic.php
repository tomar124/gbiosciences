...
namespace AstraPrefixed\JWadhams;
...
class JsonLogic
{
    ...
    public static function get_values($logic, $fix_unary = \true)
    {
    ...
    {
        return \is_array($array) and \count($array) === 1 and \is_string(static::get_operator($array));
    }
    ...
    public static function apply($logic = [], $data = [])
    {
        ...
        $operators = ['==' => function ($a, $b) {
            ...
            foreach (\explode('.', $a) as $prop) {
                if ((\is_array($data) || $data instanceof \ArrayAccess) && isset($data[$prop])) {
                    ...
                    return $default;
                    //Trying to get a value from a primitive
                }
            }
            return $data;
        ...
        if ($op === 'if' || $op == '?:') {
            ...
            return \array_values(\array_filter($scopedData, function ($datum) use($scopedLogic) {
            ...
            return \array_map(function ($datum) use($scopedLogic) {
            ...
            return \array_reduce($scopedData, function ($accumulator, $current) use($scopedLogic) {
                return static::apply($scopedLogic, ['current' => $current, 'accumulator' => $accumulator]);
            }, $initial);
        } elseif ($op === "all") {
            $scopedData = static::apply($values[0], $data);
            $scopedLogic = $values[1];
    ...
    public static function rule_like($rule, $pattern)
    {
        if (\is_string($pattern) and $pattern[0] === '{') {
            $pattern = \json_decode($pattern, \true);
        }
        ...
        if (static::is_logic($pattern)) {
            if (static::is_logic($rule)) {
                ...
                if ($pattern_op === "@" || $pattern_op === $rule_op) {
                    ...
                    return static::rule_like(static::get_values($rule, \false), static::get_values($pattern, \false));
                }
            }
            return \false;
            //$pattern is logic, rule isn't, can't be eq
        }
        if (\is_array($pattern)) {
            if (\is_array($rule)) {
                ...
                */
                ...
                return \true;
                //If they *all* passed, we pass
            } else {
                return \false;
                //Pattern is array, rule isn't
            }
        }
