 ...
 */
namespace AstraPrefixed\Slim\Handlers;
...
class PhpError extends AbstractError
{
    ...
    {
        $json = ['message' => 'Slim Application Error'];
        if ($this->displayErrorDetails) {
            $json['error'] = [];
            do {
                $json['error'][] = ['type' => \get_class($error), 'code' => $error->getCode(), 'message' => $error->getMessage(), 'file' => $error->getFile(), 'line' => $error->getLine(), 'trace' => \explode("\n", $error->getTraceAsString())];
            } while ($error = $error->getPrevious());
        }
