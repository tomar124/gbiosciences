...
use Closure;
use Exception;
use AstraPrefixed\FastRoute\Dispatcher;
use AstraPrefixed\Psr\Container\ContainerExceptionInterface as ContainerException;
...
use AstraPrefixed\Slim\Exception\InvalidMethodException;
use AstraPrefixed\Slim\Exception\MethodNotAllowedException;
use AstraPrefixed\Slim\Exception\NotFoundException;
use AstraPrefixed\Slim\Exception\SlimException;
use AstraPrefixed\Slim\Http\Body;
use AstraPrefixed\Slim\Http\Headers;
use AstraPrefixed\Slim\Http\Request;
use AstraPrefixed\Slim\Http\Uri;
use AstraPrefixed\Slim\Interfaces\RouteGroupInterface;
use AstraPrefixed\Slim\Interfaces\RouteInterface;
...
class App
{
    ...
    protected function processInvalidMethod(ServerRequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($routeInfo[RouterInterface::DISPATCH_STATUS] === Dispatcher::METHOD_NOT_ALLOWED) {
            return $this->handleException(new MethodNotAllowedException($request, $response, $routeInfo[RouterInterface::ALLOWED_METHODS]), $request, $response);
        }
    ...
    public function process(ServerRequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($this->container->get('settings')['determineRouteBeforeAppMiddleware'] === \true) {
    ...
    public function respond(ResponseInterface $response)
    {
        ...
        if (!\headers_sent()) {
            ...
            foreach ($response->getHeaders() as $name => $values) {
                $first = \stripos($name, 'Set-Cookie') === 0 ? \false : \true;
    ...
    protected function finalize(ResponseInterface $response)
    {
        ...
        if (isset($this->container->get('settings')['addContentLengthHeader']) && $this->container->get('settings')['addContentLengthHeader'] == \true) {
            if (\ob_get_length() > 0) {
