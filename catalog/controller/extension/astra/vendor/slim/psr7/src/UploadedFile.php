...
namespace AstraPrefixed\Slim\Psr7;

use InvalidArgumentException;
use AstraPrefixed\Psr\Http\Message\StreamInterface;
use AstraPrefixed\Psr\Http\Message\UploadedFileInterface;
use RuntimeException;
use AstraPrefixed\Slim\Psr7\Factory\StreamFactory;
class UploadedFile implements UploadedFileInterface
{
     ...
     */
    public function __construct($fileNameOrStream, ?string $name = null, ?string $type = null, ?int $size = null, int $error = \UPLOAD_ERR_OK, bool $sapi = \false)
    {
    ...
    private static function parseUploadedFiles(array $uploadedFiles) : array
    {
        ...
        foreach ($uploadedFiles as $field => $uploadedFile) {
            ...
            if (!\is_array($uploadedFile['error'])) {
                $parsed[$field] = new static($uploadedFile['tmp_name'], isset($uploadedFile['name']) ? $uploadedFile['name'] : null, isset($uploadedFile['type']) ? $uploadedFile['type'] : null, isset($uploadedFile['size']) ? $uploadedFile['size'] : null, $uploadedFile['error'], \true);
            } else {
                $subArray = [];
