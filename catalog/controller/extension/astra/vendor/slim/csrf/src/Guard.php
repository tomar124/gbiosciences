...
namespace AstraPrefixed\Slim\Csrf;
...
class Guard
{
     ...
     */
    public function __construct($prefix = 'csrf', &$storage = null, callable $failureCallable = null, $storageLimit = 200, $strength = 16, $persistentTokenMode = \false)
    {
    ...
    public function generateToken()
    {
        ...
        $this->keyPair = [$this->prefix . '_name' => $name, $this->prefix . '_value' => $value];
        return $this->keyPair;
    }
    /**
    ...
    protected function getLastKeyPair()
    {
        ...
        $keyPair = [$this->prefix . '_name' => $name, $this->prefix . '_value' => $value];
        return $keyPair;
    }
    /**
    ...
    protected function enforceStorageLimit()
    {
        ...
        if (!\is_array($this->storage) && !($this->storage instanceof Countable && $this->storage instanceof Traversable)) {
            return;
        }
        if (\is_array($this->storage)) {
    ...
    {
        return $request->withAttribute($this->prefix . '_name', $pair[$this->prefix . '_name'])->withAttribute($this->prefix . '_value', $pair[$this->prefix . '_value']);
    }
    /**
    ...
    public function getFailureCallable()
    {
        if (\is_null($this->failureCallable)) {
            $this->failureCallable = function (ServerRequestInterface $request, ResponseInterface $response, $next) {
                $body = new \AstraPrefixed\Slim\Http\Body(\fopen('php://temp', 'r+'));
