...
class JsonApiSerializer extends ArraySerializer
{
    ...
    public function item($resourceKey, array $data)
    {
        ...
        $resource = ['data' => ['type' => $resourceKey, 'id' => "{$id}", 'attributes' => $data]];
    ...
    {
        return \array_key_exists('data', $data) && \array_key_exists(0, $data['data']);
    }
    /**
     * @param array $data
     *
    ...
    private function fillRelationshipAsCollection($data, $relationship, $key)
    {
        foreach ($relationship as $index => $relationshipData) {
            ...
            if ($this->shouldIncludeLinks()) {
                $data['data'][$index]['relationships'][$key] = \array_merge(['links' => ['self' => "{$this->baseUrl}/{$data['data'][$index]['type']}/{$data['data'][$index]['id']}/relationships/{$key}", 'related' => "{$this->baseUrl}/{$data['data'][$index]['type']}/{$data['data'][$index]['id']}/{$key}"]], $data['data'][$index]['relationships'][$key]);
            }
        }
        return $data;
    }
    ...
    private function fillRelationshipAsSingleResource($data, $relationship, $key)
    {
        ...
        if ($this->shouldIncludeLinks()) {
            $data['data']['relationships'][$key] = \array_merge(['links' => ['self' => "{$this->baseUrl}/{$data['data']['type']}/{$data['data']['id']}/relationships/{$key}", 'related' => "{$this->baseUrl}/{$data['data']['type']}/{$data['data']['id']}/{$key}"]], $data['data']['relationships'][$key]);
            return $data;
        }
        return $data;
    }
    ...
    private function buildRelationships($includeKey, $relationships, $includeObject, $key)
    {
        ...
        if ($this->isNull($includeObject)) {
            ...
            $relationship = ['data' => ['type' => $includeObject['data']['type'], 'id' => $includeObject['data']['id']]];
        }
    ...
    private function addIncludedDataToRelationship($includeObject, $relationship)
    {
        foreach ($includeObject['data'] as $object) {
            $relationship['data'][] = ['type' => $object['type'], 'id' => $object['id']];
        }
        return $relationship;
    }
    /**
