...
namespace AstraPrefixed\Curl;

use AstraPrefixed\Curl\ArrayUtil;
use AstraPrefixed\Curl\Decoder;
use AstraPrefixed\Curl\Url;
class Curl
{
    ...
    public function buildPostData($data)
    {
        ...
        if (isset($this->headers['Content-Type']) && \preg_match($this->jsonPattern, $this->headers['Content-Type']) && (\is_array($data) || \is_object($data) && \interface_exists('JsonSerializable', \false) && $data instanceof \JsonSerializable)) {
            $data = \AstraPrefixed\Curl\Encoder::encodeJson($data);
        ...
        if (!$binary_data && (\is_array($data) || \is_object($data)) && (!isset($this->headers['Content-Type']) || !\preg_match('/^multipart\\/form-data/', $this->headers['Content-Type']))) {
    ...
    public function _fastDownload($url, $filename, $connections = 4)
    {
        ...
        $this->setOpts(array(\CURLOPT_CUSTOMREQUEST => 'GET', \CURLOPT_NOBODY => \true, \CURLOPT_HEADER => \true, \CURLOPT_ENCODING => ''));
        $this->setUrl($url);
        $this->exec();
    ...
    public function exec($ch = null)
    {
        ...
        if ($this->getOpt(\CURLINFO_HEADER_OUT) === \true) {
    ...
    public function get($url, $data = array())
    {
        ...
        $this->setOpt(\CURLOPT_HTTPGET, \true);
        return $this->exec();
    }
    /**
     * Get Info
    ...
    public function head($url, $data = array())
    {
        ...
        $this->setOpt(\CURLOPT_NOBODY, \true);
        return $this->exec();
    }
    /**
     * Options
        ...
        if ($follow_303_with_post) {
            ...
            if (isset($this->options[\CURLOPT_CUSTOMREQUEST])) {
                if (\version_compare(\PHP_VERSION, '5.5.11') < 0) {
                    \trigger_error('Due to technical limitations of PHP <= 5.5.11, it is not possible to ' . 'perform a post-redirect-get request using a php-curl-class Curl object that ' . 'has already been used to perform other types of requests. Either use a new ' . 'php-curl-class Curl object or upgrade your PHP engine.', \E_USER_ERROR);
                } else {
                    $this->setOpt(\CURLOPT_CUSTOMREQUEST, null);
                }
            }
        }
        $this->setOpt(\CURLOPT_POST, \true);
        $this->setOpt(\CURLOPT_POSTFIELDS, $this->buildPostData($data));
        return $this->exec();
    }
    /**
    ...
    {
        $this->jsonDecoder = 'AstraPrefixed\\Curl\\Decoder::decodeJson';
    ...
    {
        $this->xmlDecoder = 'AstraPrefixed\\Curl\\Decoder::decodeXml';
    ...
    public function setDefaultDecoder($mixed = 'json')
    {
        if ($mixed === \false) {
            ...
            if ($mixed === 'json') {
                $this->defaultDecoder = 'AstraPrefixed\\Curl\\Decoder::decodeJson';
            } elseif ($mixed === 'xml') {
                $this->defaultDecoder = 'AstraPrefixed\\Curl\\Decoder::decodeXml';
            }
        }
    }
    /**
    ...
    public function setOpt($option, $value)
    {
        ...
        if (\in_array($option, \array_keys($required_options), \true) && $value !== \true) {
     ...
     */
    public function setProxyTunnel($tunnel = \true)
    {
     ...
     */
    public function verbose($on = \true, $output = 'STDERR')
    {
        if ($output === 'STDERR') {
            if (!\defined('STDERR')) {
     ...
     * @access public
     */
    public function setAutoReferer($auto_referer = \true)
    {
     ...
     * @access public
     */
    public function setAutoReferrer($auto_referrer = \true)
    {
     ...
     * @access public
     */
    public function setFollowLocation($follow_location = \true)
    {
     ...
     * @access public
     */
    public function setForbidReuse($forbid_reuse = \true)
    {
    ...
    {
        return \array_fill_keys(self::$RFC2616, \true);
    }
    /**
     * Get RFC 6265
     *
    ...
    {
        return \array_fill_keys(self::$RFC6265, \true);
    }
    /**
     * Get Total Time
     *
    ...
    private function downloadComplete($fh)
    {
        ...
        $this->setOpt(\CURLOPT_RETURNTRANSFER, \true);
    }
    /**
     * Parse Headers
     *
    ...
    private function initialize($base_url = null)
    {
        ...
        $this->setOpt(\CURLINFO_HEADER_OUT, \true);
