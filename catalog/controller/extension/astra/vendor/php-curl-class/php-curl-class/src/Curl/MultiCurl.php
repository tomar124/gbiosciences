...
class MultiCurl
{
    ...
    public function addDownload($url, $mixed_filename)
    {
        ...
        $curl->setOpt(\CURLOPT_HTTPGET, \true);
        return $curl;
    }
    /**
     * Add Get
    ...
    public function addHead($url, $data = array())
    {
        ...
        $curl->setOpt(\CURLOPT_NOBODY, \true);
        return $curl;
    }
    /**
     * Add Options
    ...
    public function setOpt($option, $value)
    {
        ...
        if ($option === \CURLOPT_URL) {
            foreach ($this->curls as $curl_id => $curl) {
                if (!isset($this->instanceSpecificOptions[$curl_id][$option]) || $this->instanceSpecificOptions[$curl_id][$option] === null) {
    ...
    {
        $rate_limit_pattern = '/' . '^' . '(\\d+)' . '\\/' . '(\\d+)?' . '(s|m|h)' . '$' . '/' . 'i' . '';
        if (!\preg_match($rate_limit_pattern, $rate_limit, $matches)) {
            throw new \UnexpectedValueException('rate limit must be formatted as $max_requests/$interval(s|m|h) ' . '(e.g. "60/1m" for a maximum of 60 requests per 1 minute)');
        }
    ...
    public function start()
    {
        ...
        $this->currentStartTime = \microtime(\true);
        $this->currentRequestCount = 0;
        do {
            while (\count($this->curls) && \count($this->activeCurls) < $this->concurrency && (!$this->rateLimitEnabled || $this->hasRequestQuota())) {
                $this->initHandle();
            }
    ...
    private function waitUntilRequestQuotaAvailable()
    {
        ...
        $sleep_seconds = $sleep_until - \microtime(\true);
