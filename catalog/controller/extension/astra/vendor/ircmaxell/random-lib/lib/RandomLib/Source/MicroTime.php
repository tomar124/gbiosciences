 ...
 * @version    Build @@version@@
 */
namespace AstraPrefixed\RandomLib\Source;

use AstraPrefixed\SecurityLib\Strength;
use AstraPrefixed\SecurityLib\Util;
/**
 * The Microtime Random Number Source
 *
 * This uses the current micro-second (looped several times) for a **very** weak
 ...
 * @codeCoverageIgnore
 */
final class MicroTime extends \AstraPrefixed\RandomLib\AbstractSource
{
    /**
    ...
    public function __construct()
    {
        ...
        if (!\defined('HHVM_VERSION') && \function_exists('AstraPrefixed\\zend_thread_id')) {
            $state .= zend_thread_id();
        }
        if (\function_exists('AstraPrefixed\\hphp_get_thread_id')) {
        ...
        self::$state = \hash('sha512', $state, \true);
    ...
    public function generate($size)
    {
        ...
        self::$state = \hash('sha512', self::$state . $seed, \true);
        /**
        ...
        for ($i = 0; $i < $size; $i += 8) {
            ...
            self::$state = \hash('sha512', $seed, \true);
            /**
