...
class City extends Country
{
    ...
    public function __construct($raw, $locales = ['en'])
    {
        ...
        $this->city = new \AstraPrefixed\GeoIp2\Record\City($this->get('city'), $locales);
        $this->location = new \AstraPrefixed\GeoIp2\Record\Location($this->get('location'));
        $this->postal = new \AstraPrefixed\GeoIp2\Record\Postal($this->get('postal'));
    ...
    private function createSubdivisions($raw, $locales)
    {
        ...
        foreach ($raw['subdivisions'] as $sub) {
            \array_push($this->subdivisions, new \AstraPrefixed\GeoIp2\Record\Subdivision($sub, $locales));
        }
    }
    /**
     * @ignore
    ...
    {
        return empty($this->subdivisions) ? new \AstraPrefixed\GeoIp2\Record\Subdivision([], $this->locales) : \end($this->subdivisions);
    }
}
