...
use AstraPrefixed\Respect\Validation\Validatable;
abstract class AbstractRelated extends AbstractRule
{
    public $mandatory = \true;
    public $reference = '';
    public $validator;
    public abstract function hasReference($input);
    public abstract function getReferenceValue($input);
    public function __construct($reference, Validatable $validator = null, $mandatory = \true)
    {
    ...
    {
        return !$this->mandatory && !$hasReference || (\is_null($this->validator) || $this->validator->{$type}($this->getReferenceValue($input)));
    }
    public function assert($input)
    {
        ...
        try {
            ...
            throw $this->reportError($this->reference, ['hasReference' => \true])->addRelated($e);
        }
    }
