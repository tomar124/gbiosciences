...
class Domain extends AbstractComposite
{
    ...
    public function __construct($tldCheck = \true)
    {
        $this->checks[] = new NoWhitespace();
        $this->checks[] = new Contains('.');
        $this->checks[] = new Length(3, null);
        $this->tldCheck($tldCheck);
        $this->otherParts = new AllOf(new Alnum('-'), new Not(new StartsWith('-')), new OneOf(new Not(new Contains('--')), new Callback(function ($str) {
            return \substr_count($str, '--') == 1;
        })), new Not(new EndsWith('-')));
    }
    public function tldCheck($do = \true)
    {
        if ($do === \true) {
            $this->tld = new Tld();
        } else {
            $this->tld = new AllOf(new Not(new StartsWith('-')), new NoWhitespace(), new Length(2, null));
        }
        return \true;
    }
    public function validate($input)
    {
        ...
        if (\count($parts = \explode('.', $input)) < 2 || !$this->tld->validate(\array_pop($parts))) {
            return \false;
        }
        foreach ($parts as $p) {
