...
class Ip extends AbstractRule
{
    ...
    {
        return (bool) \filter_var($address, \FILTER_VALIDATE_IP, ['flags' => $this->ipOptions]);
    }
    protected function verifyNetwork($input)
    {
        ...
        return \bccomp($input, \sprintf('%u', \ip2long($this->networkRange['min']))) >= 0 && \bccomp($input, \sprintf('%u', \ip2long($this->networkRange['max']))) <= 0;
    }
