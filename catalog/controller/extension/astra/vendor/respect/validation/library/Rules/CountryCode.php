...
class CountryCode extends AbstractRule
{
    ...
    {
        $index = \array_search($set, self::getAvailableSets(), \true);
        if (\false === $index) {
    ...
    {
        return [self::ALPHA2, self::ALPHA3, self::NUMERIC];
    }
    ...
    {
        return \in_array(\strtoupper($input), $this->getCountryCodeList($this->index), \true);
    }
}
