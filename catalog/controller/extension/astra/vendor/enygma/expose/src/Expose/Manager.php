...
class Manager
{
     ...
     */
    public function __construct(\AstraPrefixed\Expose\FilterCollection $filters, \AstraPrefixed\Psr\Log\LoggerInterface $logger = null, \AstraPrefixed\Expose\Queue $queue = null)
    {
    ...
    {
        $this->getLogger()->info('Executing on data ' . \md5(\print_r($data, \true)));
        if ($queueRequests === \true) {
        ...
        $converter = new \AstraPrefixed\Expose\Converter\Converter();
        ...
        if ($threshold !== null && $impact >= $threshold && $notify === \true) {
            return $this->sendNotification($filterMatches);
        } else {
            if ($threshold === null && $notify === \true) {
                return $this->sendNotification($filterMatches);
            }
        }
        return \true;
    ...
    protected function processFilters($value, $index, $path)
    {
        ...
        while ($filters->valid() && !$this->impactLimitReached()) {
            ...
            $filters->next();
            if ($filter->execute($value) === \true) {
                $filterMatches[] = $filter;
                $this->getLogger()->info('Match found on Filter ID ' . $filter->getId(), array($filter->toArray()));
                $report = new \AstraPrefixed\Expose\Report($index, $value, $path);
    ...
    protected function impactLimitReached()
    {
        ...
        if ($reached) {
            $this->getLogger()->info('Reached Impact limit');
        }
        return $reached;
    }
    /**
    ...
    {
        if ($this->queue === null) {
            throw new \AstraPrefixed\Expose\Exception\QueueNotDefined('Queue object is not defined');
        }
        return $this->queue;
    }
    /**
    ...
    {
        $this->data = new \AstraPrefixed\Expose\DataCollection($data);
    }
    /**
     ...
     */
    public function setFilters(\AstraPrefixed\Expose\FilterCollection $filters)
    {
    ...
    {
        if ($this->logger === null) {
            throw new \AstraPrefixed\Expose\Exception\LoggerNotDefined('Logger instance not defined');
        }
        return $this->logger;
    }
    /**
    ...
    public function setConfig($config)
    {
        if (\is_array($config)) {
            ...
            if (\is_file($config)) {
                $cfg = \parse_ini_file($config, \true);
                $this->config = new Config($cfg);
            } else {
                throw new \InvalidArgumentException('Could not load configuration file ' . $config);
            }
        }
    }
    /**
    ...
    public function setThreshold($threshold)
    {
        if (\is_numeric($threshold) === \false) {
            throw new \InvalidArgumentException('Invalid threshold "' . $threshold . '", must be numeric');
        }
     ...
     */
    public function setCache(\AstraPrefixed\Expose\Cache $cache)
    {
        $this->cache = $cache;
    }
    /**
