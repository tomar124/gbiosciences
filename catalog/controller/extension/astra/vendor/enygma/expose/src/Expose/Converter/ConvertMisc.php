...
namespace AstraPrefixed\Expose\Converter;
...
class ConvertMisc
{
    ...
    public function convertEntities($value)
    {
        ...
        $value = \preg_replace('/(?:j\\s*a\\s*v\\s*a\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t\\s*:)|(d\\s*a\\s*t\\s*a\\s*:)/ms', 'javascript:', $value);
        return $value;
    }
    /**
     * Normalize quotes
    ...
    public function convertFromControlChars($value)
    {
        ...
        $search = array(\chr(0), \chr(1), \chr(2), \chr(3), \chr(4), \chr(5), \chr(6), \chr(7), \chr(8), \chr(11), \chr(12), \chr(14), \chr(15), \chr(16), \chr(17), \chr(18), \chr(19), \chr(24), \chr(25), \chr(192), \chr(193), \chr(238), \chr(255), '\\0');
        $value = \str_replace($search, '%00', $value);
        //take care for malicious unicode characters
        $value = \urldecode(\preg_replace('/(?:%E(?:2|3)%8(?:0|1)%(?:A|8|9)\\w|%EF%BB%BF|%EF%BF%BD)|(?:&#(?:65|8)\\d{3};?)/i', null, \urlencode($value)));
        ...
        $value = \preg_replace('/(?:&#(?:65|8)\\d{3};?)|' . '(?:&#(?:56|7)3\\d{2};?)|' . '(?:&#x(?:fe|20)\\w{2};?)|' . '(?:&#x(?:d[c-f])\\w{2};?)/i', null, $value);
    ...
    public function convertFromUTF7($value)
    {
        if (\preg_match('/\\+A\\w+-?/m', $value)) {
            if (\function_exists('mb_convert_encoding')) {
                ...
                $schemes = array('+ACI-' => '"', '+ADw-' => '<', '+AD4-' => '>', '+AFs-' => '[', '+AF0-' => ']', '+AHs-' => '{', '+AH0-' => '}', '+AFw-' => '\\', '+ADs-' => ';', '+ACM-' => '#', '+ACY-' => '&', '+ACU-' => '%', '+ACQ-' => '$', '+AD0-' => '=', '+AGA-' => '`', '+ALQ-' => '"', '+IBg-' => '"', '+IBk-' => '"', '+AHw-' => '|', '+ACo-' => '*', '+AF4-' => '^', '+ACIAPg-' => '">', '+ACIAPgA8-' => '">');
                $value = \str_ireplace(\array_keys($schemes), \array_values($schemes), $value);
            }
        }
        return $value;
    }
    ...
    public function convertFromConcatenated($value)
    {
        ...
        $converted = \preg_replace('/(?:\\(*[.\\d]e[+-]*[^a-z\\W]+\\)*)|(?:NaN|Infinity)\\W/ims', 1, $converted);
    ...
    public function convertFromProprietaryEncodings($value)
    {
        ...
        //strip emoticons
        $value = \preg_replace('/(?:\\s[:;]-[)\\/PD]+)|(?:\\s;[)PD]+)|(?:\\s:[)PD]+)|-\\.-|\\^\\^/m', null, $value);
