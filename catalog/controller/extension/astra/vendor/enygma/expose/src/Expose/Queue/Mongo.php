...
namespace AstraPrefixed\Expose\Queue;

class Mongo extends \AstraPrefixed\Expose\Queue
{
    /**
     * Queue database name
     * @var string
    ...
    {
        $data = array('data' => $requestData, 'remote_ip' => isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 0, 'datetime' => \time(), 'processed' => \false);
    ...
    {
        return $this->getCollection()->update(array('_id' => $id), array('$set' => array('processed' => \true)));
    }
    /**
    ...
    {
        $results = $this->getCollection()->find(array('processed' => \false))->limit($limit);
