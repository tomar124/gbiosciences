...
class Verification
{
    ...
    public function verify()
    {
        try {
            ...
            switch ($key->getClass()) {
                case 'secret':
                    $result = \hash_equals($this->expectedSignature()->string(), $this->providedSignature());
                    if (!$result) {
                ...
                case 'asymmetric':
                    ...
                    $result = $algorithm->verify($signedString->string(), $this->parameter('signature'), $key->getVerifyingKey());
                    return $result;
                default:
    ...
    {
        return new Signature($this->message, $this->keyId(), $this->algorithm(), $this->headerList());
    }
    /**
     * @return string
     */
