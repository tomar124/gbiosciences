...
use AstraPrefixed\GuzzleHttp\Promise\RejectedPromise;
...
class MockHandler implements \Countable
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        $response = $response instanceof \Exception ? \AstraPrefixed\GuzzleHttp\Promise\rejection_for($response) : \AstraPrefixed\GuzzleHttp\Promise\promise_for($response);
        return $response->then(function ($value) use($request, $options) {
            ...
            if (isset($options['sink'])) {
                ...
                } elseif ($sink instanceof \AstraPrefixed\Psr\Http\Message\StreamInterface) {
                    $sink->write($contents);
                }
            }
            return $value;
            ...
            return \AstraPrefixed\GuzzleHttp\Promise\rejection_for($reason);
        });
    }
    /**
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || \is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \AstraPrefixed\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
