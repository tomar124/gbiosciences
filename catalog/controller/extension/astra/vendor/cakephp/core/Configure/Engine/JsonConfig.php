 ...
 */
namespace AstraPrefixed\Cake\Core\Configure\Engine;

use AstraPrefixed\Cake\Core\Configure\ConfigEngineInterface;
use AstraPrefixed\Cake\Core\Configure\FileConfigTrait;
...
class JsonConfig implements ConfigEngineInterface
{
    ...
    public function read($key)
    {
        ...
        if (\json_last_error() !== \JSON_ERROR_NONE) {
            throw new Exception(\sprintf('Error parsing JSON string fetched from config file "%s.json": %s', $key, \json_last_error_msg()));
        }
        if (!\is_array($values)) {
            throw new Exception(\sprintf('Decoding JSON config file "%s.json" did not return an array', $key));
        }
        return $values;
    }
    /**
