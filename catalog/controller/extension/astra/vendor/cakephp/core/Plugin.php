 ...
 */
namespace AstraPrefixed\Cake\Core;

use AstraPrefixed\Cake\Core\Exception\MissingPluginException;
...
class Plugin
{
    ...
    {
        deprecationWarning('Plugin::load() is deprecated. ' . 'Use Application::addPlugin() instead. ' . 'This method will be removed in 4.0.0.');
        if (\is_array($plugin)) {
        ...
        $config += ['autoload' => \false, 'bootstrap' => \false, 'routes' => \false, 'console' => \true, 'classBase' => 'src', 'ignoreMissing' => \false, 'name' => $plugin];
        ...
        if ($config['autoload'] === \true) {
            ...
            static::$_loader->addNamespace(\str_replace('/', '\\', $plugin), $config['path'] . $config['classBase'] . \DIRECTORY_SEPARATOR);
            static::$_loader->addNamespace(\str_replace('/', '\\', $plugin) . '\\Test', $config['path'] . 'tests' . \DIRECTORY_SEPARATOR);
        }
        if ($config['bootstrap'] === \true) {
    ...
    {
        deprecationWarning('Plugin::bootstrap() is deprecated. ' . 'This method will be removed in 4.0.0.');
        $plugin = static::getCollection()->get($name);
        ...
        return static::_includeFile($plugin->getConfigPath() . 'bootstrap.php', \true);
    }
    /**
    ...
    {
        deprecationWarning('You no longer need to call `Plugin::routes()` after upgrading to use Http\\Server. ' . 'See https://book.cakephp.org/3/en/development/application.html#adding-the-new-http-stack-to-an-existing-application ' . 'for upgrade information.');
        if ($name === null) {
        ...
        return (bool) static::_includeFile($plugin->getConfigPath() . 'routes.php', \true);
    }
    /**
    ...
    {
        if ($plugin !== null) {
            deprecationWarning('Checking a single plugin with Plugin::loaded() is deprecated. ' . 'Use Plugin::isLoaded() instead.');
            return static::getCollection()->has($plugin);
        }
        $names = [];
