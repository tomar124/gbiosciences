 ...
 */
namespace AstraPrefixed\Cake\Collection;

use AppendIterator;
use ArrayIterator;
use AstraPrefixed\Cake\Collection\Iterator\BufferedIterator;
use AstraPrefixed\Cake\Collection\Iterator\ExtractIterator;
use AstraPrefixed\Cake\Collection\Iterator\FilterIterator;
use AstraPrefixed\Cake\Collection\Iterator\InsertIterator;
use AstraPrefixed\Cake\Collection\Iterator\MapReduce;
use AstraPrefixed\Cake\Collection\Iterator\NestIterator;
use AstraPrefixed\Cake\Collection\Iterator\ReplaceIterator;
use AstraPrefixed\Cake\Collection\Iterator\SortIterator;
use AstraPrefixed\Cake\Collection\Iterator\StoppableIterator;
use AstraPrefixed\Cake\Collection\Iterator\TreeIterator;
use AstraPrefixed\Cake\Collection\Iterator\UnfoldIterator;
use AstraPrefixed\Cake\Collection\Iterator\ZipIterator;
use Countable;
use LimitIterator;
use LogicException;
...
trait CollectionTrait
{
    ...
    public function extract($matcher)
    {
        ...
        if (\is_string($matcher) && \strpos($matcher, '{*}') !== \false) {
            $extractor = $extractor->filter(function ($data) {
    ...
    public function avg($matcher = null)
    {
        ...
        $result = $result->reduce(function ($acc, $current) {
            list($count, $sum) = $acc;
    ...
    {
        $options = ['keyPath' => $this->_propertyExtractor($keyPath), 'valuePath' => $this->_propertyExtractor($valuePath), 'groupPath' => $groupPath ? $this->_propertyExtractor($groupPath) : null];
        $mapper = function ($value, $key, $mapReduce) use($options) {
            ...
            $mapReduce->emitIntermediate([$rowKey($value, $key) => $rowVal($value, $key)], $key);
        };
    ...
    public function nest($idPath, $parentPath, $nestingKey = 'children')
    {
        ...
        return $this->newCollection(new MapReduce($this->unwrap(), $mapper, $reducer))->map(function ($value) use(&$isObject) {
     ...
     * {@inheritDoc}
     */
    public function toArray($preserveKeys = \true)
    {
     ...
     * {@inheritDoc}
     */
    public function compile($preserveKeys = \true)
    {
    ...
    {
        $dir = \strtolower($dir);
        $modes = ['desc' => TreeIterator::SELF_FIRST, 'asc' => TreeIterator::CHILD_FIRST, 'leaves' => TreeIterator::LEAVES_ONLY];
        return new TreeIterator(new NestIterator($this, $nestingKey), isset($modes[$dir]) ? $modes[$dir] : $dir);
    }
    /**
     * {@inheritDoc}
     *
    ...
    public function unfold(callable $transformer = null)
    {
        ...
        return $this->newCollection(new RecursiveIteratorIterator(new UnfoldIterator($this->unwrap(), $transformer), RecursiveIteratorIterator::LEAVES_ONLY));
    }
    /**
     * {@inheritDoc}
     */
     ...
     * {@inheritDoc}
     */
    public function chunkWithKeys($chunkSize, $preserveKeys = \true)
    {
