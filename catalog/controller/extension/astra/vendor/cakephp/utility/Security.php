...
use AstraPrefixed\Cake\Utility\Crypto\Mcrypt;
use AstraPrefixed\Cake\Utility\Crypto\OpenSsl;
...
class Security
{
    ...
    public static function randomBytes($length)
    {
        ...
        if (!\function_exists('openssl_random_pseudo_bytes')) {
            throw new RuntimeException('You do not have a safe source of random data available. ' . 'Install either the openssl extension, or paragonie/random_compat. ' . 'Or use Security::insecureRandomBytes() alternatively.');
        }
        $bytes = \openssl_random_pseudo_bytes($length, $strongSource);
        if (!$strongSource) {
            \trigger_error('openssl was unable to use a strong source of entropy. ' . 'Consider updating your system libraries, or ensuring ' . 'you have more available entropy.', \E_USER_WARNING);
        }
        return $bytes;
    }
    /**
    ...
    public static function insecureRandomBytes($length)
    {
        ...
        while ($byteLength < $length) {
            $bytes .= static::hash(Text::uuid() . \uniqid(\mt_rand(), \true), 'sha512', \true);
    ...
    {
        deprecationWarning('Security::salt() is deprecated. ' . 'Use Security::getSalt()/setSalt() instead.');
        if ($salt === null) {
            return static::$_salt;
        }
