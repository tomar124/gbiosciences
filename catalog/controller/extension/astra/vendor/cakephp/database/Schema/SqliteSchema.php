...
class SqliteSchema extends BaseSchema
{
    ...
    public function columnSql(TableSchema $schema, $name)
    {
        ...
        if (\in_array($data['type'], $integerTypes, \true) && isset($data['length']) && [$name] !== (array) $schema->primaryKey()) {
            $out .= '(' . (int) $data['length'] . ')';
        }
