...
use AstraPrefixed\Cake\Database\Exception;
use AstraPrefixed\Cake\Database\Schema\TableSchema;
/**
...
class MysqlSchema extends BaseSchema
{
    ...
    protected function _convertColumn($column)
    {
        ...
        if ($col === 'char') {
            return ['type' => TableSchema::TYPE_STRING, 'length' => $length, 'fixed' => \true];
        }
    ...
    public function columnSql(TableSchema $schema, $name)
    {
        ...
        $specialMap = ['string' => \true, 'text' => \true, 'binary' => \true];
        ...
        $hasLength = [TableSchema::TYPE_INTEGER, TableSchema::TYPE_SMALLINTEGER, TableSchema::TYPE_TINYINTEGER, TableSchema::TYPE_STRING];
        if (\in_array($data['type'], $hasLength, \true) && isset($data['length'])) {
            $out .= '(' . (int) $data['length'] . ')';
        }
        $hasPrecision = [TableSchema::TYPE_FLOAT, TableSchema::TYPE_DECIMAL];
        if (\in_array($data['type'], $hasPrecision, \true) && isset($data['length'])) {
        ...
        $addAutoIncrement = [$name] == (array) $schema->primaryKey() && !$schema->hasAutoincrement() && !isset($data['autoIncrement']);
        if (\in_array($data['type'], [TableSchema::TYPE_INTEGER, TableSchema::TYPE_BIGINTEGER]) && ($data['autoIncrement'] === \true || $addAutoIncrement)) {
            $out .= ' AUTO_INCREMENT';
        }
        if (isset($data['null']) && $data['null'] === \true && $data['type'] === TableSchema::TYPE_TIMESTAMP) {
            $out .= ' NULL';
            unset($data['default']);
        }
        if (isset($data['default']) && \in_array($data['type'], [TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_DATETIME]) && \in_array(\strtolower($data['default']), ['current_timestamp', 'current_timestamp()'])) {
