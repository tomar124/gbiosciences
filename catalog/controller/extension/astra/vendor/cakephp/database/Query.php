...
use AstraPrefixed\Cake\Database\Expression\IdentifierExpression;
use AstraPrefixed\Cake\Database\Expression\OrderByExpression;
use AstraPrefixed\Cake\Database\Expression\OrderClauseExpression;
use AstraPrefixed\Cake\Database\Expression\QueryExpression;
use AstraPrefixed\Cake\Database\Expression\ValuesExpression;
use AstraPrefixed\Cake\Database\Statement\CallbackStatement;
use AstraPrefixed\Cake\Datasource\QueryInterface;
...
class Query implements ExpressionInterface, IteratorAggregate
{
     ...
     * @var array
     */
    protected $_parts = ['delete' => \true, 'update' => [], 'set' => [], 'insert' => [], 'values' => [], 'select' => [], 'distinct' => \false, 'modifier' => [], 'from' => [], 'join' => [], 'where' => null, 'group' => [], 'having' => null, 'order' => null, 'limit' => null, 'offset' => null, 'union' => [], 'epilog' => null];
    /**
    ...
    {
        deprecationWarning('Query::connection() is deprecated. ' . 'Use Query::setConnection()/getConnection() instead.');
        if ($connection !== null) {
    ...
    {
        deprecationWarning('Query::orWhere() is deprecated as it creates hard to predict SQL based on the ' . 'current query state. Use `Query::where()` instead.');
     ...
     * @return $this
     */
    public function enableBufferedResults($enable = \true)
    {
        $this->_dirty();
    ...
    {
        deprecationWarning('Query::bufferResults() is deprecated. ' . 'Use Query::enableBufferedResults()/isBufferedResultsEnabled() instead.');
        if ($enable !== null) {
    ...
    {
        deprecationWarning('Query::selectTypeMap() is deprecated. ' . 'Use Query::setSelectTypeMap()/getSelectTypeMap() instead.');
        if ($typeMap !== null) {
