...
use AstraPrefixed\Cake\Core\App;
use AstraPrefixed\Cake\Core\Retry\CommandRetry;
use AstraPrefixed\Cake\Database\Exception\MissingConnectionException;
use AstraPrefixed\Cake\Database\Exception\MissingDriverException;
use AstraPrefixed\Cake\Database\Exception\MissingExtensionException;
use AstraPrefixed\Cake\Database\Exception\NestedTransactionRollbackException;
use AstraPrefixed\Cake\Database\Log\LoggedQuery;
use AstraPrefixed\Cake\Database\Log\LoggingStatement;
use AstraPrefixed\Cake\Database\Log\QueryLogger;
use AstraPrefixed\Cake\Database\Retry\ReconnectStrategy;
use AstraPrefixed\Cake\Database\Schema\CachedCollection;
use AstraPrefixed\Cake\Database\Schema\Collection as SchemaCollection;
use AstraPrefixed\Cake\Datasource\ConnectionInterface;
use AstraPrefixed\Cake\Log\Log;
use Exception;
use Throwable;
/**
...
class Connection implements ConnectionInterface
{
    ...
    {
        if ($this->_transactionStarted && \class_exists('AstraPrefixed\\Cake\\Log\\Log')) {
    ...
    {
        deprecationWarning('Connection::schemaCollection() is deprecated. ' . 'Use Connection::setSchemaCollection()/getSchemaCollection() instead.');
        if ($collection !== null) {
    ...
    public function update($table, array $data, array $conditions = [], $types = [])
    {
        return $this->getDisconnectRetry()->run(function () use($table, $data, $conditions, $types) {
            return $this->newQuery()->update($table)->set($data, $types)->where($conditions, $types)->execute();
        });
    }
    /**
    ...
    {
        deprecationWarning('Connection::useSavePoints() is deprecated. ' . 'Use Connection::enableSavePoints()/isSavePointsEnabled() instead.');
        if ($enable !== null) {
    ...
    {
        deprecationWarning('Connection::logQueries() is deprecated. ' . 'Use enableQueryLogging() and isQueryLoggingEnabled() instead.');
        if ($enable === null) {
