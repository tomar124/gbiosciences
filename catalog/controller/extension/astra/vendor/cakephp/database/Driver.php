...
use AstraPrefixed\Cake\Database\Statement\PDOStatement;
...
abstract class Driver implements DriverInterface
{
    ...
    {
        $connection = new PDO($dsn, $config['username'], $config['password'], $config['flags']);
    ...
    {
        deprecationWarning(\get_called_class() . '::connection() is deprecated. ' . 'Use setConnection()/getConnection() instead.');
        if ($connection !== null) {
     ...
     * {@inheritDoc}
     */
    public abstract function supportsDynamicConstraints();
    /**
     * {@inheritDoc}
     */
    ...
    public function schemaValue($value)
    {
        ...
        if (\is_int($value) || $value === '0' || \is_numeric($value) && \strpos($value, ',') === \false && $value[0] !== '0' && \strpos($value, 'e') === \false) {
            return (string) $value;
        }
     ...
     * {@inheritDoc}
     */
    public function enableAutoQuoting($enable = \true)
    {
    ...
    {
        deprecationWarning('Driver::autoQuoting() is deprecated. ' . 'Use Driver::enableAutoQuoting()/isAutoQuotingEnabled() instead.');
        if ($enable !== null) {
