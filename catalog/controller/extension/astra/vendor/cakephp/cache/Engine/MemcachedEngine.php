...
class MemcachedEngine extends CacheEngine
{
     ...
     * @var array
     */
    protected $_defaultConfig = ['compress' => \false, 'duration' => 3600, 'groups' => [], 'host' => null, 'username' => null, 'password' => null, 'persistent' => \false, 'port' => null, 'prefix' => 'cake_', 'probability' => 100, 'serialize' => 'php', 'servers' => ['127.0.0.1'], 'options' => []];
    /**
    ...
    public function init(array $config = [])
    {
        ...
        if (\count($serverList)) {
            if ($this->_config['persistent'] !== \false) {
                ...
                if ($actualServers !== $configuredServers) {
                    $message = "Invalid cache configuration. Multiple persistent cache configurations are detected" . " with different 'servers' values. 'servers' values for persistent cache configurations" . " must be the same when using the same persistence id.";
        ...
        if ($this->_config['username'] !== null && $this->_config['password'] !== null) {
            ...
            $this->_Memcached->setOption(Memcached::OPT_BINARY_PROTOCOL, \true);
    ...
    {
        $this->_Memcached->setOption(Memcached::OPT_LIBKETAMA_COMPATIBLE, \true);
