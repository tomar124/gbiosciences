 ...
 */
namespace AstraPrefixed\Cake\Cache\Engine;

use AstraPrefixed\Cake\Cache\CacheEngine;
...
class FileEngine extends CacheEngine
{
     ...
     * @var array
     */
    protected $_defaultConfig = ['duration' => 3600, 'groups' => [], 'isWindows' => \false, 'lock' => \true, 'mask' => 0664, 'path' => null, 'prefix' => 'cake_', 'probability' => 100, 'serialize' => \true];
    /**
    ...
    protected function _setKey($key, $createKey = \false)
    {
        ...
        if (empty($this->_File) || $this->_File->getBasename() !== $key || $this->_File->valid() === \false) {
            ...
            if (!$exists && !\chmod($this->_File->getPathname(), (int) $this->_config['mask'])) {
                \trigger_error(\sprintf('Could not apply permission mask "%s" on cache file "%s"', $this->_File->getPathname(), $this->_config['mask']), \E_USER_WARNING);
            }
        }
        return \true;
    }
    ...
    protected function _active()
    {
        ...
        if (!\is_dir($path)) {
            //@codingStandardsIgnoreStart
            $success = @\mkdir($path, 0775, \true);
            //@codingStandardsIgnoreEnd
        }
        $isWritableDir = $dir->isDir() && $dir->isWritable();
        if (!$success || $this->_init && !$isWritableDir) {
            $this->_init = \false;
            \trigger_error(\sprintf('%s is not writable', $this->_config['path']), \E_USER_WARNING);
        }
        return $success;
    }
    /**
    ...
    public function key($key)
    {
        ...
        $key = Inflector::underscore(\str_replace([\DIRECTORY_SEPARATOR, '/', '.', '<', '>', '?', ':', '|', '*', '"'], '_', (string) $key));
        return $key;
    }
    /**
    ...
    public function clearGroup($group)
    {
        ...
        $filtered = new CallbackFilterIterator($contents, function (SplFileInfo $current) use($group, $prefix) {
            ...
            $hasPrefix = $prefix === '' || \strpos($current->getBasename(), $prefix) === 0;
            if ($hasPrefix === \false) {
                return \false;
            }
            $pos = \strpos($current->getPathname(), \DIRECTORY_SEPARATOR . $group . \DIRECTORY_SEPARATOR);
            return $pos !== \false;
        });
