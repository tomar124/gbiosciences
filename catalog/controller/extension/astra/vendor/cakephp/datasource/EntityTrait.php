 ...
 */
namespace AstraPrefixed\Cake\Datasource;

use AstraPrefixed\Cake\Collection\Collection;
use AstraPrefixed\Cake\Utility\Hash;
use AstraPrefixed\Cake\Utility\Inflector;
...
trait EntityTrait
{
    ...
    public function set($property, $value = null, array $options = [])
    {
        ...
        foreach ($property as $p => $value) {
            if ($options['guard'] === \true && !$this->isAccessible($p)) {
                continue;
            }
            $this->setDirty($p, \true);
            if (!\array_key_exists($p, $this->_original) && \array_key_exists($p, $this->_properties) && $this->_properties[$p] !== $value) {
    ...
    {
        deprecationWarning(\get_called_class() . '::hiddenProperties() is deprecated. ' . 'Use setHidden()/getHidden() instead.');
        if ($properties === null) {
            return $this->_hidden;
        }
    ...
    {
        deprecationWarning(\get_called_class() . '::virtualProperties() is deprecated. ' . 'Use setVirtual()/getVirtual() instead.');
        if ($properties === null) {
    ...
    {
        deprecationWarning(\get_called_class() . '::visibleProperties() is deprecated. ' . 'Use getVisible() instead.');
    ...
    {
        deprecationWarning(\get_called_class() . '::dirty() is deprecated. ' . 'Use setDirty()/isDirty() instead.');
        if ($property === null) {
            return $this->isDirty();
        }
        if ($isDirty === null) {
     ...
     * @return $this
     */
    public function setDirty($property, $isDirty = \true)
    {
        if ($isDirty === \false) {
     ...
     * @return bool
     */
    public function hasErrors($includeNested = \true)
    {
    ...
    public function getErrors()
    {
        ...
        return $this->_errors + (new Collection($diff))->filter(function ($value) {
    ...
    {
        deprecationWarning(\get_called_class() . '::errors() is deprecated. ' . 'Use setError()/getError() or setErrors()/getErrors() instead.');
        if ($field === null) {
    ...
    {
        deprecationWarning(\get_called_class() . '::invalid() is deprecated. ' . 'Use setInvalid()/getInvalid()/getInvalidField() instead.');
        if ($field === null) {
            return $this->_invalid;
        }
    ...
    {
        deprecationWarning(\get_called_class() . '::accessible() is deprecated. ' . 'Use setAccess()/isAccessible() instead.');
        if ($set === null) {
    ...
    {
        $value = isset($this->_accessible[$property]) ? $this->_accessible[$property] : null;
    ...
    {
        deprecationWarning(\get_called_class() . '::source() is deprecated. ' . 'Use setSource()/getSource() instead.');
        if ($alias === null) {
    ...
    public function __debugInfo()
    {
        ...
        return $properties + ['[new]' => $this->isNew(), '[accessible]' => $this->_accessible, '[dirty]' => $this->_dirty, '[original]' => $this->_original, '[virtual]' => $this->_virtual, '[hasErrors]' => $this->hasErrors(), '[errors]' => $this->_errors, '[invalid]' => $this->_invalid, '[repository]' => $this->_registryAlias];
    }
}
