...
use AstraPrefixed\Cake\Datasource\Exception\RecordNotFoundException;
/**
...
trait QueryTrait
{
    ...
    {
        if ($table === null) {
            deprecationWarning('Using Query::repository() as getter is deprecated. ' . 'Use getRepository() instead.');
    ...
    {
        if ($value === null) {
            deprecationWarning('Using ' . \get_called_class() . '::eagerLoaded() as a getter is deprecated. ' . 'Use isEagerLoaded() instead.');
            return $this->_eagerLoaded;
        }
    ...
    public function mapReduce(callable $mapper = null, callable $reducer = null, $overwrite = \false)
    {
        ...
        if ($mapper === null) {
            if (!$overwrite) {
                deprecationWarning('Using QueryTrait::mapReduce() as a getter is deprecated. ' . 'Use getMapReducers() instead.');
            }
    ...
    public function formatResults(callable $formatter = null, $mode = 0)
    {
        ...
        if ($formatter === null) {
            if ($mode !== self::OVERWRITE) {
                deprecationWarning('Using QueryTrait::formatResults() as a getter is deprecated. ' . 'Use getResultFormatters() instead.');
            }
     ...
     * @return $this
     */
    public abstract function applyOptions(array $options);
    /**
