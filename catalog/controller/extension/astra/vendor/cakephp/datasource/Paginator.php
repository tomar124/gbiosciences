...
use AstraPrefixed\Cake\Core\InstanceConfigTrait;
use AstraPrefixed\Cake\Datasource\Exception\PageOutOfBoundsException;
/**
...
class Paginator implements PaginatorInterface
{
     ...
     * @var array
     */
    protected $_defaultConfig = ['page' => 1, 'limit' => 20, 'maxLimit' => 100, 'whitelist' => ['limit', 'sort', 'page', 'direction']];
    /**
    ...
    protected function addSortingParams(array $params, array $data)
    {
        ...
        $params += ['sort' => $data['options']['sort'], 'direction' => isset($data['options']['sort']) ? \current($order) : null, 'sortDefault' => $sortDefault, 'directionDefault' => $directionDefault, 'completeSort' => $order];
        return $params;
    }
    /**
    ...
    public function validateSort(RepositoryInterface $object, array $options)
    {
        ...
        if (isset($options['sortWhitelist'])) {
            ...
            $inWhitelist = \in_array($field, $options['sortWhitelist'], \true);
            if (!$inWhitelist) {
                $options['order'] = [];
                $options['sort'] = null;
                return $options;
