...
namespace AstraPrefixed\Illuminate\Support\Testing\Fakes;

use AstraPrefixed\Illuminate\Support\Str;
use AstraPrefixed\Illuminate\Support\Collection;
use AstraPrefixed\Illuminate\Support\Traits\Macroable;
use AstraPrefixed\PHPUnit\Framework\Assert as PHPUnit;
use AstraPrefixed\Illuminate\Contracts\Translation\HasLocalePreference;
use AstraPrefixed\Illuminate\Contracts\Notifications\Factory as NotificationFactory;
use AstraPrefixed\Illuminate\Contracts\Notifications\Dispatcher as NotificationDispatcher;
class NotificationFake implements NotificationFactory, NotificationDispatcher
{
    ...
    {
        PHPUnit::assertTrue(($count = $this->sent($notifiable, $notification)->count()) === $times, "Expected [{$notification}] to be sent {$times} times, but was sent {$count} times.");
    }
    /**
    ...
    public function sendNow($notifiables, $notification, array $channels = null)
    {
        ...
        foreach ($notifiables as $notifiable) {
            ...
            $this->notifications[\get_class($notifiable)][$notifiable->getKey()][\get_class($notification)][] = ['notification' => $notification, 'channels' => $channels ?: $notification->via($notifiable), 'notifiable' => $notifiable, 'locale' => $notification->locale ?? $this->locale ?? value(function () use($notifiable) {
