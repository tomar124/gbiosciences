...
namespace AstraPrefixed\Illuminate\Database\Query\Grammars;

use AstraPrefixed\Illuminate\Support\Arr;
use AstraPrefixed\Illuminate\Support\Str;
use AstraPrefixed\Illuminate\Database\Query\Builder;
class PostgresGrammar extends Grammar
{
    ...
    public function prepareBindingsForUpdate(array $bindings, array $values)
    {
        $values = collect($values)->map(function ($value, $column) {
            return $this->isJsonSelector($column) && !$this->isExpression($value) ? \json_encode($value) : $value;
        })->all();
    ...
    {
        $bindingsWithoutWhere = Arr::except($bindings, ['select', 'where']);
        return \array_values(\array_merge($bindings['where'], Arr::flatten($bindingsWithoutWhere)));
    }
    /**
