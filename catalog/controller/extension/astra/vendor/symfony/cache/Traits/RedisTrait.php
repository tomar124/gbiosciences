...
use AstraPrefixed\Predis\Connection\Aggregate\ClusterInterface;
use AstraPrefixed\Predis\Connection\Aggregate\RedisCluster;
use AstraPrefixed\Predis\Connection\Factory;
use AstraPrefixed\Predis\Response\Status;
use AstraPrefixed\Symfony\Component\Cache\Exception\CacheException;
...
trait RedisTrait
{
    ...
    private function init($redisClient, $namespace = '', $defaultLifetime = 0)
    {
        ...
        if (!$redisClient instanceof \Redis && !$redisClient instanceof \RedisArray && !$redisClient instanceof \RedisCluster && !$redisClient instanceof \AstraPrefixed\Predis\Client && !$redisClient instanceof RedisProxy) {
    ...
    public static function createConnection($dsn, array $options = [])
    {
        ...
        if (null === $params['class'] && !\extension_loaded('redis') && !\class_exists(\AstraPrefixed\Predis\Client::class)) {
            throw new CacheException(\sprintf('Cannot find the "redis" extension, and "predis/predis" is not installed: "%s".', $dsn));
        }
        $class = null === $params['class'] ? \extension_loaded('redis') ? \Redis::class : \AstraPrefixed\Predis\Client::class : $params['class'];
        ...
        } elseif (\is_a($class, \AstraPrefixed\Predis\Client::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \AstraPrefixed\Predis\Client && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    protected function doClear($namespace)
    {
        ...
        if ($this->redis instanceof \AstraPrefixed\Predis\Client) {
            $evalArgs = [0, $namespace];
            $connection = $this->redis->getConnection();
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                $hosts = [];
                foreach ($connection as $c) {
                    $hosts[] = new \AstraPrefixed\Predis\Client($c);
                }
            }
        ...
        foreach ($hosts as $host) {
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \AstraPrefixed\Predis\Client ? $host->scan($cursor, 'MATCH', $namespace . '*', 'COUNT', 1000) : $host->scan($cursor, $namespace . '*', 1000);
    ...
    {
        $ids = [];
        if ($this->redis instanceof \RedisCluster || $this->redis instanceof \AstraPrefixed\Predis\Client && $this->redis->getConnection() instanceof RedisCluster) {
        ...
        } elseif ($this->redis instanceof \AstraPrefixed\Predis\Client) {
