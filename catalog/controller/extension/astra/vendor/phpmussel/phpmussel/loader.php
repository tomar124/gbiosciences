...
if (\defined('phpMussel')) {
    ...
    echo '[phpMussel] ' . (isset($phpMussel['L10N']) ? $phpMussel['L10N']->getString('instance_already_active') : 'Instance already active! Please double-check your hooks.');
    die;
}
\define('phpMussel', \true);
...
$phpMussel['Direct'] = function () {
    return !isset($_SERVER['SCRIPT_FILENAME']) || \str_replace("\\", '/', \strtolower(\realpath($_SERVER['SCRIPT_FILENAME']))) === \str_replace("\\", '/', \strtolower(__FILE__));
};
/** Checks whether we're calling phpMussel through an alternative pathway (e.g., Cronable). */
$phpMussel['Alternate'] = \class_exists('AstraPrefixed\\Maikuolan\\Cronable\\Cronable');
...
if ($phpMussel['Config']['general']['enable_plugins']) {
    ...
    if (($phpMussel['MusselPlugins']['tempdata']['d'] = \opendir($phpMussel['Vault'] . 'plugins')) && \is_resource($phpMussel['MusselPlugins']['tempdata']['d'])) {
        while (\true) {
            $phpMussel['MusselPlugins']['tempdata']['f'] = \readdir($phpMussel['MusselPlugins']['tempdata']['d']);
            if (empty($phpMussel['MusselPlugins']['tempdata']['f'])) {
                break;
            }
            if ($phpMussel['MusselPlugins']['tempdata']['f'] !== '.' && $phpMussel['MusselPlugins']['tempdata']['f'] !== '..' && \is_dir($phpMussel['pluginPath'] . $phpMussel['MusselPlugins']['tempdata']['f']) && \file_exists($phpMussel['pluginPath'] . $phpMussel['MusselPlugins']['tempdata']['f'] . '/plugin.php') && !\is_link($phpMussel['pluginPath'] . $phpMussel['MusselPlugins']['tempdata']['f'] . '/plugin.php')) {
...
if (!$phpMussel['Mussel_sapi'] || $phpMussel['Alternate']) {
     ...
     */
    if (!$phpMussel['Config']['general']['disable_frontend'] && \file_exists($phpMussel['Vault'] . 'frontend.php') && ($phpMussel['Direct']() || $phpMussel['Alternate'])) {
