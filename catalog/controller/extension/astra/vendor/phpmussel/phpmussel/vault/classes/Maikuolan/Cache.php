...
class Cache
{
    ...
    public function setEntry($Key, $Value, $TTL = 3600)
    {
        ...
        if ($this->Using === 'PDO') {
            ...
            if ($PDO !== \false && $PDO->execute([':key' => $Key, ':data' => $Value, ':time' => $TTL])) {
                return $PDO->rowCount() > 0 && ($this->Modified = \true);
            }
            return \false;
        }
        if (\is_array($this->WorkingData)) {
    ...
    public function clearCache()
    {
        ...
        if ($this->Using === 'Memcached') {
            return $this->WorkingData->flush() && ($this->Modified = \true);
        }
        if ($this->Using === 'Redis') {
            return $this->WorkingData->flushDb() && ($this->Modified = \true);
        }
        if ($this->Using === 'PDO') {
    ...
    public function getAllEntries()
    {
        if ($this->Using === 'APCu') {
            ...
            foreach ($Data['cache_list'] as $Entry) {
                ...
                $Output[$Entry['info']] = $Entry['ttl'] > 0 ? ['Data' => $Entry['Data'], 'Time' => $Creation + $Entry['ttl']] : $Entry['Data'];
            }
            return $Output;
        }
        if ($this->Using === 'Memcached') {
            ...
            foreach ($Data as $Entry) {
                if (!\is_array($Entry) || !isset($Entry['key'], $Entry['value'], $Entry['cas']) || \strlen($Entry['key']) > self::KEY_SIZE_LIMIT) {
                    continue;
                }
        ...
        if ($this->Using === 'PDO') {
            ...
            if ($PDO !== \false && $PDO->execute()) {
                ...
                foreach ($Data as $Entry) {
                    ...
                    $Output[$Entry['Key']] = $Entry['Time'] > 0 ? ['Data' => $this->unserializeEntry($Entry['Data']), 'Time' => $Entry['Time']] : $this->unserializeEntry($Entry['Data']);
                }
                return $Output;
            }
            return [];
    ...
    public function clearExpired(array &$Data)
    {
        ...
        foreach ($Data as $Key => $Value) {
            ...
            if (!\is_array($Value) || !isset($Value['Time']) || $Value['Time'] > \time()) {
                $Updated[$Key] = $Value;
            } else {
                $Cleared = \true;
            }
