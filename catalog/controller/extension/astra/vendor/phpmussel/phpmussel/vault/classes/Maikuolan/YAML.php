...
class YAML
{
    ...
    private function normaliseValue(&$Value, $ValueLen, $ValueLow)
    {
        ...
        $AnchorMatches = [];
        if (\preg_match('~^&([\\dA-Za-z]+) +(.*)$~', $Value, $AnchorMatches) && isset($AnchorMatches[1], $AnchorMatches[2])) {
            $Value = $AnchorMatches[2];
            $this->Anchors[$AnchorMatches[1]] = $Value;
            $ValueLen = \strlen($Value);
            $ValueLow = \strtolower($Value);
        } elseif (\preg_match('~^\\*([\\dA-Za-z]+)$~', $Value, $AnchorMatches) && isset($AnchorMatches[1], $this->Anchors[$AnchorMatches[1]])) {
            $Value = $this->Anchors[$AnchorMatches[1]];
            $ValueLen = \strlen($Value);
            $ValueLow = \strtolower($Value);
        }
        /** Check for string quotes. */
        foreach ([['"', '"', 1], ["'", "'", 1], ['`', '`', 1], ["‘", "’", 1], ["“", "”", 1], ["â€˜", "â€™", 3], ["â€œ", "â€", 3]] as $Wrapper) {
    ...
    public function process($In, array &$Arr, $Depth = 0)
    {
        ...
        $TabLen = $SoL = 0;
        while ($SoL !== \false) {
            $ThisLine = ($EoL = \strpos($In, "\n", $SoL)) === \false ? \substr($In, $SoL) : \substr($In, $SoL, $EoL - $SoL);
