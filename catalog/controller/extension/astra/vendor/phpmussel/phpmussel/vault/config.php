...
$phpMussel['HTTP_HOST'] = empty($_SERVER['HTTP_HOST']) ? '' : (\strpos($_SERVER['HTTP_HOST'], ':') === \false ? $_SERVER['HTTP_HOST'] : \substr($_SERVER['HTTP_HOST'], 0, \strpos($_SERVER['HTTP_HOST'], ':')));
...
/** phpMussel favicon. */
$phpMussel['favicon'] = 'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAABmJLR0QA/wD/AP+gvaeTAAA' . 'ACXBIWXMAAA3XAAAN1wFCKJt4AAAAB3RJTUUH4AoZABIBssLx0wAAAs9JREFUOMt9kl1IU2' . 'EYx//vOTtnLJinydTY2BKZadlNEnTKPjCo7EOlujPow7JCkT6oiz5uK4KI9C7NbSZWZJCld' . 'aNZ3kj0MSFormaFO27WINbGcaszz3m7GFublc/d+z6//5/nz/MQ/KeqqzcfSibnjieTSkEi' . 'kZAikUh/Q8O+jqtXr8xmc+Rf4jVrxFZCSDshqTYhBIqiIBgMomJlxcrhoaH3aZaZLxZFcfm' . '9e3fbS0pKAAAsy8JoNILjODgcDnzw+V5n838ZCMJiS3FxMcxmMzRNQ1PTEQwODsBkMkFRfq' . 'GsrNxQU7P9WE6EU6fPnGAYZjYcDnt7brvH1q5d56eUOggh4HkeRUVFkCQJgiBgx46dcLm62' . 'sbHx08CgM5qsRorKytv2Gw2gBDU19fD7/drL54/RywWhaIokCQJAGCxWGC3LwXDsL5MBLGq' . 'SsjLy4PJZII5Px+lpaWora1lVFXNiaaqKjZu3IQ7d3pRWFj47OChxpSBntdzLMumIE0DpRS' . 'yLEPTtBwDc0EBWIbF1NSXV4SQp26XM2Ugy7FfqqqB5/kMLMsyYrFo5k0pRc227eh70If16z' . 'fsmlNVLhPh8aNH4VgsCo7TgVIKAAgEAjMsk7ugZFLB9HTg5N49e452dnYks9c4FwwGZ5DSg' . 'ud5hEKha7xefz8N6XQ6TE764fF42gyLDC2fP31yX7585c8dyLJ8SdM0MAyD0dFRXLxw7rog' . 'CEvSE/F6HpIk/QSAiQnf6mg05jp//lzqDhobD0OlEERR/AFKNY/H4/B//LjbarU+/vp1pio' . 'en3WzLAe73SbH4/GH/f0P92dH0zmdXQAQ5XS65bc6b/oAYMuWreXLljke9Pb2dNfV7X4biX' . 'xvlmX5zbdweAWllCWEqFigiNfr9Q0MPDkAAGNjLzMNl8s12dzSumohMbq7e86OjIw4HY5Sw' . '/ze0PDwu1AolACwKP33G4ncJFWmwHSnAAAAAElFTkSuQmCC';
...
if (isset($GLOBALS['phpMussel_Config'])) {
    ...
    $phpMussel['Config'] = \parse_ini_file($phpMussel['Vault'] . 'config.ini', \true);
}
...
if (!empty($_SERVER['HTTP_HOST']) && ($phpMussel['Domain'] = \preg_replace('/^www\\./', '', \strtolower($_SERVER['HTTP_HOST']))) && !\preg_match('/[^.\\da-z-]/', $phpMussel['Domain']) && \is_readable($phpMussel['Vault'] . $phpMussel['Domain'] . '.config.ini')) {
    /** Attempts to parse the overrides file found (this is configuration specific to the requested domain). */
    if ($phpMussel['Overrides'] = \parse_ini_file($phpMussel['Vault'] . $phpMussel['Domain'] . '.config.ini', \true)) {
...
$phpMussel['IPAddr'] = $phpMussel['Config']['general']['ipaddr'] !== 'REMOTE_ADDR' && empty($_SERVER[$phpMussel['Config']['general']['ipaddr']]) ? 'REMOTE_ADDR' : $phpMussel['Config']['general']['ipaddr'];
...
$phpMussel['Time'] = \time() + $phpMussel['Config']['general']['timeOffset'] * 60;
/** Set timezone. */
...
$phpMussel['Mussel_sapi'] = empty($_SERVER['REQUEST_METHOD']) || \substr(\php_sapi_name(), 0, 3) === 'cli' || empty($_SERVER[$phpMussel['IPAddr']]) && empty($_SERVER['HTTP_USER_AGENT']) && !empty($_SERVER['argc']) && \is_numeric($_SERVER['argc']) && $_SERVER['argc'] > 0;
...
$phpMussel['DefaultAlgo'] = !empty($phpMussel['Config']['general']['default_algo']) && \defined($phpMussel['Config']['general']['default_algo']) ? \constant($phpMussel['Config']['general']['default_algo']) : \PASSWORD_DEFAULT;
/** Used just for the current, specific request instance. */
$phpMussel['InstanceCache'] = [];
/** Instantiate the request class. */
$phpMussel['Request'] = new \AstraPrefixed\Maikuolan\Common\Request();
$phpMussel['Request']->Channels = ($Channels = $phpMussel['ReadFile']($phpMussel['Vault'] . 'channels.yaml')) ? (new \AstraPrefixed\Maikuolan\Common\YAML($Channels))->Data : [];
...
$phpMussel['Request']->SendToOut = \defined('DEV_DEBUG_MODE') && \DEV_DEBUG_MODE === \true;
