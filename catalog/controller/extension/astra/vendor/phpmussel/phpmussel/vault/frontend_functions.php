...
$phpMussel['Congruency'] = function ($Downstream, $Upstream) use(&$phpMussel) {
    ...
    $DownstreamArray = (new \AstraPrefixed\Maikuolan\Common\YAML($Downstream))->Data;
    $UpstreamArray = (new \AstraPrefixed\Maikuolan\Common\YAML($Upstream))->Data;
...
$phpMussel['SanityCheck'] = function ($FileName, $FileData) {
    ...
    if (\preg_match('~\\.ya?ml$~i', $FileName)) {
        $ThisYAML = new \AstraPrefixed\Maikuolan\Common\YAML();
    ...
    if (\strtolower(\substr($FileName, -4)) === '.png') {
        return \substr($FileData, 0, 4) === "�PNG";
    }
    ...
    foreach ($List as $Item => $List) {
        ...
        if (\is_dir($Item)) {
            ...
            if (($ExtDel = \strrpos($Item, '.')) !== \false) {
                ...
                if (\preg_match('/^(?:.?[BGL]Z.?|7Z|A(CE|LZ|P[KP]|R[CJ]?)?|B([AH]|Z2?)|CAB|DMG|' . 'I(CE|SO)|L(HA|Z[HOWX]?)|P(AK|AQ.?|CK|EA)|RZ|S(7Z|EA|EN|FX|IT.?|QX)|' . 'X(P3|Z)|YZ1|Z(IP.?|Z)?|(J|M|PH|R|SH|T|X)AR)$/', $Ext)) {
                    $Arr[$Key]['CanEdit'] = \false;
                ...
                } elseif (\preg_match('/^(?:H?264|3GP(P2)?|A(M[CV]|VI)|BIK|D(IVX|V5?)|F([4L][CV]|MV)|GIFV|HLV|' . 'M(4V|OV|P4|PE?G[4V]?|KV|VR)|OGM|V(IDEO|OB)|W(EBM|M[FV]3?)|X(WMV|VID))$/', $Ext)) {
                    $Arr[$Key]['CanEdit'] = \false;
...
$phpMussel['FileManager-PathSecurityCheck'] = function ($Path) {
    ...
    if (\preg_match('~(?://|[^!\\d\\w\\._-]$)~i', $Path) || \preg_match('~^(?:/\\.\\.|./\\.|\\.{3})$~', \str_replace("\\", '/', \substr($Path, -3)))) {
        return \false;
    }
...
$phpMussel['IsInUse'] = function (array $Component) use(&$phpMussel) {
    ...
    foreach ($Files as $File) {
        if (\substr($File, 0, 11) === 'signatures/' && \preg_match('~,(?:[\\w\\d]+:)?' . \preg_quote(\substr($File, 11)) . ',~', ',' . $phpMussel['Config']['signatures']['Active'] . ',')) {
            return \true;
        }
    }
    return \false;
...
$phpMussel['FetchRemote'] = function () use(&$phpMussel) {
    ...
    $phpMussel['FetchRemote-ContextFree']($phpMussel['Components']['ThisComponent']['RemoteData'], $phpMussel['Components']['ThisComponent']['Remote']);
};
/**
...
$phpMussel['FetchRemote-ContextFree'] = function (&$RemoteData, &$Remote) use(&$phpMussel) {
    ...
    if ($RemoteData === \false) {
        ...
        if (\strtolower(\substr($Remote, -2)) === 'gz' && \substr($RemoteData, 0, 2) === "\37�") {
            $RemoteData = \gzdecode($RemoteData);
        }
        if (empty($RemoteData)) {
            $RemoteData = '-';
        }
        $phpMussel['FECacheAdd']($phpMussel['FE']['Cache'], $phpMussel['FE']['Rebuild'], $Remote, $RemoteData, $phpMussel['Time'] + 3600);
    }
};
/**
...
$phpMussel['GetAssetPath'] = function ($Asset, $CanFail = \false) use(&$phpMussel) {
    if ($phpMussel['Config']['template_data']['theme'] !== 'default' && \file_exists($phpMussel['Vault'] . 'fe_assets/' . $phpMussel['Config']['template_data']['theme'] . '/' . $Asset)) {
...
$phpMussel['NumberFormatter'] = new \AstraPrefixed\Maikuolan\Common\NumberFormatter($phpMussel['Config']['general']['numbers']);
/**
...
$phpMussel['Number_L10N_JS'] = function () use(&$phpMussel) {
    if ($phpMussel['NumberFormatter']->ConversionSet === 'Western') {
        ...
        $ConvJS = 'var nls=[' . $phpMussel['NumberFormatter']->getSetCSV($phpMussel['NumberFormatter']->ConversionSet) . '];return nls[l10nd]||l10nd';
    }
    return \sprintf('function l10nn(l10nd){%4$s};function nft(r){var x=r.indexOf(\'.\')!=-1?' . '\'%1$s\'+r.replace(/^.*\\./gi,\'\'):\'\',n=r.replace(/\\..*$/gi,\'\').rep' . 'lace(/[^0-9]/gi,\'\'),t=n.length;for(e=\'\',b=%5$d,i=1;i<=t;i++){b>%3$d' . '&&(b=1,e=\'%2$s\'+e);var e=l10nn(n.substring(t-i,t-(i-1)))+e;b++}var t=' . 'x.length;for(y=\'\',b=1,i=1;i<=t;i++){var y=l10nn(x.substring(t-i,t-(i-' . '1)))+y}return e+y}', $phpMussel['NumberFormatter']->DecimalSeparator, $phpMussel['NumberFormatter']->GroupSeparator, $phpMussel['NumberFormatter']->GroupSize, $ConvJS, $phpMussel['NumberFormatter']->GroupOffset + 1);
};
/**
...
$phpMussel['FilterSwitch'] = function (array $Switches, $Selector, &$StateModified, &$Redirect, &$Options) use(&$phpMussel) {
    foreach ($Switches as $Switch) {
        ...
        $phpMussel['FE'][$Switch] = empty($phpMussel['QueryVars'][$Switch]) ? \false : $phpMussel['QueryVars'][$Switch] === 'true' && !$State || $phpMussel['QueryVars'][$Switch] !== 'true' && $State;
        if ($State) {
            $StateModified = \true;
        }
...
$phpMussel['Quarantine-RecursiveList'] = function ($DeleteMode = \false) use(&$phpMussel) {
    ...
    foreach ($List as $Item => $List) {
        ...
        if ($DeleteMode) {
            ...
            $phpMussel['FE']['state_msg'] .= '<code>' . $DeleteMe . '</code> ' . $phpMussel['L10N']->getString(\unlink($phpMussel['qfuPath'] . $DeleteMe) ? 'response_file_deleted' : 'response_delete_error') . '<br />';
            continue;
        }
        $Key++;
        $Arr[$Key] = ['QFU-Name' => \substr($Item, $Offset), 'QFU-JS-ID' => \substr($Item, $Offset, -4), 'QFU-Size' => \filesize($Item)];
        $phpMussel['FormatFilesize']($Arr[$Key]['QFU-Size']);
        $Head = $phpMussel['ReadFile']($Item, 256);
        /** Upload date/time. */
        $Arr[$Key]['Upload-Date'] = ($DatePos = \strpos($Head, 'Time/Date Uploaded: ')) !== \false ? $phpMussel['TimeFormat']((int) \substr($Head, $DatePos + 20, 16), $phpMussel['Config']['general']['timeFormat']) : $phpMussel['L10N']->getString('field_filetype_unknown');
        /** Upload origin. */
        $Arr[$Key]['Upload-Origin'] = ($OriginStartPos = \strpos($Head, 'Uploaded From: ')) !== \false && ($OriginEndPos = \strpos($Head, ' ', $OriginStartPos + 15)) !== \false ? \substr($Head, $OriginStartPos + 15, $OriginEndPos - $OriginStartPos - 15) : $phpMussel['L10N']->getString('field_filetype_unknown');
        /** If the phpMussel QFU (Quarantined File Upload) header isn't found, it probably isn't a quarantined file. */
        if (($HeadPos = \strpos($Head, "�phpMussel!")) !== \false && \substr($Head, $HeadPos + 31, 1) === "\1") {
        ...
        if (\strlen($Arr[$Key]['Upload-MD5']) === 32) {
            $Arr[$Key]['Upload-MD5'] = \sprintf('<a href="https://www.virustotal.com/#/file/%1$s" rel="noopener noreferrer external">%1$s</a>', $Arr[$Key]['Upload-MD5']);
        }
    }
    return $Arr;
};
...
$phpMussel['Quarantine-Restore'] = function ($File, $Key) use(&$phpMussel) {
    ...
    /** Fetch headers. */
    if (($HeadPos = \strpos($Data, "�phpMussel!")) === \false || \substr($Data, $HeadPos + 31, 1) !== "\1") {
...
$phpMussel['Traverse'] = function ($Path) {
    return !\preg_match('~(?://|(?<![\\da-z])\\.\\.(?![\\da-z])|/\\.(?![\\da-z])|(?<![\\da-z])\\./|[\\x01-\\x1f\\[-^`?*$])~i', \str_replace("\\", '/', $Path));
};
/**
...
$phpMussel['UpdatesHandler'] = function ($Action, $ID = '') use(&$phpMussel) {
    ...
    if ($Action === 'update-and-activate-component') {
        ...
        foreach ($ID as $ThisID) {
            ...
            if (isset($phpMussel['Components']['Meta'][$ThisID]) && $phpMussel['IsActivable']($phpMussel['Components']['Meta'][$ThisID]) && !$phpMussel['IsInUse']($phpMussel['Components']['Meta'][$ThisID])) {
...
$phpMussel['UpdatesHandler-Update'] = function ($ID) use(&$phpMussel) {
    ...
    foreach ($ID as $ThisTarget) {
        if (!isset($phpMussel['Components']['Meta'][$ThisTarget]['Remote'], $phpMussel['Components']['Meta'][$ThisTarget]['Reannotate'])) {
            continue;
        }
        $BytesAdded = 0;
        $BytesRemoved = 0;
        $TimeRequired = \microtime(\true);
        $Reactivate = \false;
        if ($phpMussel['IsActivable']($phpMussel['Components']['Meta'][$ThisTarget]) && ($Reactivate = $phpMussel['IsInUse']($phpMussel['Components']['Meta'][$ThisTarget]))) {
        ...
        $phpMussel['CheckConstraints']($phpMussel['Components']['RemoteMeta'][$ThisTarget], \true);
        $UpdateFailed = \false;
        if ($phpMussel['Components']['RemoteMeta'][$ThisTarget]['All Constraints Met'] && !empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['From']) && !empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To']) && !empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Reannotate']) && $phpMussel['Traverse']($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Reannotate']) && ($ThisReannotate = $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Reannotate']) && \file_exists($phpMussel['Vault'] . $ThisReannotate) && ($OldMeta = $phpMussel['Updater-IO']->readFile($phpMussel['Vault'] . $ThisReannotate)) && \preg_match("~(\n" . \preg_quote($ThisTarget) . ":?)(\n [^\n]*)*\n~i", $OldMeta, $OldMetaMatches) && ($OldMetaMatches = $OldMetaMatches[0]) && \preg_match("~(\n" . \preg_quote($ThisTarget) . ":?)(\n [^\n]*)*\n~i", $NewMeta, $NewMetaMatches) && ($NewMetaMatches = $NewMetaMatches[0])) {
            ...
            for ($Iterate = 0; $Iterate < $Count; $Iterate++) {
                ...
                if ($Rollback) {
                    if (isset($phpMussel['RemoteFiles'][$ThisFileName]) && !isset($phpMussel['IgnoredFiles'][$ThisFileName]) && \is_readable($phpMussel['Vault'] . $ThisFileName)) {
                ...
                if (!empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['Checksum'][$Iterate]) && !empty($phpMussel['Components']['Meta'][$ThisTarget]['Files']['Checksum'][$Iterate]) && $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['Checksum'][$Iterate] === $phpMussel['Components']['Meta'][$ThisTarget]['Files']['Checksum'][$Iterate]) {
                    $phpMussel['IgnoredFiles'][$ThisFileName] = \true;
                    continue;
                }
                if (empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['From'][$Iterate]) || !($ThisFile = $phpMussel['Request']($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['From'][$Iterate]))) {
                    $Iterate = 0;
                    $Rollback = \true;
                    continue;
                }
                if (\strtolower(\substr($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['From'][$Iterate], -2)) === 'gz' && \strtolower(\substr($ThisFileName, -2)) !== 'gz' && \substr($ThisFile, 0, 2) === "\37�") {
                ...
                if (!empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['Checksum'][$Iterate])) {
                    ...
                    if (\hash('md5', $ThisFile) . ':' . $ThisLen !== $ThisChecksum && \hash('sha1', $ThisFile) . ':' . $ThisLen !== $ThisChecksum && \hash('sha256', $ThisFile) . ':' . $ThisLen !== $ThisChecksum) {
                        $phpMussel['FE']['state_msg'] .= '<code>' . $ThisTarget . '</code> – ' . '<code>' . $ThisFileName . '</code> – ' . $phpMussel['L10N']->getString('response_checksum_error') . '<br />';
                        if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['On Checksum Error'])) {
                            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['On Checksum Error'], \true);
                        }
                        $Iterate = 0;
                        $Rollback = \true;
                        continue;
                ...
                if (\preg_match('~\\.(?:css|dat|gif|inc|jpe?g|php|png|ya?ml|[a-z]{0,2}db)$~i', $ThisFileName) && !$phpMussel['SanityCheck']($ThisFileName, $ThisFile)) {
                    $phpMussel['FE']['state_msg'] .= \sprintf('<code>%s</code> – <code>%s</code> – %s<br />', $ThisTarget, $ThisFileName, $phpMussel['L10N']->getString('response_sanity_1'));
                    if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['On Sanity Error'])) {
                        $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['On Sanity Error'], \true);
                    }
                    $Iterate = 0;
                    $Rollback = \true;
                    continue;
            ...
            if ($Rollback) {
                ...
                if (!empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To']) && \is_array($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To'])) {
                ...
                if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['Files']['To'])) {
                    ...
                    foreach ($ThisArr as $ThisFile) {
                        if (!empty($ThisFile) && $phpMussel['Traverse']($ThisFile)) {
                            ...
                            if (!isset($phpMussel['RemoteFiles'][$ThisFile]) && !isset($phpMussel['IgnoredFiles'][$ThisFile]) && \file_exists($phpMussel['Vault'] . $ThisFile)) {
                ...
                if (empty($phpMussel['Components']['Meta'][$ThisTarget]['Version']) && empty($phpMussel['Components']['Meta'][$ThisTarget]['Files'])) {
                    ...
                    if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['When Install Succeeds'])) {
                        $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['When Install Succeeds'], \true);
                    }
                } else {
                    ...
                    if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['When Update Succeeds'])) {
                        $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['When Update Succeeds'], \true);
                    }
                }
        ...
        if ($UpdateFailed) {
            ...
            if (empty($phpMussel['Components']['Meta'][$ThisTarget]['Version']) && empty($phpMussel['Components']['Meta'][$ThisTarget]['Files'])) {
                ...
                if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['When Install Fails'])) {
                    $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['When Install Fails'], \true);
                }
            } else {
                ...
                if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['When Update Fails'])) {
                    $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['When Update Fails'], \true);
                }
            }
        }
        $phpMussel['FormatFilesize']($BytesAdded);
        $phpMussel['FormatFilesize']($BytesRemoved);
        $phpMussel['FE']['state_msg'] .= \sprintf($phpMussel['FE']['CronMode'] ? " « +%s | -%s | %s »\n" : ' <code><span class="txtGn">+%s</span> | <span class="txtRd">-%s</span> | <span class="txtOe">%s</span></code><br />', $BytesAdded, $BytesRemoved, $phpMussel['NumberFormatter']->format(\microtime(\true) - $TimeRequired, 3));
        if ($Reactivate === \true) {
...
$phpMussel['UpdatesHandler-Uninstall'] = function ($ID) use(&$phpMussel) {
    ...
    if (empty($InUse) && !empty($phpMussel['Components']['Meta'][$ID]['Files']['To']) && $ID !== 'l10n/' . $phpMussel['Config']['general']['lang'] && $ID !== 'theme/' . $phpMussel['Config']['template_data']['theme'] && $ID !== 'phpMussel' && !empty($phpMussel['Components']['Meta'][$ID]['Reannotate']) && !empty($phpMussel['Components']['Meta'][$ID]['Uninstallable']) && ($OldMeta = $phpMussel['Updater-IO']->readFile($phpMussel['Vault'] . $phpMussel['Components']['Meta'][$ID]['Reannotate'])) && \preg_match("~(\n" . \preg_quote($ID) . ":?)(\n [^\n]*)*\n~i", $OldMeta, $OldMetaMatches) && ($OldMetaMatches = $OldMetaMatches[0])) {
        ...
        if (!empty($phpMussel['Components']['Meta'][$ID]['When Uninstall Succeeds'])) {
            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ID]['When Uninstall Succeeds'], \true);
        }
    } else {
        ...
        if (!empty($phpMussel['Components']['Meta'][$ID]['When Uninstall Fails'])) {
            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ID]['When Uninstall Fails'], \true);
        }
    }
    $phpMussel['FormatFilesize']($BytesRemoved);
    $phpMussel['FE']['state_msg'] .= \sprintf($phpMussel['FE']['CronMode'] ? " « -%s | %s »\n" : ' <code><span class="txtRd">-%s</span> | <span class="txtOe">%s</span></code><br />', $BytesRemoved, $phpMussel['NumberFormatter']->format(\microtime(\true) - $TimeRequired, 3));
};
/**
...
$phpMussel['UpdatesHandler-Activate'] = function ($ID) use(&$phpMussel) {
    ...
    $phpMussel['Activation'] = ['Config' => $phpMussel['Updater-IO']->readFile($phpMussel['Vault'] . $phpMussel['FE']['ActiveConfigFile']), 'Active' => $phpMussel['Config']['signatures']['Active'], 'Modified' => \false];
    $InUse = $phpMussel['ComponentFunctionUpdatePrep']($ID);
    $phpMussel['FE']['state_msg'] .= '<code>' . $ID . '</code> – ';
    if (empty($InUse) && !empty($phpMussel['Components']['Meta'][$ID]['Files']['To'])) {
        $phpMussel['Activation']['Active'] = \array_unique(\array_filter(\explode(',', $phpMussel['Activation']['Active']), function ($Component) use(&$phpMussel) {
        ...
        foreach ($phpMussel['Components']['Meta'][$ID]['Files']['To'] as $ThisFile) {
            if (!empty($ThisFile) && \file_exists($phpMussel['Vault'] . $ThisFile) && \substr($ThisFile, 0, 11) === 'signatures/' && $phpMussel['Traverse']($ThisFile)) {
        ...
        if ($phpMussel['Activation']['Active'] !== $phpMussel['Config']['signatures']['Active']) {
            $phpMussel['Activation']['Modified'] = \true;
        }
    }
    if (!$phpMussel['Activation']['Modified'] || !$phpMussel['Activation']['Config']) {
        ...
        if (!empty($phpMussel['Components']['Meta'][$ID]['When Activation Fails'])) {
            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ID]['When Activation Fails'], \true);
        }
    } else {
        $EOL = \strpos($phpMussel['Activation']['Config'], "\r\nActive=") !== \false ? "\r\n" : "\n";
        $phpMussel['Activation']['Config'] = \str_replace($EOL . "Active='" . $phpMussel['Config']['signatures']['Active'] . "'" . $EOL, $EOL . "Active='" . $phpMussel['Activation']['Active'] . "'" . $EOL, $phpMussel['Activation']['Config']);
        ...
        if (!empty($phpMussel['Components']['Meta'][$ID]['When Activation Succeeds'])) {
            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ID]['When Activation Succeeds'], \true);
        }
        $Success = \true;
    }
    /** Cleanup. */
    ...
    if (!empty($Success) && !empty($phpMussel['Components']['Meta'][$ID]['Dependencies']) && \is_array($phpMussel['Components']['Meta'][$ID]['Dependencies'])) {
        foreach ($phpMussel['Components']['Meta'][$ID]['Dependencies'] as $Dependency => $Constraints) {
            if (!isset($phpMussel['Components']['Meta'][$Dependency]) || empty($phpMussel['Components']['Installed Versions'][$Dependency]) || !$phpMussel['IsActivable']($phpMussel['Components']['Meta'][$Dependency]) || $phpMussel['IsInUse']($phpMussel['Components']['Meta'][$ThisTarget])) {
                continue;
            }
...
$phpMussel['UpdatesHandler-Deactivate'] = function ($ID) use(&$phpMussel) {
    ...
    $phpMussel['Deactivation'] = ['Config' => $phpMussel['Updater-IO']->readFile($phpMussel['Vault'] . $phpMussel['FE']['ActiveConfigFile']), 'Active' => $phpMussel['Config']['signatures']['Active'], 'Modified' => \false];
    $InUse = $phpMussel['ComponentFunctionUpdatePrep']($ID);
    $phpMussel['FE']['state_msg'] .= '<code>' . $ID . '</code> – ';
    if (!empty($InUse) && !empty($phpMussel['Components']['Meta'][$ID]['Files']['To'])) {
        $phpMussel['Deactivation']['Active'] = \array_unique(\array_filter(\explode(',', $phpMussel['Deactivation']['Active']), function ($Component) use(&$phpMussel) {
            $Component = \strpos($Component, ':') === \false ? $Component : \substr($Component, \strpos($Component, ':') + 1);
            return $Component && \file_exists($phpMussel['sigPath'] . $Component);
        }));
        ...
        foreach ($phpMussel['Components']['Meta'][$ID]['Files']['To'] as $phpMussel['Deactivation']['ThisFile']) {
            if (\substr($phpMussel['Deactivation']['ThisFile'], 0, 11) === 'signatures/') {
                $phpMussel['Deactivation']['Active'] = \preg_replace('~,(?:[\\w\\d]+:)?' . \preg_quote(\substr($phpMussel['Deactivation']['ThisFile'], 11)) . ',~', ',', $phpMussel['Deactivation']['Active']);
            }
        }
        ...
        if ($phpMussel['Deactivation']['Active'] !== $phpMussel['Config']['signatures']['Active']) {
            $phpMussel['Deactivation']['Modified'] = \true;
        }
    }
    if (!$phpMussel['Deactivation']['Modified'] || !$phpMussel['Deactivation']['Config']) {
        ...
        if (!empty($phpMussel['Components']['Meta'][$ID]['When Deactivation Fails'])) {
            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ID]['When Deactivation Fails'], \true);
        }
    } else {
        $EOL = \strpos($phpMussel['Deactivation']['Config'], "\r\nActive=") !== \false ? "\r\n" : "\n";
        $phpMussel['Deactivation']['Config'] = \str_replace($EOL . "Active='" . $phpMussel['Config']['signatures']['Active'] . "'" . $EOL, $EOL . "Active='" . $phpMussel['Deactivation']['Active'] . "'" . $EOL, $phpMussel['Deactivation']['Config']);
        ...
        if (!empty($phpMussel['Components']['Meta'][$ID]['When Deactivation Succeeds'])) {
            $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ID]['When Deactivation Succeeds'], \true);
        }
    }
    /** Cleanup. */
...
$phpMussel['UpdatesHandler-Repair'] = function ($ID) use(&$phpMussel) {
    ...
    $ID = \array_unique($ID);
    foreach ($ID as $ThisTarget) {
        if (!isset($phpMussel['Components']['Meta'][$ThisTarget]['Files']['To'], $phpMussel['Components']['Meta'][$ThisTarget]['Remote'])) {
            continue;
        }
        $BytesAdded = 0;
        $BytesRemoved = 0;
        ...
        if (isset($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To'], $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['From'], $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['Checksum'])) {
        ...
        if (!$RepairFailed && $phpMussel['Components']['Meta'][$ThisTarget]['Files']['To'] === $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To']) {
            $Files = \count($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To']);
            for ($Iterator = 0; $Iterator < $Files; $Iterator++) {
                if (!isset($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To'][$Iterator], $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['From'][$Iterator], $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['Checksum'][$Iterator]) || !$phpMussel['Traverse']($phpMussel['Vault'] . $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Files']['To'][$Iterator])) {
                    $RepairFailed = \true;
                    break;
                }
                ...
                if (\hash('md5', $LocalFile) . ':' . $LocalFileSize === $RemoteChecksum || \hash('sha1', $LocalFile) . ':' . $LocalFileSize === $RemoteChecksum || \hash('sha256', $LocalFile) . ':' . $LocalFileSize === $RemoteChecksum) {
                    continue;
                }
                $RemoteFile = $phpMussel['Request']($RemoteFileFrom);
                if (\strtolower(\substr($RemoteFileFrom, -2)) === 'gz' && \strtolower(\substr($RemoteFileTo, -2)) !== 'gz' && \substr($RemoteFile, 0, 2) === "\37�") {
                    $RemoteFile = \gzdecode($RemoteFile);
                }
                $RemoteFileSize = \strlen($RemoteFile);
                if (\hash('md5', $RemoteFile) . ':' . $RemoteFileSize !== $RemoteChecksum && \hash('sha1', $RemoteFile) . ':' . $RemoteFileSize !== $RemoteChecksum && \hash('sha256', $RemoteFile) . ':' . $RemoteFileSize !== $RemoteChecksum || \preg_match('~\\.(?:css|dat|gif|inc|jpe?g|php|png|ya?ml|[a-z]{0,2}db)$~i', $RemoteFileTo) && !$phpMussel['SanityCheck']($RemoteFileTo, $RemoteFile)) {
                    $RepairFailed = \true;
                    continue;
                }
        ...
        if (!$RepairFailed && !empty($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Reannotate']) && $phpMussel['Traverse']($phpMussel['Components']['RemoteMeta'][$ThisTarget]['Reannotate']) && ($ThisReannotate = $phpMussel['Components']['RemoteMeta'][$ThisTarget]['Reannotate']) && \file_exists($phpMussel['Vault'] . $ThisReannotate) && ($OldMeta = $phpMussel['Updater-IO']->readFile($phpMussel['Vault'] . $ThisReannotate)) && \preg_match("~(\n" . \preg_quote($ThisTarget) . ":?)(\n [^\n]*)*\n~i", $OldMeta, $OldMetaMatches) && ($OldMetaMatches = $OldMetaMatches[0]) && ($NewMeta = $RemoteData) && \preg_match("~(\n" . \preg_quote($ThisTarget) . ":?)(\n [^\n]*)*\n~i", $NewMeta, $NewMetaMatches) && ($NewMetaMatches = $NewMetaMatches[0])) {
            $NewMeta = \str_replace($OldMetaMatches, $NewMetaMatches, $OldMeta);
            /** Assign updated component annotation. */
            $phpMussel['Updater-IO']->writeFile($phpMussel['Vault'] . $ThisReannotate, $NewMeta);
            ...
            if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['When Repair Succeeds'])) {
                $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['When Repair Succeeds'], \true);
            }
            /** Replace downstream meta with upstream meta. */
            $phpMussel['Components']['Meta'][$ThisTarget] = $phpMussel['Components']['RemoteMeta'][$ThisTarget];
        } else {
            ...
            if (!empty($phpMussel['Components']['Meta'][$ThisTarget]['When Repair Fails'])) {
                $phpMussel['FE_Executor']($phpMussel['Components']['Meta'][$ThisTarget]['When Repair Fails'], \true);
            }
        }
        $phpMussel['FormatFilesize']($BytesAdded);
        $phpMussel['FormatFilesize']($BytesRemoved);
...
$phpMussel['UpdatesHandler-Verify'] = function ($ID) use(&$phpMussel) {
    ...
    foreach ($ID as $ThisID) {
        ...
        for ($Iterate = 0; $Iterate < $Count; $Iterate++) {
            ...
            /** Sanity check. */
            if (\preg_match('~\\.(?:css|dat|gif|inc|jpe?g|php|png|ya?ml|[a-z]{0,2}db)$~i', $ThisFile)) {
                $Class = $phpMussel['SanityCheck']($ThisFile, $ThisFileData) ? 'txtGn' : 'txtRd';
                $Sanity = \sprintf('<span class="%s">%s</span>', $Class, $phpMussel['L10N']->getString($Class === 'txtGn' ? 'response_passed' : 'response_failed'));
                if ($Class === 'txtRd') {
                    $Passed = \false;
                }
            } else {
            ...
            if ($Checksum) {
                ...
                $Integrity = \sprintf('<span class="%s">%s</span>', $Class, $phpMussel['L10N']->getString($Class === 'txtGn' ? 'response_passed' : 'response_failed'));
            } else {
                $Class = 's';
                $Integrity = \sprintf('<span class="txtOe">%s</span>', $phpMussel['L10N']->getString('response_skipped'));
            }
            /** Append results. */
            $Table .= \sprintf('<code>%1$s</code> – %7$s%8$s – %9$s%10$s<br />%2$s – <code class="%6$s">%3$s</code><br />%4$s – <code class="%6$s">%5$s</code><hr />', $ThisFile, $phpMussel['L10N']->getString('label_actual'), $Actual ?: '?', $phpMussel['L10N']->getString('label_expected'), $Checksum ?: '?', $Class, $phpMussel['L10N']->getString('label_integrity_check'), $Integrity, $phpMussel['L10N']->getString('label_sanity_check'), $Sanity);
        }
        $Table .= '</blockquote>';
        $phpMussel['FE']['state_msg'] .= \sprintf('<div><span class="comCat" style="cursor:pointer"><code>%s</code> – <span class="%s">%s</span></span>%s</div>', $ThisID, $Passed ? 's' : 'txtRd', $phpMussel['L10N']->getString($Passed ? 'response_verification_success' : 'response_verification_failed'), $Table);
    }
};
/**
...
$phpMussel['SigInfoHandler'] = function (array $Active) use(&$phpMussel) {
    ...
    $Arr = ['Vendors' => $phpMussel['shorthand.yaml']['Vendor Search Patterns'], 'SigTypes' => $phpMussel['shorthand.yaml']['Metadata Search Pattern Partials']];
    /** Expand patterns for signature metadata. */
    foreach ($Arr['SigTypes'] as &$Type) {
        $Type = \sprintf('\\x1a(?![\\x80-\\x8f])[\\x0%1$s\\x1%1$s\\x2%1$s\\x3%1$s\\x4%1$s\\x5%1$s\\x6%1$s\\x7%1$s\\x8%1$s\\x9%1$s\\xa%1$s\\xb%1$s\\xc%1$s\\xd%1$s\\xe%1$s\\ef%1$s].', $Type);
    }
    ...
    $Classes = [['General_Command_Detections', ''], ['Filename', '\\n(?!>)'], ['Hash', '\\n[\\dA-Fa-f]{32,}:\\d+:'], ['Standard', '\\n(?!>)'], ['Standard_RegEx', '\\n(?!>)'], ['Normalised', '\\n(?!>)'], ['Normalised_RegEx', '\\n(?!>)'], ['HTML', '\\n(?!>)'], ['HTML_RegEx', '\\n(?!>)'], ['PE_Extended', '\\n\\$PE\\w+:[\\dA-Fa-f]{32,}:\\d+:'], ['PE_Sectional', '\\n\\d+:[\\dA-Fa-f]{32,}:'], ['Complex_Extended', '\\n\\$\\S+;'], ['URL_Scanner', '\\n(?:TLD|(?:DOMAIN|URL)(?:-NOLOOKUP)?|QUERY)\\S+:']];
    ...
    foreach ($Subs as $Sub) {
        ...
        foreach ($Totals[$Sub] as $Key => &$Total) {
            ...
            $Label = $phpMussel['L10N']->getString($Key === 'Other' && $Sub === 'SigTypes' ? 'siginfo_key_Other_Metadata' : 'siginfo_key_' . $Key);
 ...
 * @return void
 */
$phpMussel['InitialPrepwork'] = function ($Title = '', $Tips = '', $JS = \true) use(&$phpMussel) {
    /** Set page title. */
...
$phpMussel['FileManager-IsLogFile'] = function ($File) use(&$phpMussel) {
    ...
    if (!$Pattern_scan_log && $phpMussel['Config']['general']['scan_log']) {
        $Pattern_scan_log = $phpMussel['BuildLogPattern']($phpMussel['Config']['general']['scan_log'], \true);
    }
    static $Pattern_scan_log_serialized = \false;
    if (!$Pattern_scan_log_serialized && $phpMussel['Config']['general']['scan_log_serialized']) {
        $Pattern_scan_log_serialized = $phpMussel['BuildLogPattern']($phpMussel['Config']['general']['scan_log_serialized'], \true);
    }
    static $Pattern_scan_kills = \false;
    if (!$Pattern_scan_kills && $phpMussel['Config']['general']['scan_kills']) {
        $Pattern_scan_kills = $phpMussel['BuildLogPattern']($phpMussel['Config']['general']['scan_kills'], \true);
    }
    static $Pattern_FrontEndLog = \false;
    if (!$Pattern_FrontEndLog && $phpMussel['Config']['general']['FrontEndLog']) {
        $Pattern_FrontEndLog = $phpMussel['BuildLogPattern']($phpMussel['Config']['general']['FrontEndLog'], \true);
    }
    static $Pattern_PHPMailer_EventLog = \false;
    if (!$Pattern_PHPMailer_EventLog && $phpMussel['Config']['PHPMailer']['EventLog']) {
        $Pattern_PHPMailer_EventLog = $phpMussel['BuildLogPattern']($phpMussel['Config']['PHPMailer']['EventLog'], \true);
    }
    return \preg_match('~\\.log(?:\\.gz)?$~', \strtolower($File)) || $phpMussel['Config']['general']['scan_log'] && \preg_match($Pattern_scan_log, $File) || $phpMussel['Config']['general']['scan_log_serialized'] && \preg_match($Pattern_scan_log_serialized, $File) || $phpMussel['Config']['general']['scan_kills'] && \preg_match($Pattern_scan_kills, $File) || $phpMussel['Config']['general']['FrontEndLog'] && \preg_match($Pattern_FrontEndLog, $File) || $phpMussel['Config']['PHPMailer']['EventLog'] && \preg_match($Pattern_PHPMailer_EventLog, $File);
};
/**
...
$phpMussel['FELogger'] = function ($IPAddr, $User, $Message) use(&$phpMussel) {
    /** Guard. */
    if (empty($phpMussel['FE']['DateTime']) || !$phpMussel['Config']['general']['FrontEndLog'] || !($File = $phpMussel['BuildPath']($phpMussel['Vault'] . $phpMussel['Config']['general']['FrontEndLog']))) {
        return;
    }
...
$phpMussel['SendEmail'] = function (array $Recipients = [], $Subject = '', $Body = '', $AltBody = '', array $Attachments = []) use(&$phpMussel) {
    ...
    $EventLogData = \sprintf('%s - %s - ', $phpMussel['Config']['legal']['pseudonymise_ip_addresses'] ? $phpMussel['Pseudonymise-IP']($_SERVER[$phpMussel['IPAddr']]) : $_SERVER[$phpMussel['IPAddr']], isset($phpMussel['FE']['DateTime']) ? $phpMussel['FE']['DateTime'] : $phpMussel['TimeFormat']($phpMussel['Time'], $phpMussel['Config']['general']['timeFormat']));
    /** Operation success state. */
    $State = \false;
    /** Check whether class exists to either load it and continue or fail the operation. */
    if (!\class_exists('AstraPrefixed\\PHPMailer\\PHPMailer\\PHPMailer')) {
        $EventLogData .= $phpMussel['L10N']->getString('state_failed_missing') . "\n";
    } else {
        try {
            /** Create a new PHPMailer instance. */
            $Mail = new \AstraPrefixed\PHPMailer\PHPMailer\PHPMailer();
            ...
            if ($phpMussel['Config']['PHPMailer']['SkipAuthProcess']) {
                $Mail->SMTPOptions = ['ssl' => ['verify_peer' => \false, 'verify_peer_name' => \false, 'allow_self_signed' => \true]];
            }
            ...
            if (!empty($phpMussel['Config']['PHPMailer']['SMTPSecure']) && $phpMussel['Config']['PHPMailer']['SMTPSecure'] !== '-') {
            ...
            $Mail->setFrom($phpMussel['Config']['PHPMailer']['setFromAddress'], $phpMussel['Config']['PHPMailer']['setFromName']);
            /** Set the optional "reply to" address and name. */
            if (!empty($phpMussel['Config']['PHPMailer']['addReplyToAddress']) && !empty($phpMussel['Config']['PHPMailer']['addReplyToName'])) {
                $Mail->addReplyTo($phpMussel['Config']['PHPMailer']['addReplyToAddress'], $phpMussel['Config']['PHPMailer']['addReplyToName']);
            }
            ...
            $EventLogData .= ($State ? \sprintf($phpMussel['L10N']->getString('state_email_sent'), $SuccessDetails) : $phpMussel['L10N']->getString('response_error') . ' - ' . $Mail->ErrorInfo) . "\n";
        } catch (\Exception $e) {
    ...
    foreach ($Arr as $Key => $Value) {
        ...
        if (!\is_array($Value)) {
            if (\substr($Value, 0, 2) === '{"' && \substr($Value, -2) === '"}') {
                $Try = \json_decode($Value, \true);
                if ($Try !== null) {
                    $Value = $Try;
                }
            } elseif (\preg_match('~\\.ya?ml$~i', $Key) || \preg_match('~^(?:Data|\\d+)$~', $Key) && \preg_match('~\\.ya?ml$~i', $ParentKey) || \substr($Value, 0, 4) === "---\n") {
                $Try = new \AstraPrefixed\Maikuolan\Common\YAML();
        ...
        if (\is_array($Value)) {
            if ($Depth === 0) {
                ...
                $Size = isset($Value['Data']) && \is_string($Value['Data']) ? \strlen($Value['Data']) : (isset($Value[0]) && \is_string($Value[0]) ? \strlen($Value[0]) : \false);
                if ($Size !== \false) {
...
$phpMussel['Formatter'] = function (&$In) {
    ...
    foreach ($Data as &$Block) {
        ...
        if ($SeparatorType === 2) {
            $Block = \preg_replace(['~(a\\:\\d+\\:)\\{~', '~("|\\d);\\}~', '~\\:(\\d+)~', '~\\:"([^"]+)"~'], ['\\1<span class="txtRd">{</span>', '\\1;<span class="txtRd">}</span>', ':<span class="txtGn">\\1</span>', ':"<span class="txtBl">\\1</span>"'], $Block);
        }
    }
    $In = '<div style="filter:saturate(60%)"><span class="' . ($SeparatorType === 2 ? 'txtOe' : 's') . '">' . \implode('', $Data) . '</span></div>';
};
/**
...
$phpMussel['LTRinRTF'] = function ($String = '') use(&$phpMussel) {
    /** Get direction. */
    $Direction = !isset($phpMussel['L10N']) || empty($phpMussel['L10N']->Data['Text Direction']) || $phpMussel['L10N']->Data['Text Direction'] !== 'rtl' ? 'ltr' : 'rtl';
    ...
    return \preg_replace(['~^(.+)-&gt;(.+)$~i', '~^(.+)➡(.+)$~i'], ['\\2&lt;-\\1', '\\2⬅\\1'], $String);
};
/**
...
$phpMussel['CheckConstraints'] = function (array &$ThisComponent, $Source = \false, $Name = '') use(&$phpMussel) {
    $ThisComponent['All Constraints Met'] = \true;
    ...
    if (!isset($ThisComponent['Dependencies']) || !\is_array($ThisComponent['Dependencies']) || $Name && !isset($phpMussel['Components']['Installed Versions'][$Name])) {
        return;
    }
    foreach ($ThisComponent['Dependencies'] as $Dependency => $Constraints) {
        ...
        if ($Constraints === 'Latest' || \strlen($Constraints) < 1) {
            ...
            $ThisComponent['Dependency Status'] .= \sprintf('<span class="txtRd">%s – %s</span><br />', $Dependency, $phpMussel['L10N']->getString('response_not_satisfied'));
        } elseif (isset($phpMussel['Components']['Installed Versions'][$Dependency]) && $phpMussel['Operation']->singleCompare($phpMussel['Components']['Installed Versions'][$Dependency], $Constraints) || \extension_loaded($Dependency) && ($phpMussel['Components']['Installed Versions'][$Dependency] = (new \ReflectionExtension($Dependency))->getVersion()) && $phpMussel['Operation']->singleCompare($phpMussel['Components']['Installed Versions'][$Dependency], $Constraints)) {
            $ThisComponent['Dependency Status'] .= \sprintf('<span class="txtGn">%s%s – %s</span><br />', $Dependency, $Constraints === '*' ? '' : ' (' . $Constraints . ')', $phpMussel['L10N']->getString('response_satisfied'));
        } elseif ($Source && isset($phpMussel['Components']['Available Versions'][$Dependency]) && $phpMussel['Operation']->singleCompare($phpMussel['Components']['Available Versions'][$Dependency], $Constraints)) {
            $ThisComponent['Dependency Status'] .= \sprintf('<span class="txtOe">%s%s – %s</span><br />', $Dependency, $Constraints === '*' ? '' : ' (' . $Constraints . ')', $phpMussel['L10N']->getString('response_ready_to_install'));
            ...
            $ThisComponent['All Constraints Met'] = \false;
            $ThisComponent['Dependency Status'] .= \sprintf('<span class="txtRd">%s%s – %s</span><br />', $Dependency, $Constraints === '*' ? '' : ' (' . $Constraints . ')', $phpMussel['L10N']->getString('response_not_satisfied'));
        }
    }
    if ($ThisComponent['Dependency Status']) {
        $ThisComponent['Dependency Status'] = \sprintf('<hr /><small><span class="s">%s</span><br />%s</small>', $phpMussel['L10N']->getString('label_dependencies'), $ThisComponent['Dependency Status']);
    }
};
/**
