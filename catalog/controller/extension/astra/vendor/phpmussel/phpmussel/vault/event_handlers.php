...
$phpMussel['Events']->addHandler('writeToSerialLog', function () use(&$phpMussel) {
    /** Guard. */
    if (!$phpMussel['Config']['general']['scan_log_serialized'] || !($File = $phpMussel['BuildPath']($phpMussel['Vault'] . $phpMussel['Config']['general']['scan_log_serialized']))) {
        return \false;
    }
    ...
    $ScanData = empty($phpMussel['whyflagged']) ? $phpMussel['L10N']->getString('data_not_available') : \trim($phpMussel['whyflagged']);
    ...
    $Data = \serialize(['start_time' => isset($phpMussel['InstanceCache']['start_time']) ? $phpMussel['InstanceCache']['start_time'] : '-', 'end_time' => isset($phpMussel['InstanceCache']['end_time']) ? $phpMussel['InstanceCache']['end_time'] : '-', 'origin' => $Origin, 'objects_scanned' => isset($phpMussel['InstanceCache']['objects_scanned']) ? $phpMussel['InstanceCache']['objects_scanned'] : 0, 'detections_count' => isset($phpMussel['InstanceCache']['detections_count']) ? $phpMussel['InstanceCache']['detections_count'] : 0, 'scan_errors' => isset($phpMussel['InstanceCache']['scan_errors']) ? $phpMussel['InstanceCache']['scan_errors'] : 0, 'detections' => $ScanData]) . "\n";
...
$phpMussel['Events']->addHandler('writeToScanLog', function ($Data) use(&$phpMussel) {
    /** Guard. */
    if (!$phpMussel['Config']['general']['scan_log'] || !($File = $phpMussel['BuildPath']($phpMussel['Vault'] . $phpMussel['Config']['general']['scan_log']))) {
        return \false;
    }
    if (!\file_exists($File)) {
...
$phpMussel['Events']->addHandler('writeToScanKillsLog', function ($Data) use(&$phpMussel) {
    /** Guard. */
    if (empty($phpMussel['killdata']) || !$phpMussel['Config']['general']['scan_kills'] || !($File = $phpMussel['BuildPath']($phpMussel['Vault'] . $phpMussel['Config']['general']['scan_kills']))) {
        return \false;
    }
    if (!\file_exists($File)) {
        $Data = $phpMussel['safety'] . "\n\n" . $Data;
...
$phpMussel['Events']->addHandler('error', function ($Data) use(&$phpMussel) {
    ...
    $Message = \sprintf('[%s] Error at %s:L%d (error code %d)%s.', \date('c', \time()), empty($Data[2]) ? '?' : $Data[2], empty($Data[3]) ? 0 : $Data[3], empty($Data[0]) ? 0 : $Data[0], empty($Data[1]) ? '' : ': "' . $Data[1] . '"');
...
$phpMussel['Events']->addHandler('final', function () use(&$phpMussel) {
    /** Guard. */
    if (!isset($phpMussel['Pending-Error-Log-Data']) || !$phpMussel['Config']['general']['error_log'] || !($File = $phpMussel['BuildPath']($phpMussel['Vault'] . $phpMussel['Config']['general']['error_log']))) {
        return \false;
    }
    $Truncate = $phpMussel['ReadBytes']($phpMussel['Config']['general']['truncate']);
...
$phpMussel['Events']->addHandler('writeToPHPMailerEventLog', function ($Data) use(&$phpMussel) {
    /** Guard. */
    if (!$phpMussel['Config']['PHPMailer']['event_log'] || !($EventLog = $phpMussel['BuildPath']($phpMussel['Vault'] . $phpMussel['Config']['PHPMailer']['event_log']))) {
        return \false;
    }
    $Truncate = $phpMussel['ReadBytes']($phpMussel['Config']['general']['truncate']);
