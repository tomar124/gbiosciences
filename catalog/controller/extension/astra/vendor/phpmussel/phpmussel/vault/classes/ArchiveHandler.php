 ...
 */
namespace AstraPrefixed\phpMussel\ArchiveHandler;
    ...
    public function __construct($File)
    {
        ...
        if ($Check && isset($Matches, $Matches[0], $Matches[0][0])) {
            ...
            for ($Iterator = 0; $Iterator < $Count; $Iterator++) {
                $Tree[$Iterator] = ['Object Number' => $Matches[1][$Iterator], 'Generation Number' => $Matches[2][$Iterator], 'Data' => $Matches[3][$Iterator]];
                ...
                while (($SPos = \strpos($Tree[$Iterator]['Data'], '/', $Offset)) !== \false) {
                    ...
                    foreach ([['[[', ']]'], ['{{', '}}'], ['((', '))'], ['<<', '>>']] as $Boundary) {
                        ...
                        while (($Working = \substr($Tree[$Iterator]['Data'], $BoundaryOffset, $BoundaryClose - $BoundaryOffset)) && ($RPos = \strpos($Working, $Boundary[0])) !== \false && ($Try = \strpos($Tree[$Iterator]['Data'], $Boundary[1], $BoundaryClose + $BoundaryWidth)) !== \false) {
                            $BoundaryOffset += $RPos + $BoundaryWidth;
                            $BoundaryClose = $Try;
                        }
                        if ($BoundaryOpen !== \false && $BoundaryClose !== \false && $BoundaryClose > $BoundaryOpen && ($NextSPos === \false || $BoundaryOpen < $NextSPos && \trim(\substr($Tree[$Iterator]['Data'], $BoundaryClose + $BoundaryWidth, $NextSPos - $BoundaryClose - $BoundaryWidth)) === '')) {
            ...
            if (isset($Tree[$Iterator]['Data']) && \is_array($Tree[$Iterator]['Data'])) {
                foreach ($Tree[$Iterator]['Data'] as $ParamKey => $ParamValue) {
                    ...
                    if ($Check) {
                        ...
                        if (isset($Tree[$Matches[1]], $Tree[$Matches[1]]['Object Number'], $Tree[$Matches[1]]['Generation Number']) && $Matches[1] === $Tree[$Matches[1]]['Object Number'] - 1 && $Matches[2] === (int) $Tree[$Matches[1]]['Generation Number']) {
                            if (isset($Tree[$Matches[1]]['Data']['Type'], $Tree[$Matches[1]]['Data']['Count']) && $Tree[$Matches[1]]['Data']['Type'] === '/' . $ParamKey) {
        ...
        for ($Iterator = 0; $Iterator < $Counts; $Iterator++) {
            if (isset($Tree[$Iterator]['Data']) && \is_array($Tree[$Iterator]['Data'])) {
                if (isset($Tree[$Iterator]['Data']['Type'], $Tree[$Iterator]['Stream']) && $Tree[$Iterator]['Data']['Type'] === '/EmbeddedFile') {
                    $Object = [];
