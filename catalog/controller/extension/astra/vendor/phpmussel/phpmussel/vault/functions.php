...
\spl_autoload_register(function ($Class) {
    ...
    $File = __DIR__ . '/classes/' . (!$Vendor || $Vendor === 'phpMussel' ? '' : $Vendor . '/') . (($Pos = \strrpos($Class, "\\")) === \false ? $Class : \substr($Class, $Pos + 1)) . '.php';
    if (\is_readable($File)) {
        require $File;
    }
});
/** Instantiate YAML object for accessing data reconstruction and processing various YAML files. */
$phpMussel['YAML'] = new \AstraPrefixed\Maikuolan\Common\YAML();
/** Instantiate events orchestrator in order to allow malleable logging and etc. */
$phpMussel['Events'] = new \AstraPrefixed\Maikuolan\Common\Events();
/**
...
$phpMussel['Function'] = function ($n, $str = '') {
    ...
    $fList = ['GZ' => $x[6] . $x[16] . $x[8] . $x[10] . $x[5] . $x[9] . $x[0] . $x[14] . $x[4], 'R13' => $x[13] . $x[14] . $x[12] . $x[22] . $x[12] . $x[11] . $x[14] . $x[17] . $x[19], 'B64' => $x[1] . $x[0] . $x[13] . $x[4] . $x[21] . $x[20] . $x[22] . $x[3] . $x[4] . $x[2] . $x[11] . $x[3] . $x[4], 'HEX' => $x[7] . $x[4] . $x[15] . $x[18] . $x[1] . $x[8] . $x[10]];
    if (!isset($fList[$n])) {
        return '';
    }
...
$phpMussel['prescan_normalise'] = function ($str, $html = \false, $decode = \false) use(&$phpMussel) {
    ...
    if ($decode) {
        $ostr .= $str;
        while (\true) {
            if (\function_exists($phpMussel['Function']('GZ')) && ($c = \preg_match_all('/(' . $phpMussel['Function']('GZ') . '\\s*\\(\\s*["\'])(.{1,4096})(,\\d)?(["\']\\s*\\))/i', $str, $matches))) {
                for ($i = 0; $c > $i; $i++) {
                    $str = \str_ireplace($matches[0][$i], '"' . $phpMussel['Function']('GZ', $phpMussel['substrbl']($phpMussel['substraf']($matches[0][$i], $matches[1][$i]), $matches[4][$i])) . '"', $str);
                }
                continue;
            }
            if ($c = \preg_match_all('/(' . $phpMussel['Function']('B64') . '|decode_base64|base64\\.b64decode|atob|Base64\\.decode64)(\\s*' . '\\(\\s*["\'\\`])([\\da-z+\\/]{4})*([\\da-z+\\/]{4}|[\\da-z+\\/]{3}=|[\\da-z+\\/]{2}==)(["\'\\`]' . '\\s*\\))/i', $str, $matches)) {
                for ($i = 0; $c > $i; $i++) {
                    $str = \str_ireplace($matches[0][$i], '"' . $phpMussel['Function']('B64', $phpMussel['substrbl']($phpMussel['substraf']($matches[0][$i], $matches[1][$i] . $matches[2][$i]), $matches[5][$i])) . '"', $str);
                }
                continue;
            }
            if ($c = \preg_match_all('/(' . $phpMussel['Function']('R13') . '\\s*\\(\\s*["\'])([^\'"\\(\\)]{1,4096})(["\']\\s*\\))/i', $str, $matches)) {
                for ($i = 0; $c > $i; $i++) {
                    $str = \str_ireplace($matches[0][$i], '"' . $phpMussel['Function']('R13', $phpMussel['substrbl']($phpMussel['substraf']($matches[0][$i], $matches[1][$i]), $matches[3][$i])) . '"', $str);
                }
                continue;
            }
            if ($c = \preg_match_all('/(' . $phpMussel['Function']('HEX') . '\\s*\\(\\s*["\'])([\\da-f]{1,4096})(["\']\\s*\\))/i', $str, $matches)) {
                for ($i = 0; $c > $i; $i++) {
                    $str = \str_ireplace($matches[0][$i], '"' . $phpMussel['HexSafe']($phpMussel['substrbl']($phpMussel['substraf']($matches[0][$i], $matches[1][$i]), $matches[3][$i])) . '"', $str);
                }
                continue;
            }
            if ($c = \preg_match_all('/([Uu][Nn][Pp][Aa][Cc][Kk]\\s*\\(\\s*["\']\\s*H\\*\\s*["\']\\s*,\\s*["\'])([\\da-fA-F]{1,4096})(["\']\\s*\\))/', $str, $matches)) {
                for ($i = 0; $c > $i; $i++) {
    ...
    unset($ostr);
    if ($html) {
        $str = \preg_replace(['@<script[^>]*?>.*?</script>@si', '@<[\\/\\!]*?[^<>]*?>@si', '@<style[^>]*?>.*?</style>@siU', '@<![\\s\\S]*?--[ \\t\\n\\r]*>@'], '', $str);
    }
    return \trim($str);
};
/**
...
$phpMussel['CleanCache'] = function ($Delete = '') use(&$phpMussel) {
    ...
    $phpMussel['InstanceCache']['CacheCleaned'] = \true;
    $CacheFiles = [];
        ...
        foreach ($FileData as &$ThisData) {
            ...
            if ($Delete && $Delete === $ThisData[0] || $ThisData[1] > 0 && $phpMussel['Time'] > $ThisData[1]) {
                ...
                while (\strpos($CacheFiles[$FileKey], $ThisData[0] . ':') !== \false) {
                    $CacheFiles[$FileKey] = \str_ireplace($ThisData[0] . ':' . $phpMussel['substrbf']($phpMussel['substraf']($CacheFiles[$FileKey], $ThisData[0] . ':'), ';') . ';', '', $CacheFiles[$FileKey]);
                }
                $ThisData = '';
                continue;
            }
...
$phpMussel['FetchCache'] = function ($Entry = '') use(&$phpMussel) {
    ...
    if (!($Item = \strpos($FileData, $Entry . ':') !== \false ? $Entry . ':' . $phpMussel['substrbf']($phpMussel['substraf']($FileData, $Entry . ':'), ';') . ';' : '')) {
        return '';
    }
...
$phpMussel['PrepareHashCache'] = function () use(&$phpMussel) {
    ...
    if ($phpMussel['HashCache']['Data'] = $phpMussel['Config']['general']['scan_cache_expiry'] > 0 ? $phpMussel['FetchCache']('HashCache') : '') {
...
$phpMussel['Quarantine'] = function ($In, $Key, $IP, $ID) use(&$phpMussel) {
    ...
    if (!$In || !$Key || !$IP || !$ID || !\function_exists('gzdeflate') || \strlen($Key) < 128 && !($Key = $phpMussel['HexSafe'](\hash('sha512', $Key) . \hash('whirlpool', $Key)))) {
        return \false;
    }
    ...
    $k = \strlen($Key);
    $FileSize = \strlen($In);
    $Head = "¡phpMussel!" . $phpMussel['HexSafe'](\hash('md5', $In)) . \pack('l*', $FileSize) . "\1";
    $In = \gzdeflate($In, 9);
    $Out = '';
    $i = 0;
    while ($i < $FileSize) {
    ...
    $Out = "/== phpMussel Quarantined File Upload =" . "=\\\n| Time/Date Uploaded: " . \str_pad($phpMussel['Time'], 18, ' ') . "|\n| Uploaded From: " . \str_pad($IP, 22, ' ') . " |\n\\" . \str_repeat("=", 39) . "/\n\n\n" . $Head . $Out;
 ...
 *      error occurs).
 */
$phpMussel['lv_match'] = function ($Needle, $Haystack, $pos_A = 0, $pos_Z = 0, $min = 0, $max = -1, $bool = \true, $case = \false, $cost_ins = 1, $cost_rep = 1, $cost_del = 1) {
    ...
    if ($cost_ins === 1 && $cost_rep === 1 && $cost_del === 1) {
        $lv = $case ? \levenshtein($Haystack, $Needle) : \levenshtein(\strtolower($Haystack), \strtolower($Needle));
    } else {
        $lv = $case ? \levenshtein($Haystack, $Needle, $cost_ins, $cost_rep, $cost_del) : \levenshtein(\strtolower($Haystack), \strtolower($Needle), $cost_ins, $cost_rep, $cost_del);
    }
...
$phpMussel['vn_shorthand'] = function ($VN) use(&$phpMussel) {
    ...
    if (!isset($phpMussel['shorthand.yaml'])) {
        ...
        $phpMussel['shorthand.yaml'] = (new \AstraPrefixed\Maikuolan\Common\YAML($phpMussel['ReadFile']($phpMussel['Vault'] . 'shorthand.yaml')))->Data;
    }
    ...
    if (!empty($phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]]) && \is_array($phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]]) && !empty($phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]][$Nibbles[1]]) && \is_string($phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]][$Nibbles[1]])) {
        $SkipMeta = \true;
        $Out .= $phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]][$Nibbles[1]] . '-';
    } elseif (!empty($phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]]) && \is_string($phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]])) {
        $Out .= $phpMussel['shorthand.yaml']['Vendor Shorthand'][$Nibbles[0]] . '-';
    }
    /** Populate weight options. */
    if (!empty($phpMussel['shorthand.yaml']['Vendor Weight Options'][$Nibbles[0]][$Nibbles[1]]) && $phpMussel['shorthand.yaml']['Vendor Weight Options'][$Nibbles[0]][$Nibbles[1]] === 'Weighted' || !empty($phpMussel['shorthand.yaml']['Vendor Weight Options'][$Nibbles[0]]) && $phpMussel['shorthand.yaml']['Vendor Weight Options'][$Nibbles[0]] === 'Weighted') {
        $phpMussel['InstanceCache']['weighted'] = \true;
    }
    ...
    if (!empty($phpMussel['shorthand.yaml']['Malware Type Ignore Options'][$Nibbles[0]][$Nibbles[1]])) {
        ...
        if (isset($phpMussel['Config']['signatures'][$IgnoreOption]) && !$phpMussel['Config']['signatures'][$IgnoreOption]) {
            $phpMussel['InstanceCache']['ignoreme'] = \true;
        }
    }
...
$phpMussel['SafeBrowseLookup'] = function (array $URLs, array $URLsNoLookup = [], array $DomainsNoLookup = []) use(&$phpMussel) {
    ...
    $Arr = \json_encode(['client' => ['clientId' => 'phpMussel', 'clientVersion' => $phpMussel['ScriptVersion']], 'threatInfo' => ['threatTypes' => ['THREAT_TYPE_UNSPECIFIED', 'MALWARE', 'SOCIAL_ENGINEERING', 'UNWANTED_SOFTWARE', 'POTENTIALLY_HARMFUL_APPLICATION'], 'platformTypes' => ['ANY_PLATFORM'], 'threatEntryTypes' => ['URL'], 'threatEntries' => $URLs]], \JSON_UNESCAPED_SLASHES | \JSON_PRETTY_PRINT);
    ...
    while (\strpos($phpMussel['InstanceCache']['urlscanner_google'], $cacheRef) !== \false) {
        ...
        $phpMussel['InstanceCache']['urlscanner_google'] = \str_ireplace($cacheRef . $Response . ';', '', $phpMussel['InstanceCache']['urlscanner_google']);
        $Response = '';
    }
    ...
    /** Perform lookup. */
    $Response = $phpMussel['Request']('https://safebrowsing.googleapis.com/v4/threatMatches:find?key=' . $phpMussel['Config']['urlscanner']['google_api_key'], $Arr, $phpMussel['Request']->DefaultTimeout, ['Content-type: application/json']);
 ...
 */
$phpMussel['Detected'] = function (&$Heuristic, &$Indentation, &$VN, &$OriginalFilename, &$OriginalFilenameSafe, &$Out, &$Flagged, &$Checksum, &$StringLength) use(&$phpMussel) {
    if (!$Flagged) {
    ...
    if ($phpMussel['InstanceCache']['weighted']) {
        $Heuristic['weight']++;
        $Heuristic['cli'] .= $Indentation . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('detected'), $VN)) . "\n";
        $Heuristic['web'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('detected'), $VN) . ' (' . $OriginalFilenameSafe . ')');
        return;
    }
    $Out .= $Indentation . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('detected'), $VN)) . "\n";
    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('detected'), $VN) . ' (' . $OriginalFilenameSafe . ')');
};
/**
...
$phpMussel['DataHandler'] = function ($str = '', $dpt = 0, $OriginalFilename = '') use(&$phpMussel) {
    ...
    if (!isset($phpMussel['InstanceCache'])) {
        throw new \Exception($phpMussel['L10N']->getString('required_variables_not_defined') ?: '[phpMussel] Required variables aren\'t defined: Can\'t continue.');
    }
    ...
    if (!$OriginalFilename) {
        ...
        $phpMussel['InstanceCache']['detections_count']++;
        $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('scan_missing_filename')) . "\n";
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_missing_filename'));
        return [2, $Out];
    }
     ...
     */
    $CoExMeta = '$OriginalFilename:' . $OriginalFilename . ';$StringLength:' . $StringLength . ';$md5:' . $md5 . ';$sha1:' . $sha1 . ';$sha256:' . $sha256 . ';$crc32b:' . $crc32b . ';$fourcc:' . $fourcc . ';$twocc:' . $twocc . ';';
    ...
    if (isset($phpMussel['HashCache']['Data'][$phpMussel['HashCacheData']])) {
        ...
        if (isset($phpMussel['DebugArr'])) {
            ...
            $phpMussel['DebugArr'][$phpMussel['DebugArrKey']] = ['Filename' => $OriginalFilename, 'FromCache' => \true, 'Depth' => $dpt, 'Size' => $StringLength, 'MD5' => $md5, 'SHA1' => $sha1, 'SHA256' => $sha256, 'CRC32B' => $crc32b, '2CC' => $twocc, '4CC' => $fourcc, 'ScanPhase' => $phpMussel['InstanceCache']['phase'], 'Container' => $phpMussel['InstanceCache']['container'], 'Results' => !$Out ? 1 : 2, 'Output' => $Out];
        }
        /** Object not flagged. */
        if (!$Out) {
            return [1, ''];
    ...
    if ($phpMussel['Config']['attack_specific']['scannable_threshold'] > 0 && $StringLength > $phpMussel['ReadBytes']($phpMussel['Config']['attack_specific']['scannable_threshold'])) {
    ...
    $decode_or_not = $phpMussel['Config']['attack_specific']['decode_threshold'] > 0 && $StringLength > $phpMussel['ReadBytes']($phpMussel['Config']['attack_specific']['decode_threshold']) || $StringLength < 16 ? 0 : 1;
    ...
    foreach (['detect_adware', 'detect_joke_hoax', 'detect_pua_pup', 'detect_packer_packed', 'detect_shell', 'detect_deface', 'detect_encryption'] as $Flag) {
    ...
    $str_html = $phpMussel['prescan_normalise']($str, \true, $decode_or_not);
    ...
    $is_html = \strpos(',asp*,dht*,eml*,hta*,htm*,jsp*,php*,sht*,', ',' . $xts . ',') !== \false || \preg_match('/3c(?:21646f6374797065|6(?:120|26f6479|8656164|8746d6c|96672616d65|96d67|f626a656374)|7(?:36372697074|461626c65|469746c65))/i', $str_hex_norm) || \preg_match('/(?:6(?:26f6479|8656164|8746d6c)|7(?:36372697074|461626c65|469746c65))3e/i', $str_hex_norm);
    /** Look for potential email indicators. */
    $is_email = \strpos(',htm*,ema*,eml*,', ',' . $xts . ',') !== \false || \preg_match('/0a(?:4(?:36f6e74656e742d54797065|4617465|6726f6d|d6573736167652d4944|d4' . '94d452d56657273696f6e)|5(?:265706c792d546f|2657475726e2d50617468|3656e64' . '6572|375626a656374|46f|82d4d61696c6572))3a20/i', $str_hex) || \preg_match('/0a2d2d.{32}(?:2d2d)?(?:0d)?0a/i', $str_hex);
    ...
    $is_swf = \strpos(',435753,465753,5a5753,', ',' . \substr($str_hex, 0, 6) . ',') !== \false || \strpos(',swf,swt,', ',' . $xt . ',') !== \false;
    ...
    $is_ole = !empty($phpMussel['InstanceCache']['file_is_ole']) && (!empty($phpMussel['InstanceCache']['file_is_macro']) || \strpos(',bin,ole,xml,rels,', ',' . $xt . ',') !== \false);
    ...
    if (!$phpMussel['InstanceCache']['switch.dat']) {
        ...
        if (!$phpMussel['Config']['signatures']['fail_silently']) {
            ...
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_signature_file_missing') . ' (switch.dat)');
            return [-3, $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('scan_signature_file_missing') . ' (switch.dat)') . "\n"];
        }
    }
    foreach ($phpMussel['InstanceCache']['switch.dat'] as $ThisRule) {
        ...
        foreach ($ThisRule as $Fragment) {
            ...
            if ($Fragment[0] === 'LV') {
                ...
                if (isset($Fragment[3])) {
                    if ($Fragment[2] === 'A') {
                        if (\strpos(',FD,FD-RX,FD-NORM,FD-NORM-RX,', ',' . $Fragment[0] . ',') === \false || $Fragment[0] === 'FD' && \strpos("\1" . \substr($str_hex, 0, $Fragment[3] * 2), "\1" . $Fragment[1]) === \false || $Fragment[0] === 'FD-RX' && !\preg_match('/\\A(?:' . $Fragment[1] . ')/i', \substr($str_hex, 0, $Fragment[3] * 2)) || $Fragment[0] === 'FD-NORM' && \strpos("\1" . \substr($str_hex_norm, 0, $Fragment[3] * 2), "\1" . $Fragment[1]) === \false || $Fragment[0] === 'FD-NORM-RX' && !\preg_match('/\\A(?:' . $Fragment[1] . ')/i', \substr($str_hex_norm, 0, $Fragment[3] * 2))) {
                            continue 2;
                        }
                    } elseif (\strpos(',FD,FD-RX,FD-NORM,FD-NORM-RX,', ',' . $Fragment[0] . ',') === \false || $Fragment[0] === 'FD' && \strpos(\substr($str_hex, $Fragment[2] * 2, $Fragment[3] * 2), $Fragment[1]) === \false || $Fragment[0] === 'FD-RX' && !\preg_match('/(?:' . $Fragment[1] . ')/i', \substr($str_hex, $Fragment[2] * 2, $Fragment[3] * 2)) || $Fragment[0] === 'FD-NORM' && \strpos(\substr($str_hex_norm, $Fragment[2] * 2, $Fragment[3] * 2), $Fragment[1]) === \false || $Fragment[0] === 'FD-NORM-RX' && !\preg_match('/(?:' . $Fragment[1] . ')/i', \substr($str_hex_norm, $Fragment[2] * 2, $Fragment[3] * 2))) {
                        continue 2;
                    }
                } else {
                    if ($Fragment[2] === 'A') {
                        if (\strpos(',FN,FD,FD-RX,FD-NORM,FD-NORM-RX,', ',' . $Fragment[0] . ',') === \false || $Fragment[0] === 'FN' && !\preg_match('/\\A(?:' . $Fragment[1] . ')/i', $OriginalFilename) || $Fragment[0] === 'FD' && \strpos("\1" . $str_hex, "\1" . $Fragment[1]) === \false || $Fragment[0] === 'FD-RX' && !\preg_match('/\\A(?:' . $Fragment[1] . ')/i', $str_hex) || $Fragment[0] === 'FD-NORM' && \strpos("\1" . $str_hex_norm, "\1" . $Fragment[1]) === \false || $Fragment[0] === 'FD-NORM-RX' && !\preg_match('/\\A(?:' . $Fragment[1] . ')/i', $str_hex_norm)) {
                            continue 2;
                        }
                    } elseif (\strpos(',FD,FD-RX,FD-NORM,FD-NORM-RX,', ',' . $Fragment[0] . ',') === \false || $Fragment[0] === 'FD' && \strpos(\substr($str_hex, $Fragment[2] * 2), $Fragment[1]) === \false || $Fragment[0] === 'FD-RX' && !\preg_match('/(?:' . $Fragment[1] . ')/i', \substr($str_hex, $Fragment[2] * 2)) || $Fragment[0] === 'FD-NORM' && \strpos(\substr($str_hex_norm, $Fragment[2] * 2), $Fragment[1]) === \false || $Fragment[0] === 'FD-NORM-RX' && !\preg_match('/(?:' . $Fragment[1] . ')/i', \substr($str_hex_norm, $Fragment[2] * 2))) {
                        continue 2;
                    }
                }
            } elseif ($Fragment[0] === 'FN' && !\preg_match('/(?:' . $Fragment[1] . ')/i', $OriginalFilename) || $Fragment[0] === 'FS-MIN' && $StringLength < $Fragment[1] || $Fragment[0] === 'FS-MAX' && $StringLength > $Fragment[1] || $Fragment[0] === 'FD' && \strpos($str_hex, $Fragment[1]) === \false || $Fragment[0] === 'FD-RX' && !\preg_match('/(?:' . $Fragment[1] . ')/i', $str_hex) || $Fragment[0] === 'FD-NORM' && \strpos($str_hex_norm, $Fragment[1]) === \false || $Fragment[0] === 'FD-NORM-RX' && !\preg_match('/(?:' . $Fragment[1] . ')/i', $str_hex_norm)) {
                continue 2;
    ...
    $PEOriginalFilename = '';
    $PECompanyName = '';
    if (!empty($phpMussel['InstanceCache']['PE_Sectional']) || !empty($phpMussel['InstanceCache']['PE_Extended']) || $phpMussel['Config']['attack_specific']['corrupted_exe']) {
        $PEArr = ['SectionArr' => []];
        if ($twocc === '4d5a') {
            ...
            if (!$PEArr['DoScan']) {
                if ($phpMussel['Config']['attack_specific']['corrupted_exe']) {
                    ...
                    $heur['detections']++;
                    $phpMussel['InstanceCache']['detections_count']++;
                    $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('corrupted')) . "\n";
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('corrupted') . ' (' . $OriginalFilenameSafe . ')');
                }
            } else {
                $is_pe = \true;
                $asciiable = \false;
                ...
                for ($PEArr['k'] = 0; $PEArr['k'] < $NumOfSections; $PEArr['k']++) {
                    $PEArr['SectionArr'][$PEArr['k']] = ['SectionHead' => \substr($str, $PEArr['Offset'] + 24 + $PEArr['OptHdrSize'] + $PEArr['k'] * 40, $NumOfSections * 40)];
                    $PEArr['SectionArr'][$PEArr['k']]['SectionName'] = \str_ireplace("\0", '', \substr($PEArr['SectionArr'][$PEArr['k']]['SectionHead'], 0, 8));
                    $PEArr['SectionArr'][$PEArr['k']]['VirtualSize'] = $phpMussel['UnpackSafe']('S', \substr($PEArr['SectionArr'][$PEArr['k']]['SectionHead'], 8, 4));
                    $PEArr['SectionArr'][$PEArr['k']]['VirtualSize'] = $PEArr['SectionArr'][$PEArr['k']]['VirtualSize'][1];
                    $PEArr['SectionArr'][$PEArr['k']]['VirtualAddress'] = $phpMussel['UnpackSafe']('S', \substr($PEArr['SectionArr'][$PEArr['k']]['SectionHead'], 12, 4));
                    $PEArr['SectionArr'][$PEArr['k']]['VirtualAddress'] = $PEArr['SectionArr'][$PEArr['k']]['VirtualAddress'][1];
                    ...
                    $phpMussel['PEData'] .= $SizeOfRawData . ':' . $PEArr['SectionArr'][$PEArr['k']]['sha256'] . ':' . $OriginalFilename . '-' . $PEArr['SectionArr'][$PEArr['k']]['SectionName'] . "\n";
                    $CoExMeta .= \sprintf('SectionName:%s;VirtualSize:%s;VirtualAddress:%s;SizeOfRawData:%s;SHA256:%s;', $PEArr['SectionArr'][$PEArr['k']]['SectionName'], $PEArr['SectionArr'][$PEArr['k']]['VirtualSize'], $PEArr['SectionArr'][$PEArr['k']]['VirtualAddress'], $SizeOfRawData, $PEArr['SectionArr'][$PEArr['k']]['sha256']);
                    $PEArr['SectionArr'][$PEArr['k']] = [$SizeOfRawData . ':' . $PEArr['SectionArr'][$PEArr['k']]['md5'] . ':', $SizeOfRawData . ':' . $PEArr['SectionArr'][$PEArr['k']]['sha1'] . ':', $SizeOfRawData . ':' . $PEArr['SectionArr'][$PEArr['k']]['sha256'] . ':'];
                }
                if (\strpos($str, "V\0a\0r\0F\0i\0l\0e\0I\0n\0f\0o\0\0\0\0\0\$") !== \false) {
                    $PEArr['Parts'] = $phpMussel['substral']($str, "V\0a\0r\0F\0i\0l\0e\0I\0n\0f\0o\0\0\0\0\0\$");
                    $PEArr['FINFO'] = [];
                    foreach ([["F\0i\0l\0e\0D\0e\0s\0c\0r\0i\0p\0t\0i\0o\0n\0\0\0", 'PEFileDescription'], ["F\0i\0l\0e\0V\0e\0r\0s\0i\0o\0n\0\0\0", 'PEFileVersion'], ["P\0r\0o\0d\0u\0c\0t\0N\0a\0m\0e\0\0\0", 'PEProductName'], ["P\0r\0o\0d\0u\0c\0t\0V\0e\0r\0s\0i\0o\0n\0\0\0", 'PEProductVersion'], ["L\0e\0g\0a\0l\0C\0o\0p\0y\0r\0i\0g\0h\0t\0\0\0", 'PECopyright'], ["O\0r\0i\0g\0i\0n\0a\0l\0F\0i\0l\0e\0n\0a\0m\0e\0\0\0", 'PEOriginalFilename'], ["C\0o\0m\0p\0a\0n\0y\0N\0a\0m\0e\0\0\0", 'PECompanyName']] as $PEVars) {
                        if (\strpos($PEArr['Parts'], $PEVars[0]) !== \false && (${$PEVars[1]} = \trim(\str_ireplace("\0", '', $phpMussel['substrbf']($phpMussel['substral']($PEArr['Parts'], $PEVars[0]), "\0\0\0"))))) {
                            foreach (['md5', 'sha1', 'sha256'] as $TryHash) {
                                $PEArr['FINFO'][] = \sprintf('$%s:%s:%d:', $PEVars[1], $TryHash = \hash($TryHash, ${$PEVars[1]}), \strlen(${$PEVars[1]}));
                            }
                        }
                    }
    ...
    $is_not_php = \strpos(',phar,', ',' . $xt . ',') === \false && \strpos(',php*,', ',' . $xts . ',') === \false && \strpos(',phar,', ',' . $gzxt . ',') === \false && \strpos(',php*,', ',' . $gzxts . ',') === \false && \strpos($str_hex_norm, '3c3f706870') === \false || $is_pe;
    /** Set debug values, if this has been enabled. */
    if (isset($phpMussel['DebugArr'])) {
        $phpMussel['DebugArrKey'] = \count($phpMussel['DebugArr']);
        $phpMussel['DebugArr'][$phpMussel['DebugArrKey']] = ['Filename' => $OriginalFilename, 'FromCache' => \false, 'Depth' => $dpt, 'Size' => $StringLength, 'MD5' => $md5, 'SHA1' => $sha1, 'SHA256' => $sha256, 'CRC32B' => $crc32b, '2CC' => $twocc, '4CC' => $fourcc, 'ScanPhase' => $phase, 'Container' => $container, 'FileSwitch' => $fileswitch, 'Is_ELF' => $is_elf, 'Is_Graphics' => $is_graphics, 'Is_HTML' => $is_html, 'Is_Email' => $is_email, 'Is_MachO' => $is_macho, 'Is_PDF' => $is_pdf, 'Is_SWF' => $is_swf, 'Is_PE' => $is_pe, 'Is_Not_HTML' => $is_not_html, 'Is_Not_PHP' => $is_not_php];
        if ($is_pe) {
            $phpMussel['DebugArr'][$phpMussel['DebugArrKey']] += ['NumOfSections' => $NumOfSections, 'PEFileDescription' => $PEFileDescription, 'PEFileVersion' => $PEFileVersion, 'PEProductName' => $PEProductName, 'PEProductVersion' => $PEProductVersion, 'PECopyright' => $PECopyright, 'PEOriginalFilename' => $PEOriginalFilename, 'PECompanyName' => $PECompanyName];
        }
    }
    /** Plugin hook: "during_scan". */
    $phpMussel['Execute_Hook']('during_scan');
    /** Begin URL scanner. */
    if (isset($phpMussel['InstanceCache']['URL_Scanner']) || !empty($phpMussel['Config']['urlscanner']['google_api_key'])) {
        $phpMussel['LookupCount'] = 0;
        $URLScanner = ['FixedSource' => \preg_replace('~(data|f(ile|tps?)|https?|sftp):~i', "\1\\1:", \str_replace("\\", '/', $str_norm)) . "\1", 'DomainsNoLookup' => [], 'DomainsCount' => 0, 'Domains' => [], 'DomainPartsNoLookup' => [], 'DomainParts' => [], 'Queries' => [], 'URLsNoLookup' => [], 'URLsCount' => 0, 'URLs' => [], 'URLPartsNoLookup' => [], 'URLParts' => [], 'TLDs' => [], 'Iterable' => 0, 'Matches' => []];
        if (\preg_match_all('~(?:data|f(?:ile|tps?)|https?|sftp)://(?:www\\d{0,3}\\.)?([\\da-z.-]{1,512})[^\\da-z.-]~i', $URLScanner['FixedSource'], $URLScanner['Matches'])) {
            foreach ($URLScanner['Matches'][1] as $ThisURL) {
                ...
                if (\strpos($URLScanner['DomainParts'][$URLScanner['Iterable']], '.') !== \false) {
                    $URLScanner['TLDs'][$URLScanner['Iterable']] = 'TLD:' . $phpMussel['substral']($URLScanner['DomainParts'][$URLScanner['Iterable']], '.') . ':';
                }
        ...
        $URLScanner['Matches'] = '';
        if (\preg_match_all('~(?:data|f(?:ile|tps?)|https?|sftp)://(?:www\\d{0,3}\\.)?([!#$&-;=?@-\\[\\]_a-z\\~]+)[^!#$&-;=?@-\\[\\]_a-z\\~]~i', $URLScanner['FixedSource'], $URLScanner['Matches'])) {
            foreach ($URLScanner['Matches'][1] as $ThisURL) {
    ...
    foreach ([['General_Command_Detections', 0], ['Hash', 1], ['PE_Sectional', 2], ['PE_Extended', 3], ['URL_Scanner', 4], ['Complex_Extended', 5]] as $ThisConf) {
        ...
        foreach ($SigFiles as $SigFile) {
            ...
            if (empty($phpMussel['InstanceCache'][$SigFile])) {
                ...
                if (!$phpMussel['Config']['signatures']['fail_silently']) {
                    ...
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_signature_file_missing') . ' (' . $SigFile . ')');
                    return [-3, $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('scan_signature_file_missing') . ' (' . $SigFile . ')') . "\n"];
                }
            } elseif ($ThisConf[1] === 0) {
                ...
                foreach ($ArrayCSV as $ItemCSV) {
                    if (\strpos($str_hex_norm, $ItemCSV) !== \false) {
                        ...
                        $heur['detections']++;
                        $phpMussel['InstanceCache']['detections_count']++;
                        $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_command_injection')) . "\n";
                        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_command_injection') . ', \'' . $phpMussel['HexSafe']($ItemCSV) . '\' (' . $OriginalFilenameSafe . ')');
                    }
                }
                ...
                foreach ([$md5, $sha1, $sha256] as $CheckThisHash) {
                    if (\strpos($phpMussel['InstanceCache'][$SigFile], "\n" . $CheckThisHash . ':' . $StringLength . ':') !== \false) {
                        ...
                        if (\strpos($phpMussel['InstanceCache']['greylist'], ',' . $xSig . ',') === \false && empty($phpMussel['InstanceCache']['ignoreme'])) {
                ...
                if (!empty($PEArr['FINFO'])) {
                    foreach ($PEArr['FINFO'] as $PEArr['ThisPart']) {
                        if (\substr_count($phpMussel['InstanceCache'][$SigFile], $PEArr['ThisPart'])) {
                            ...
                            $xSig = $phpMussel['vn_shorthand']($xSig);
                            if (!\substr_count($phpMussel['InstanceCache']['greylist'], ',' . $xSig . ',') && empty($phpMussel['InstanceCache']['ignoreme'])) {
                                $phpMussel['Detected']($heur, $lnap, $xSig, $OriginalFilename, $OriginalFilenameSafe, $Out, $flagged, $md5, $StringLength);
                            }
                        }
                    }
                    ...
                    foreach ($URLScanner['ThisArr'] as $URLScanner['This']) {
                        if (\strpos($phpMussel['InstanceCache'][$SigFile], $URLScanner['This']) !== \false) {
                            ...
                            if (\substr($URLScanner['This'], 0, 15) === 'DOMAIN-NOLOOKUP') {
                                $URLScanner['DomainPartsNoLookup'][$xSig] = \true;
                                continue;
                            }
                            $URLScanner['URLPartsNoLookup'][$xSig] = \true;
                        }
                    }
                }
                foreach ([$URLScanner['TLDs'], $URLScanner['Domains'], $URLScanner['URLs'], $URLScanner['Queries']] as $URLScanner['ThisArr']) {
                    foreach ($URLScanner['ThisArr'] as $URLScanner['This']) {
                        if (\substr_count($phpMussel['InstanceCache'][$SigFile], $URLScanner['This'])) {
                            ...
                            if (($xSig = $phpMussel['vn_shorthand']($xSig)) && !\substr_count($phpMussel['InstanceCache']['greylist'], ',' . $xSig . ',') && empty($phpMussel['InstanceCache']['ignoreme'])) {
                                $phpMussel['Detected']($heur, $lnap, $xSig, $OriginalFilename, $OriginalFilenameSafe, $Out, $flagged, $md5, $StringLength);
                            }
                        }
                    }
                ...
                $SigName = '';
                foreach (['NumOfSections', 'PECompanyName', 'PECopyright', 'PEFileDescription', 'PEFileVersion', 'PEOriginalFilename', 'PEProductName', 'PEProductVersion', 'container', 'crc32b', 'fileswitch', 'fourcc', 'is_elf', 'is_email', 'is_graphics', 'is_html', 'is_macho', 'is_not_html', 'is_not_php', 'is_ole', 'is_pdf', 'is_pe', 'is_swf', 'md5', 'phase', 'sha1', 'sha256', 'StringLength', 'twocc', 'xt', 'xts'] as $ThisCheckFor) {
                    ...
                    if ($xSigCount > 0) {
                        for ($xIter = 1; $xIter < $xSigCount; $xIter++) {
                            ...
                            foreach ($xSig[$xIter] as $ThisSigPart) {
                                ...
                                if (isset($ThisSigPart[2])) {
                                    if (isset($ThisSigPart[3])) {
                                        if ($ThisSigPart[2] === 'A') {
                                            if (\strpos(',FD,FD-RX,FD-NORM,FD-NORM-RX,META,', ',' . $ThisSigPart[0] . ',') === \false || $ThisSigPart[0] === 'FD' && \strpos("\1" . \substr($str_hex, 0, $ThisSigPart[3] * 2), "\1" . $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-RX' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', \substr($str_hex, 0, $ThisSigPart[3] * 2)) || $ThisSigPart[0] === 'FD-NORM' && \strpos("\1" . \substr($str_hex_norm, 0, $ThisSigPart[3] * 2), "\1" . $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-NORM-RX' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', \substr($str_hex_norm, 0, $ThisSigPart[3] * 2)) || $ThisSigPart[0] === 'META' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', \substr($CoExMeta, 0, $ThisSigPart[3] * 2))) {
                                                continue 2;
                                            }
                                            continue;
                                        }
                                        if (\strpos(',FD,FD-RX,FD-NORM,FD-NORM-RX,META,', ',' . $ThisSigPart[0] . ',') === \false || $ThisSigPart[0] === 'FD' && \strpos(\substr($str_hex, $ThisSigPart[2] * 2, $ThisSigPart[3] * 2), $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-RX' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', \substr($str_hex, $ThisSigPart[2] * 2, $ThisSigPart[3] * 2)) || $ThisSigPart[0] === 'FD-NORM' && \strpos(\substr($str_hex_norm, $ThisSigPart[2] * 2, $ThisSigPart[3] * 2), $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-NORM-RX' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', \substr($str_hex_norm, $ThisSigPart[2] * 2, $ThisSigPart[3] * 2)) || $ThisSigPart[0] === 'META' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', \substr($CoExMeta, $ThisSigPart[2] * 2, $ThisSigPart[3] * 2))) {
                                            continue 2;
                                        }
                                        continue;
                                    }
                                    if ($ThisSigPart[2] === 'A') {
                                        if (\strpos(',FN,FD,FD-RX,FD-NORM,FD-NORM-RX,META,', ',' . $ThisSigPart[0] . ',') === \false || $ThisSigPart[0] === 'FN' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', $OriginalFilename) || $ThisSigPart[0] === 'FD' && \strpos("\1" . $str_hex, "\1" . $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-RX' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', $str_hex) || $ThisSigPart[0] === 'FD-NORM' && \strpos("\1" . $str_hex_norm, "\1" . $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-NORM-RX' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', $str_hex_norm) || $ThisSigPart[0] === 'META' && !\preg_match('/\\A(?:' . $ThisSigPart[1] . ')/i', $CoExMeta)) {
                                            continue 2;
                                        }
                                        continue;
                                    }
                                    if (\strpos(',FD,FD-RX,FD-NORM,FD-NORM-RX,META,', ',' . $ThisSigPart[0] . ',') === \false || $ThisSigPart[0] === 'FD' && \strpos(\substr($str_hex, $ThisSigPart[2] * 2), $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-RX' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', \substr($str_hex, $ThisSigPart[2] * 2)) || $ThisSigPart[0] === 'FD-NORM' && \strpos(\substr($str_hex_norm, $ThisSigPart[2] * 2), $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-NORM-RX' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', \substr($str_hex_norm, $ThisSigPart[2] * 2)) || $ThisSigPart[0] === 'META' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', \substr($CoExMeta, $ThisSigPart[2] * 2))) {
                                        continue 2;
                                    }
                                    continue;
                                }
                                if ($ThisSigPart[0] === 'FN' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', $OriginalFilename) || $ThisSigPart[0] === 'FS-MIN' && $StringLength < $ThisSigPart[1] || $ThisSigPart[0] === 'FS-MAX' && $StringLength > $ThisSigPart[1] || $ThisSigPart[0] === 'FD' && \strpos($str_hex, $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-RX' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', $str_hex) || $ThisSigPart[0] === 'FD-NORM' && \strpos($str_hex_norm, $ThisSigPart[1]) === \false || $ThisSigPart[0] === 'FD-NORM-RX' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', $str_hex_norm) || $ThisSigPart[0] === 'META' && !\preg_match('/(?:' . $ThisSigPart[1] . ')/i', $CoExMeta)) {
                                    continue 2;
                                }
                            ...
                            if ($SigName && \strpos($phpMussel['InstanceCache']['greylist'], ',' . $SigName . ',') === \false && empty($phpMussel['InstanceCache']['ignoreme'])) {
    ...
    foreach ([['Filename', 'str_hex', 'str_hex_len', 2], ['Standard', 'str_hex', 'str_hex_len', 0], ['Normalised', 'str_hex_norm', 'str_hex_norm_len', 0], ['HTML', 'str_hex_html', 'str_hex_html_len', 0], ['Standard_RegEx', 'str_hex', 'str_hex_len', 1], ['Normalised_RegEx', 'str_hex_norm', 'str_hex_norm_len', 1], ['HTML_RegEx', 'str_hex_html', 'str_hex_html_len', 1]] as $ThisConf) {
            ...
            for ($SigNum = 0; $SigNum < $NumSigs; $SigNum++) {
                ...
                if (\strpos($ThisSig, ':') !== \false) {
                    ...
                    if ($ThisConf[3] === 2) {
                        ...
                        if ($ThisSig && \strpos($phpMussel['InstanceCache']['greylist'], ',' . $VN . ',') === \false && empty($phpMussel['InstanceCache']['ignoreme'])) {
                        ...
                        $VN = $phpMussel['vn_shorthand']($VN[0]);
                        $VNLC = \strtolower($VN);
                        if ($is_not_php && (\strpos($VNLC, '-php') !== \false || \strpos($VNLC, '.php') !== \false) || $is_not_html && (\strpos($VNLC, '-htm') !== \false || \strpos($VNLC, '.htm') !== \false)) {
                            continue;
                        }
                        if (\strpos($phpMussel['InstanceCache']['greylist'], ',' . $VN . ',') === \false && empty($phpMussel['InstanceCache']['ignoreme'])) {
                            if ($ThisConf[3] === 0) {
    ...
    if (isset($URLScanner) && !$Out) {
        ...
        if ($phpMussel['Config']['urlscanner']['google_api_key'] && $URLScanner['URLsCount']) {
            ...
            for ($i = 0; $i < $URLScanner['URLChunks']; $i++) {
                ...
                if ($phpMussel['Config']['urlscanner']['maximum_api_lookups'] > 0 && $phpMussel['LookupCount'] > $phpMussel['Config']['urlscanner']['maximum_api_lookups']) {
                    if ($phpMussel['Config']['urlscanner']['maximum_api_lookups_response']) {
                        if (!$flagged) {
                            $phpMussel['killdata'] .= $sha256 . ':' . $StringLength . ':' . $OriginalFilename . "\n";
                            $flagged = \true;
                        }
                        $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('too_many_urls')) . "\n";
                        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('too_many_urls') . ' (' . $OriginalFilenameSafe . ')');
                    }
                    break;
                }
                /** Perform safe browsing API lookup (v4). */
                $URLScanner['SafeBrowseLookup'] = $phpMussel['SafeBrowseLookup']($URLScanner['URLsChunked'][$i], $URLScanner['URLPartsNoLookup'], $URLScanner['DomainPartsNoLookup']);
                ...
                if ($URLScanner['SafeBrowseLookup'] !== 204) {
                    ...
                    $URLScanner['L10N'] = $phpMussel['L10N']->getString('SafeBrowseLookup_' . $URLScanner['SafeBrowseLookup']) ?: $phpMussel['L10N']->getString('SafeBrowseLookup_999');
                    $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $URLScanner['L10N']) . "\n";
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $URLScanner['L10N'] . ' (' . $OriginalFilenameSafe . ')');
    ...
    $ThumbnailBypass = \substr($OriginalFilename, 0, 2) === '._' && !\preg_match('~[^\\x00-\\x1f]~', \substr($str, 0, 8)) && \substr($str, 8, 8) === 'Mac OS X';
    /** PHP chameleon attack detection. */
    if ($phpMussel['Config']['attack_specific']['chameleon_from_php']) {
        if ($phpMussel['ContainsMustAssert']([$phpMussel['Config']['attack_specific']['can_contain_php_file_extensions'], $phpMussel['Config']['attack_specific']['archive_file_extensions']], [$xts, $gzxts, $xt, $gzxt]) && \strpos($str_hex_norm, '3c3f706870') !== \false) {
            if (!$flagged) {
                $phpMussel['killdata'] .= $sha256 . ':' . $StringLength . ':' . $OriginalFilename . "\n";
                $flagged = \true;
            }
            $heur['detections']++;
            $phpMussel['InstanceCache']['detections_count']++;
            $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), 'PHP')) . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), 'PHP') . ' (' . $OriginalFilenameSafe . ')');
        }
    }
    ...
    if ($phpMussel['Config']['attack_specific']['chameleon_from_exe']) {
        ...
        if ($Chameleon) {
            ...
            $heur['detections']++;
            $phpMussel['InstanceCache']['detections_count']++;
            $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), $Chameleon)) . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), $Chameleon) . ' (' . $OriginalFilenameSafe . ')');
        }
    }
    ...
    if ($phpMussel['Config']['attack_specific']['chameleon_to_doc']) {
        if (\strpos(',doc,dot,pps,ppt,xla,xls,wiz,', ',' . $xt . ',') !== \false) {
            if ($fourcc !== 'd0cf11e0') {
                ...
                $heur['detections']++;
                $phpMussel['InstanceCache']['detections_count']++;
                $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), 'Office')) . "\n";
                $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), 'Office') . ' (' . $OriginalFilenameSafe . ')');
            }
        }
    }
    ...
    if (!$ThumbnailBypass && $phpMussel['Config']['attack_specific']['chameleon_to_img']) {
        ...
        if (($xt === 'bmp' || $xt === 'dib') && $twocc !== '424d' || $xt === 'gif' && (\substr($str_hex, 0, 12) !== '474946383761' && \substr($str_hex, 0, 12) !== '474946383961') || \preg_match('~j(?:fif?|if|peg?|pg)~', $xt) && \substr($str_hex, 0, 6) !== 'ffd8ff' || $xt === 'jp2' && \substr($str_hex, 0, 16) !== '0000000c6a502020' || ($xt === 'pdd' || $xt === 'psd') && $fourcc !== '38425053' || $xt === 'png' && $fourcc !== '89504e47' || $xt === 'webp' && ($fourcc !== '52494646' || \substr($str, 8, 4) !== 'WEBP') || $xt === 'xcf' && \substr($str, 0, 8) !== 'gimp xcf') {
            ...
            $heur['detections']++;
            $phpMussel['InstanceCache']['detections_count']++;
            $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), $phpMussel['L10N']->getString('image'))) . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), $phpMussel['L10N']->getString('image')) . ' (' . $OriginalFilenameSafe . ')');
        }
    }
    ...
    if ($phpMussel['Config']['attack_specific']['chameleon_to_pdf']) {
        if ($xt === 'pdf' && !$pdf_magic) {
            ...
            $heur['detections']++;
            $phpMussel['InstanceCache']['detections_count']++;
            $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), 'PDF')) . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('scan_chameleon'), 'PDF') . ' (' . $OriginalFilenameSafe . ')');
        }
    }
    /** Control character detection. */
    if ($phpMussel['Config']['attack_specific']['block_control_characters']) {
        if (\preg_match('/[\\x00-\\x08\\x0b\\x0c\\x0e\\x1f\\x7f]/i', $str)) {
            $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('detected_control_characters')) . "\n";
            $heur['detections']++;
            $phpMussel['InstanceCache']['detections_count']++;
            if (!$flagged) {
                $phpMussel['killdata'] .= $sha256 . ':' . $StringLength . ':' . $OriginalFilename . "\n";
            ...
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('detected_control_characters') . ' (' . $OriginalFilenameSafe . ')');
        }
    }
    /**
     ...
     */
    if ($heur['weight'] >= $phpMussel['Config']['heuristic']['threshold'] || $Out) {
        $Out .= $heur['cli'];
    ...
    if (!$Out && !empty($phpMussel['Config']['virustotal']['vt_public_api_key'])) {
        $DoScan = \false;
        $phpMussel['Config']['virustotal']['vt_suspicion_level'] = (int) $phpMussel['Config']['virustotal']['vt_suspicion_level'];
        if ($phpMussel['Config']['virustotal']['vt_suspicion_level'] === 0) {
            ...
            $DoScan = $heur['weight'] > 0 || $is_pe || $fileswitch === 'chrome' || $fileswitch === 'java' || $fileswitch === 'docfile' || $fileswitch === 'vt_interest';
        ...
        if ($DoScan) {
            ...
            if (!empty($phpMussel['InstanceCache']['vt_quota'])) {
                ...
                $phpMussel['InstanceCache']['vt_quota'] = \implode(';', $phpMussel['InstanceCache']['vt_quota']);
            }
            if ($x < $phpMussel['Config']['virustotal']['vt_quota_rate']) {
                $VTParams = ['apikey' => $phpMussel['Config']['virustotal']['vt_public_api_key'], 'resource' => $md5];
                $VTRequest = $phpMussel['Request']('https://www.virustotal.com/vtapi/v2/file/report?apikey=' . \urlencode($phpMussel['Config']['virustotal']['vt_public_api_key']) . '&resource=' . $md5, $VTParams, 12);
                $VTJSON = \json_decode($VTRequest, \true);
                ...
                if (isset($VTJSON['response_code'])) {
                    ...
                    if (isset($VTJSON['scans']) && $VTJSON['response_code'] === 1 && \is_array($VTJSON['scans'])) {
                            ...
                            if ($VTValue['detected'] && $VTValue['result']) {
                                ...
                                if (\strpos($phpMussel['InstanceCache']['greylist'], ',' . $VN . ',') === \false && empty($phpMussel['InstanceCache']['ignoreme'])) {
                                    ...
                                    if ($phpMussel['Config']['virustotal']['vt_weighting'] > 0) {
                                        $VTWeight['weight']++;
                                        $VTWeight['web'] .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('detected'), $VN)) . "\n";
                                        $VTWeight['cli'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('detected'), $VN) . ' (' . $OriginalFilenameSafe . ')');
                                    } else {
                                        $Out .= $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('detected'), $VN)) . "\n";
                                        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('detected'), $VN) . ' (' . $OriginalFilenameSafe . ')');
                                    }
                                }
                            }
                ...
                if ($VTWeight['weight'] > 0 && $VTWeight['weight'] >= $phpMussel['Config']['virustotal']['vt_weighting']) {
                    $Out .= $VTWeight['web'];
    ...
    if (isset($phpMussel['HashCacheData']) && !isset($phpMussel['HashCache']['Data'][$phpMussel['HashCacheData']]) && $phpMussel['Config']['general']['scan_cache_expiry'] > 0) {
        if (empty($phpMussel['HashCache']['Data']) || !\is_array($phpMussel['HashCache']['Data'])) {
            $phpMussel['HashCache']['Data'] = [];
        }
        $phpMussel['HashCache']['Data'][$phpMussel['HashCacheData']] = [$phpMussel['HashCacheData'], $phpMussel['Time'] + $phpMussel['Config']['general']['scan_cache_expiry'], empty($Out) ? '' : \bin2hex($Out), empty($phpMussel['whyflagged']) ? '' : \bin2hex($phpMussel['whyflagged'])];
    }
...
$phpMussel['MetaDataScan'] = function (&$x, &$r, $Indent, $ItemRef, $Filename, &$Data, $Depth, $Checksum) use(&$phpMussel) {
    ...
    /** Filesize thresholds. */
    if ($phpMussel['Config']['files']['filesize_archives'] && $phpMussel['Config']['files']['filesize_limit'] > 0 && $Filesize > $phpMussel['ReadBytes']($phpMussel['Config']['files']['filesize_limit'])) {
        if (!$phpMussel['Config']['files']['filesize_response']) {
            $x .= $Indent . $phpMussel['L10N']->getString('ok') . ' (' . $phpMussel['L10N']->getString('filesize_limit_exceeded') . ").\n";
            return;
        }
        $r = 2;
        $phpMussel['killdata'] .= $Checksum . ':' . $Filesize . ':' . $ItemRef . "\n";
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('filesize_limit_exceeded') . ' (' . $ItemRef . ')');
        $x .= $Indent . $phpMussel['L10N']->getString('filesize_limit_exceeded') . $phpMussel['L10N']->getString('_fullstop_final') . "\n";
        return;
    }
    ...
    if ($phpMussel['Config']['files']['filetype_archives']) {
        ...
        if ($phpMussel['ContainsMustAssert']([$phpMussel['Config']['files']['filetype_whitelist']], [$xt, $xts], ',', \true, \true)) {
            $x .= $Indent . $phpMussel['L10N']->getString('scan_no_problems_found') . "\n";
            return;
        }
        if ($phpMussel['ContainsMustAssert']([$phpMussel['Config']['files']['filetype_blacklist']], [$xt, $xts], ',', \true, \true)) {
            $r = 2;
            $phpMussel['killdata'] .= $Checksum . ':' . $Filesize . ':' . $ItemRef . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('filetype_blacklisted') . ' (' . $ItemRef . ')');
            $x .= $Indent . $phpMussel['L10N']->getString('filetype_blacklisted') . $phpMussel['L10N']->getString('_fullstop_final') . "\n";
            return;
        }
        if (!empty($phpMussel['Config']['files']['filetype_greylist']) && $phpMussel['ContainsMustAssert']([$phpMussel['Config']['files']['filetype_greylist']], [$xt, $xts])) {
            $r = 2;
            $phpMussel['killdata'] .= $Checksum . ':' . $Filesize . ':' . $ItemRef . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('filetype_blacklisted') . ' (' . $ItemRef . ')');
            $x .= $Indent . $phpMussel['L10N']->getString('filetype_blacklisted') . $phpMussel['L10N']->getString('_fullstop_final') . "\n";
    ...
    $phpMussel['InstanceCache']['file_is_macro'] = \strtolower(\substr($Filename, -14)) === 'vbaproject.bin' || \preg_match('~^\\xd0\\xcf|\\x00Attribut|\\x01CompObj|\\x05Document~', $Data);
    ...
    if ($phpMussel['Config']['attack_specific']['block_macros'] && $phpMussel['InstanceCache']['file_is_macro']) {
        $r = 2;
        $phpMussel['killdata'] .= $Checksum . ':' . $Filesize . ':' . $ItemRef . "\n";
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('macros_not_permitted') . ' (' . $ItemRef . ')');
    ...
    if ($Scan[0] === 1) {
        ...
        $CompressionObject = new \AstraPrefixed\phpMussel\CompressionHandler\CompressionHandler($Data);
...
$phpMussel['Indicator-Image'] = function ($Ext, $Head) {
    return \preg_match('/^(?:bm[2p]|c(d5|gm)|d(ib|w[fg]|xf)|ecw|fits|gif|img|j(f?if?|p[2s]|pe?g?2?|xr)|p(bm|cx|dd|gm|ic|n[gms]|' . 'pm|s[dp])|s(id|v[ag])|tga|w(bmp?|ebp|mp)|x(cf|bmp))$/', $Ext) || \preg_match('/^(?:0000000c6a502020|25504446|38425053|424d|474946383[79]61|57454250|67696d7020786366|89504e47|ffd8ff)/', $Head);
};
/**
...
$phpMussel['Recursor'] = function ($f = '', $n = \false, $zz = \false, $dpt = 0, $OriginalFilename = '') use(&$phpMussel) {
    ...
    if (empty($phpMussel['InstanceCache']['OrganisedSigFiles'])) {
        ...
        $phpMussel['InstanceCache']['OrganisedSigFiles'] = \true;
    }
    if ($phpMussel['EOF']) {
        $phpMussel['whyflagged'] = $phpMussel['killdata'] = $phpMussel['PEData'] = '';
        if ($dpt === 0 || !isset($phpMussel['InstanceCache']['objects_scanned'], $phpMussel['InstanceCache']['detections_count'], $phpMussel['InstanceCache']['scan_errors'])) {
        ...
        if (!isset($phpMussel['InstanceCache']['objects_scanned'], $phpMussel['InstanceCache']['detections_count'], $phpMussel['InstanceCache']['scan_errors'])) {
            $phpMussel['InstanceCache']['objects_scanned'] = 0;
            $phpMussel['InstanceCache']['detections_count'] = 0;
            $phpMussel['InstanceCache']['scan_errors'] = 0;
        }
    ...
    if (\is_dir($f)) {
        if (!\is_readable($f)) {
            $phpMussel['InstanceCache']['scan_errors']++;
            return !$n ? 0 : $lnap . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('failed_to_access'), $OriginalFilename)) . "\n";
        }
    ...
    if (!$f || !($d = \is_file($f))) {
        return !$n ? 0 : $lnap . $phpMussel['L10N']->getString('scan_checking') . ' \'' . $OriginalFilename . '\' (FN: ' . $fnCRC . "):\n-" . $lnap . $phpMussel['L10N']->getString('invalid_file') . "\n";
    }
    $fS = \filesize($f);
    if ($phpMussel['Config']['files']['filesize_limit'] > 0) {
        if ($fS > $phpMussel['ReadBytes']($phpMussel['Config']['files']['filesize_limit'])) {
            if (!$phpMussel['Config']['files']['filesize_response']) {
                return !$n ? 1 : $lnap . $phpMussel['L10N']->getString('scan_checking') . ' \'' . $OriginalFilename . '\' (FN: ' . $fnCRC . "):\n-" . $lnap . $phpMussel['L10N']->getString('ok') . ' (' . $phpMussel['L10N']->getString('filesize_limit_exceeded') . ").\n";
            }
            $phpMussel['killdata'] .= \str_repeat('-', 64) . ':' . $fS . ':' . $OriginalFilename . "\n";
            $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('filesize_limit_exceeded') . ' (' . $OriginalFilenameSafe . ')');
            if ($phpMussel['Config']['general']['delete_on_sight'] && \is_readable($f)) {
                \unlink($f);
            }
            return !$n ? 2 : $lnap . $phpMussel['L10N']->getString('scan_checking') . ' \'' . $OriginalFilename . '\' (FN: ' . $fnCRC . "):\n-" . $lnap . $phpMussel['L10N']->getString('filesize_limit_exceeded') . $phpMussel['L10N']->getString('_fullstop_final') . "\n";
        }
    }
    if (!$phpMussel['Config']['attack_specific']['allow_leading_trailing_dots'] && (\substr($OriginalFilename, 0, 1) === '.' || \substr($OriginalFilename, -1) === '.')) {
        $phpMussel['killdata'] .= \str_repeat('-', 64) . ':' . $fS . ':' . $OriginalFilename . "\n";
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_filename_manipulation_detected') . ' (' . $OriginalFilenameSafe . ')');
        if ($phpMussel['Config']['general']['delete_on_sight'] && \is_readable($f)) {
            \unlink($f);
        }
    ...
    if ($phpMussel['ContainsMustAssert']([$phpMussel['Config']['files']['filetype_whitelist']], [$xt, $xts, $gzxt, $gzxts], ',', \true, \true)) {
        return !$n ? 1 : $lnap . $phpMussel['L10N']->getString('scan_checking') . ' \'' . $OriginalFilename . '\' (FN: ' . $fnCRC . "):\n-" . $lnap . $phpMussel['L10N']->getString('scan_no_problems_found') . "\n";
    }
    /** Process filetype blacklisting. */
    if ($phpMussel['ContainsMustAssert']([$phpMussel['Config']['files']['filetype_blacklist']], [$xt, $xts, $gzxt, $gzxts], ',', \true, \true)) {
        $phpMussel['killdata'] .= \str_repeat('-', 64) . ':' . $fS . ':' . $OriginalFilename . "\n";
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('filetype_blacklisted') . ' (' . $OriginalFilenameSafe . ')');
        if ($phpMussel['Config']['general']['delete_on_sight'] && \is_readable($f)) {
            \unlink($f);
        }
    ...
    if (!empty($phpMussel['Config']['files']['filetype_greylist']) && $phpMussel['ContainsMustAssert']([$phpMussel['Config']['files']['filetype_greylist']], [$xt, $xts, $gzxt, $gzxts])) {
        $phpMussel['killdata'] .= \str_repeat('-', 64) . ':' . $fS . ':' . $OriginalFilename . "\n";
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('filetype_blacklisted') . ' (' . $OriginalFilenameSafe . ')');
        if ($phpMussel['Config']['general']['delete_on_sight'] && \is_readable($f)) {
            \unlink($f);
    ...
    $in = $phpMussel['ReadFile']($f, $phpMussel['Config']['attack_specific']['scannable_threshold'] > 0 && $fS > $phpMussel['ReadBytes']($phpMussel['Config']['attack_specific']['scannable_threshold']) ? $phpMussel['ReadBytes']($phpMussel['Config']['attack_specific']['scannable_threshold']) : $fS, \true);
    ...
    if (!empty($in) && $phpMussel['Config']['compatibility']['only_allow_images'] && !$phpMussel['Indicator-Image']($xt, \bin2hex(\substr($in, 0, 16)))) {
        ...
        $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('only_allow_images') . ' (' . $OriginalFilenameSafe . ')');
        if ($phpMussel['Config']['general']['delete_on_sight'] && \is_readable($f)) {
            \unlink($f);
        }
        return !$n ? 2 : $lnap . $phpMussel['L10N']->getString('scan_checking') . ' \'' . $OriginalFilename . '\' (FN: ' . $fnCRC . '; FD: ' . $fdCRC . "):\n-" . $lnap . $phpMussel['L10N']->getString('only_allow_images') . $phpMussel['L10N']->getString('_fullstop_final') . "\n";
    }
    /** Increment objects scanned count. */
    $phpMussel['InstanceCache']['objects_scanned']++;
    /** Send the scan target to the data handler. */
    ...
    if ($z[0] === 1) {
        /** Create a new compression object. */
        $CompressionObject = new \AstraPrefixed\phpMussel\CompressionHandler\CompressionHandler($in);
        /** Now we'll try to decompress the file. */
        if (!($CompressionResults = $CompressionObject->TryEverything())) {
            /** Success! Now we'll send it to the data handler. */
            try {
    ...
    if ($z[0] !== 1) {
        ...
        if ($z[0] === 2) {
            if ($phpMussel['Config']['general']['quarantine_key'] && \strlen($in) < $phpMussel['ReadBytes']($phpMussel['Config']['general']['quarantine_max_filesize'])) {
                /** Note: "qfu" = "Quarantined File Upload". */
                $qfu = $phpMussel['Time'] . '-' . \hash('md5', $phpMussel['Config']['general']['quarantine_key'] . $fdCRC . $phpMussel['Time']);
                $phpMussel['Quarantine']($in, $phpMussel['Config']['general']['quarantine_key'], $_SERVER[$phpMussel['IPAddr']], $qfu);
        ...
        /** Exit. */
        return !$n ? $z[0] : \sprintf('%s%s \'%s\' (FN: %s; FD: %s):%s%s', $lnap, $phpMussel['L10N']->getString('scan_checking'), $OriginalFilename, $fnCRC, $fdCRC, "\n", $z[1]);
    }
    $x = \sprintf('%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s-%1$s%7$s%6$s', $lnap, $phpMussel['L10N']->getString('scan_checking'), $OriginalFilename, $fnCRC, $fdCRC, "\n", $phpMussel['L10N']->getString('scan_no_problems_found'));
    /** Results. */
    $r = 1;
    /**
     * Begin archive phase.
     ...
     */
    if ($phpMussel['Config']['files']['check_archives'] && !empty($in) && $phpMussel['Config']['files']['max_recursion'] > 1) {
...
$phpMussel['ArchiveRecursor'] = function (&$x, &$r, $Data, $File = '', $ScanDepth = 0, $ItemRef = '') use(&$phpMussel) {
    ...
    if (\substr($Data, 0, 2) === 'PK') {
        ...
        if ($xt === 'ole') {
            ...
            $ConType = 'App';
        } elseif (\strpos(',docm,docx,dotm,dotx,potm,potx,ppam,ppsm,ppsx,pptm,pptx,xlam,xlsb,xlsm,xlsx,xltm,xltx,', ',' . $xt . ',') !== \false) {
            $ConType = 'OpenXML';
        } elseif (\strpos(',odc,odf,odg,odm,odp,ods,odt,otg,oth,otp,ots,ott,', ',' . $xt . ',') !== \false || $xts === 'fod*') {
        ...
        if ($ConType !== 'ZIP') {
            $phpMussel['InstanceCache']['file_is_ole'] = \true;
            $phpMussel['InstanceCache']['container'] = 'pkfile';
        }
    } elseif (\substr($Data, 257, 6) === "ustar\0" || \strpos(',tar,tgz,tbz,tlz,tz,', ',' . $xt . ',') !== \false) {
        $Handler = 'TarHandler';
        $ConType = 'TarFile';
    ...
    if (!\class_exists('AstraPrefixed\\phpMussel\\ArchiveHandler\\ArchiveHandler')) {
    ...
    if ($Handler === 'ZipHandler') {
        ...
        if ($phpMussel['Config']['files']['block_encrypted_archives']) {
            ...
            if ($Bits && $Bits[7]) {
                ...
                $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('encrypted_archive') . ' (' . $ItemRef . ')');
                $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s--%1$s%7$s%8$s%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ItemRef, \hash('crc32b', $File), \hash('crc32b', $Data), "\n", $phpMussel['L10N']->getString('encrypted_archive'), $phpMussel['L10N']->getString('_fullstop_final'));
                return;
            }
        }
        /** Guard. */
        if (!\class_exists('\\ZipArchive')) {
            if (!$phpMussel['Config']['signatures']['fail_extensions_silently']) {
                ...
                $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s--%1$s%7$s%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ItemRef, \hash('crc32b', $File), \hash('crc32b', $Data), "\n", $phpMussel['L10N']->getString('scan_extensions_missing') . ' (Zip)');
                return;
            }
        }
        ...
        if (!$File || !\is_readable($File)) {
             ...
             */
            $PointerObject = new \AstraPrefixed\phpMussel\TemporaryFileHandler\TemporaryFileHandler($Data, $phpMussel['cachePath']);
        ...
        if ($Pointer) {
            $ArchiveObject = new \AstraPrefixed\phpMussel\ArchiveHandler\ZipHandler($Pointer);
        }
    }
    /** Handle tar files. */
    if ($Handler === 'TarHandler') {
        /** TarHandler can work with data directly. */
        $ArchiveObject = new \AstraPrefixed\phpMussel\ArchiveHandler\TarHandler($Data);
    }
    /** Handle rar files. */
    if ($Handler === 'RarHandler') {
        ...
        /** We have a valid a pointer. Let's instantiate the object. */
        if ($Pointer) {
            $ArchiveObject = new \AstraPrefixed\phpMussel\ArchiveHandler\RarHandler($Pointer);
        }
    }
    /** Handle PDF files. */
    if ($Handler === 'PdfHandler') {
        ...
        $ArchiveObject = new \AstraPrefixed\phpMussel\ArchiveHandler\PdfHandler($Data);
    }
    ...
    if (isset($ArchiveObject) && \is_object($ArchiveObject)) {
        ...
        if ($ArchiveObject->ErrorState === 0) {
            ...
            while ($ArchiveObject->EntryNext()) {
                ...
                if ($phpMussel['Config']['files']['max_files_in_archives'] > 0 && $Processed > $phpMussel['Config']['files']['max_files_in_archives']) {
                    $r = 2;
                    $phpMussel['killdata'] .= $DataHash . ':' . $DataLen . ':' . $ItemRef . "\n";
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('too_many_files_in_archive') . ' (' . $ItemRef . ')');
                    $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s--%1$s%7$s%8$s%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ItemRef, \hash('crc32b', $File), \hash('crc32b', $Data), "\n", $phpMussel['L10N']->getString('too_many_files_in_archive'), $phpMussel['L10N']->getString('_fullstop_final'));
                ...
                if ($Filesize !== \strlen($Content) || $InternalCRC && \preg_replace('~^0+~', '', $DataCRC32) !== \preg_replace('~^0+~', '', $InternalCRC)) {
                    $r = 2;
                    $phpMussel['killdata'] .= $Hash . ':' . $Filesize . ':' . $ThisItemRef . "\n";
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('scan_tampering') . ' (' . $ThisItemRef . ')');
                    $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s--%1$s%7$s%8$s%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ThisItemRef, $NameCRC32, $DataCRC32, "\n", $phpMussel['L10N']->getString('recursive'), $phpMussel['L10N']->getString('_fullstop_final'));
                    unset($ArchiveObject, $Pointer, $PointerObject);
                    return;
                }
                ...
                if ($ScanDepth > $phpMussel['Config']['files']['max_recursion']) {
                    $r = 2;
                    $phpMussel['killdata'] .= $Hash . ':' . $Filesize . ':' . $ThisItemRef . "\n";
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('recursive') . ' (' . $ThisItemRef . ')');
                    $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s--%1$s%7$s%8$s%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ThisItemRef, $NameCRC32, $DataCRC32, "\n", $phpMussel['L10N']->getString('recursive'), $phpMussel['L10N']->getString('_fullstop_final'));
                    unset($ArchiveObject, $Pointer, $PointerObject);
                    return;
                }
                ...
                if ($phpMussel['QuineDetector']($ScanDepth, $DataHash, $DataLen, $Hash, $Filesize)) {
                    $r = 2;
                    $phpMussel['killdata'] .= $Hash . ':' . $Filesize . ':' . $ThisItemRef . "\n";
                    $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf($phpMussel['L10N']->getString('detected'), 'Quine') . ' (' . $ThisItemRef . ')');
                    $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s--%1$s%7$s%8$s%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ThisItemRef, $NameCRC32, $DataCRC32, "\n", \sprintf($phpMussel['L10N']->getString('detected'), 'Quine'), $phpMussel['L10N']->getString('_fullstop_final'));
                    unset($ArchiveObject, $Pointer, $PointerObject);
                    return;
                }
                /** Ready to check the entry. */
                $x .= \sprintf('-%1$s%2$s \'%3$s\' (FN: %4$s; FD: %5$s):%6$s', $Indent, $phpMussel['L10N']->getString('scan_checking'), $ThisItemRef, $NameCRC32, $DataCRC32, "\n");
                /** Scan the entry. */
                try {
                    $phpMussel['MetaDataScan']($x, $r, '--' . $Indent, $ThisItemRef, $Filename, $Content, $ScanDepth, $Hash);
                } catch (\Exception $e) {
                    unset($ArchiveObject, $Pointer, $PointerObject);
                    throw new \Exception($e->getMessage());
                }
...
$phpMussel['Fork'] = function ($Item = '', $OriginalFilename = '') use(&$phpMussel) {
    $ProcessHandle = \popen($phpMussel['Mussel_PHP'] . ' "' . $phpMussel['Vault'] . '../loader.php" "cli_scan" "' . $Item . '" "' . $OriginalFilename . '"', 'rb');
    $Output = '';
...
$phpMussel['Scan'] = function ($f = '', $n = \false, $zz = \false, $dpt = 0, $OriginalFilename = '') use(&$phpMussel) {
    ...
    $phpMussel['InstanceCache']['start_time'] = \time() + $phpMussel['Config']['general']['timeOffset'] * 60;
    ...
    $phpMussel['InstanceCache']['end_time'] = \time() + $phpMussel['Config']['general']['timeOffset'] * 60;
    ...
    if ($n && !\is_array($r)) {
        $r = \sprintf("%s %s%s\n%s%s %s%s\n", $phpMussel['InstanceCache']['start_time_2822'], $phpMussel['L10N']->getString('started'), $phpMussel['L10N']->getString('_fullstop_final'), $r, $phpMussel['InstanceCache']['end_time_2822'], $phpMussel['L10N']->getString('finished'), $phpMussel['L10N']->getString('_fullstop_final'));
    ...
    if ($phpMussel['EOF']) {
        if ($phpMussel['Config']['general']['scan_cache_expiry'] > 0) {
            ...
            /** Update hash cache. */
            $phpMussel['SaveCache']('HashCache', $phpMussel['Time'] + $phpMussel['Config']['general']['scan_cache_expiry'], \implode('', $phpMussel['HashCache']['Data']));
...
$phpMussel['TimeFormat'] = function ($Time, $In) use(&$phpMussel) {
    ...
    $Values = ['dd' => \substr($Time, 0, 2), 'mm' => \substr($Time, 2, 2), 'yyyy' => \substr($Time, 4, 4), 'yy' => \substr($Time, 6, 2), 'hh' => \substr($Time, 8, 2), 'ii' => \substr($Time, 10, 2), 'ss' => \substr($Time, 12, 2), 'Day' => \substr($Time, 14, 3), 'Mon' => \substr($Time, 17, 3), 'tz' => \substr($Time, 20, 3) . \substr($Time, 24, 2), 't:z' => \substr($Time, 20, 6)];
...
$phpMussel['AutoType'] = function (&$Var, $Type = '') use(&$phpMussel) {
    if (\in_array($Type, ['string', 'timezone', 'checkbox', 'url', 'email'], \true)) {
        $Var = (string) $Var;
    } elseif ($Type === 'int') {
        $Var = (int) $Var;
    } elseif ($Type === 'float') {
...
$phpMussel['OrganiseSigFiles'] = function () use(&$phpMussel) {
    ...
    $Classes = ['General_Command_Detections', 'Filename', 'Hash', 'Standard', 'Standard_RegEx', 'Normalised', 'Normalised_RegEx', 'HTML', 'HTML_RegEx', 'PE_Extended', 'PE_Sectional', 'Complex_Extended', 'URL_Scanner'];
...
$phpMussel['Stats-Initialise'] = function () use(&$phpMussel) {
    if ($phpMussel['Config']['general']['statistics']) {
        ...
        if (empty($phpMussel['Statistics']['Other-Since'])) {
            $phpMussel['Statistics'] = ['Other-Since' => $phpMussel['Time'], 'Web-Events' => 0, 'Web-Scanned' => 0, 'Web-Blocked' => 0, 'Web-Quarantined' => 0, 'CLI-Events' => 0, 'CLI-Scanned' => 0, 'CLI-Flagged' => 0, 'API-Events' => 0, 'API-Scanned' => 0, 'API-Flagged' => 0];
            $phpMussel['CacheModified'] = \true;
        }
    }
};
 ...
 */
$phpMussel['BuildPath'] = function ($Path, $PointsToFile = \true) use(&$phpMussel) {
    /** Input guard. */
...
$phpMussel['BuildLogPattern'] = function ($Str, $GZ = \false) {
    return '~^' . \preg_replace(['~\\\\{(?:dd|mm|yy|hh|ii|ss)\\\\}~i', '~\\\\{yyyy\\\\}~i', '~\\\\{(?:Day|Mon)\\\\}~i', '~\\\\{tz\\\\}~i', '~\\\\{t\\\\:z\\\\}~i'], ['\\d{2}', '\\d{4}', '\\w{3}', '.{1,2}\\d{4}', '.{1,2}\\d{2}\\:\\d{2}'], \preg_quote(\str_replace("\\", '/', $Str))) . ($GZ ? '(?:\\.gz)?' : '') . '$~i';
};
/**
...
$phpMussel['Pseudonymise-IP'] = function ($IP) {
    ...
    return \preg_replace('/^([01]?\\d{1,2}|2[0-4]\\d|25[0-5])\\.([01]?\\d{1,2}|2[0-4]\\d|25[0-5])\\.([01]?\\d{1,2}|2[0-4]\\d|25[0-5])\\.([01]?\\d{1,2}|2[0-4]\\d|25[0-5])$/i', '\\1.\\2.\\3.x', $IP);
};
/**
...
$phpMussel['InitialiseCache'] = function () use(&$phpMussel) {
    ...
    $phpMussel['Cache'] = new \AstraPrefixed\Maikuolan\Common\Cache();
...
$phpMussel['InitialiseErrorHandler'] = function () use(&$phpMussel) {
    ...
    $phpMussel['PreviousErrorHandler'] = \set_error_handler(function ($errno, $errstr, $errfile, $errline) use(&$phpMussel) {
        ...
        if (\strlen($errfile) > $VaultLen && \str_replace("\\", '/', \substr($errfile, 0, $VaultLen)) === \str_replace("\\", '/', $phpMussel['Vault'])) {
