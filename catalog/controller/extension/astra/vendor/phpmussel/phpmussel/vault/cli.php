...
if (!$phpMussel['Config']['general']['disable_cli'] && !$phpMussel['Config']['general']['maintenance_mode']) {
    ...
    $phpMussel['cli_args'] = [isset($argv[0]) ? $argv[0] : '', isset($argv[1]) ? $argv[1] : '', isset($argv[2]) ? $argv[2] : '', isset($argv[3]) ? $argv[3] : ''];
    ...
    if ($phpMussel['cli_args'][1] === 'cli_scan') {
        ...
        if ($phpMussel['cmd'] === 'scan') {
            ...
            try {
                ...
                /** Call recursor. */
                echo $phpMussel['Recursor'](\substr($phpMussel['cli_args'][2], 5), \true, \true, 0, $phpMussel['cli_args'][3]);
                /** Update statistics. */
            ...
            if (\in_array($phpMussel['ThisAlgo'], \hash_algos())) {
                echo $phpMussel['CLI-RecursiveCommand']($phpMussel['cli_args'][2], function ($Params) use(&$phpMussel) {
                    if (\filter_var($Params, \FILTER_VALIDATE_URL)) {
                        ...
                        $Data = $phpMussel['ReadFile']($Params, 0, \true);
                    }
                    if (empty($Data)) {
        ...
        if ($phpMussel['cmd'] === 'coex_file') {
            echo $phpMussel['CLI-RecursiveCommand']($phpMussel['cli_args'][2], function ($Params) use(&$phpMussel) {
                ...
                return \sprintf("\$sha256:%s;\$StringLength:%d;%s\n", \hash('sha256', $Data), \strlen($Data), $phpMussel['L10N']->getString('cli_signature_placeholder'));
            });
        }
        /** Fetch PE metadata. */
        if ($phpMussel['cmd'] === 'pe_meta') {
            echo $phpMussel['CLI-RecursiveCommand']($phpMussel['cli_args'][2], function ($Params) use(&$phpMussel) {
                ...
                $Returnable .= "\n";
                if (\strpos($Data, "V\0a\0r\0F\0i\0l\0e\0I\0n\0f\0o\0\0\0\0\0\$") !== \false) {
                    $PEArr['Parts'] = $phpMussel['substral']($Data, "V\0a\0r\0F\0i\0l\0e\0I\0n\0f\0o\0\0\0\0\0\$");
                    $PEArr['FINFO'] = [];
                    foreach ([["F\0i\0l\0e\0D\0e\0s\0c\0r\0i\0p\0t\0i\0o\0n\0\0\0", 'PEFileDescription'], ["F\0i\0l\0e\0V\0e\0r\0s\0i\0o\0n\0\0\0", 'PEFileVersion'], ["P\0r\0o\0d\0u\0c\0t\0N\0a\0m\0e\0\0\0", 'PEProductName'], ["P\0r\0o\0d\0u\0c\0t\0V\0e\0r\0s\0i\0o\0n\0\0\0", 'PEProductVersion'], ["L\0e\0g\0a\0l\0C\0o\0p\0y\0r\0i\0g\0h\0t\0\0\0", 'PECopyright'], ["O\0r\0i\0g\0i\0n\0a\0l\0F\0i\0l\0e\0n\0a\0m\0e\0\0\0", 'PEOriginalFilename'], ["C\0o\0m\0p\0a\0n\0y\0N\0a\0m\0e\0\0\0", 'PECompanyName']] as $PEVars) {
                        if (\strpos($PEArr['Parts'], $PEVars[0]) !== \false && ($PEArr['ThisData'] = \trim(\str_ireplace("\0", '', $phpMussel['substrbf']($phpMussel['substral']($PEArr['Parts'], $PEVars[0]), "\0\0\0"))))) {
                            $Returnable .= \sprintf("\$%s:%s:%d:%s\n", $PEVars[1], \hash('sha256', $PEArr['ThisData']), \strlen($PEArr['ThisData']), $phpMussel['L10N']->getString('cli_signature_placeholder'));
                        }
                    }
                }
                return $Returnable;
    ...
    while (\true) {
        ...
        echo "\n\n>> ";
        /** Wait for user input. */
        $phpMussel['stdin_clean'] = \trim(\fgets($phpMussel['stdin_handle']));
        ...
        if (\preg_match('~^(?:m(?:d5_file)?$|sha1_file$|hash_file\\:)~', $phpMussel['cmd'])) {
            ...
            $phpMussel['ThisAlgo'] = $phpMussel['cmd'] === 'md5' || $phpMussel['cmd'] === 'sha1' ? $phpMussel['cmd'] : \substr($phpMussel['cmd'], 5);
            if (\in_array($phpMussel['ThisAlgo'], \hash_algos())) {
            ...
            if (!\preg_match_all('/(data|file|https?|ftps?|sftp|ss[hl])\\:\\/\\/(www\\d{0,3}\\.)?([\\da-z.-]{1,512})/i', $phpMussel['stdin_clean'], $phpMussel['URL']['domain']) || !\preg_match_all('/(data|file|https?|ftps?|sftp|ss[hl])\\:\\/\\/(www\\d{0,3}\\.)?([\\!\\#\\$\\&-;\\=\\?\\@-\\[\\]_a-z~]{1,4000})/i', $phpMussel['stdin_clean'], $phpMussel['URL']['url'])) {
            ...
            if (\strpos($phpMussel['URL']['url'][3][0], '?') !== \false) {
                ...
                $phpMussel['URL']['ForThis'] = \hash('md5', $phpMussel['URL']['x']) . ':' . \strlen($phpMussel['URL']['x']);
                if (\strpos($phpMussel['URL']['AvoidMe'], $phpMussel['URL']['ForThis']) === \false && $phpMussel['URL']['ForThis'] !== 'd41d8cd98f00b204e9800998ecf8427e:0') {
                    $phpMussel['URL']['AvoidMe'] .= ',' . $phpMussel['URL']['ForThis'] . ',';
            ...
            $phpMussel['TargetData'] = \substr($phpMussel['stdin_clean'], \strlen($phpMussel['cmd']) + 1);
            echo \sprintf("\n\$sha256:%s;\$StringLength:%d;%s\n", \hash('sha256', $phpMussel['TargetData']), \strlen($phpMussel['TargetData']), $phpMussel['L10N']->getString('cli_signature_placeholder'));
            ...
            if (\is_dir($phpMussel['stdin_clean'])) {
                if (!\is_readable($phpMussel['stdin_clean'])) {
                    ...
                    foreach ($List as $Item) {
                        ...
                        if (!$Out) {
                            $Out = '> ' . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('cli_failed_to_complete') . ' (' . $Item . ')') . "\n";
                        }
                        $r .= $Out;
                ...
                if (!$Out) {
                    $Out = '> ' . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), $phpMussel['L10N']->getString('cli_failed_to_complete')) . "\n";
                }
            } elseif (!$Out) {
            ...
            $phpMussel['stdin_clean'] = \substr($phpMussel['stdin_clean'], \strlen($phpMussel['cmd']) + 1);
            echo \file_exists($phpMussel['Vault'] . 'greylist.csv') ? "\n greylist.csv:\n" . \implode("\n ", \explode(',', $phpMussel['ReadFile']($phpMussel['Vault'] . 'greylist.csv'))) : "\n " . \sprintf($phpMussel['L10N']->getString('_exclamation_final'), \sprintf($phpMussel['L10N']->getString('x_does_not_exist'), 'greylist.csv'));
