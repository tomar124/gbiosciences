...
if ($phpMussel['upload']['count'] > 0 && !$phpMussel['Config']['general']['maintenance_mode']) {
    ...
    foreach ($_FILES as $phpMussel['ThisFileKey'] => $phpMussel['ThisFileData']) {
        ...
        if (!\is_array($phpMussel['ThisFileData']['error'])) {
            $phpMussel['upload']['FilesData']['FileSet'] = ['name' => [$phpMussel['ThisFileData']['name']], 'type' => [$phpMussel['ThisFileData']['type']], 'tmp_name' => [$phpMussel['ThisFileData']['tmp_name']], 'error' => [$phpMussel['ThisFileData']['error']], 'size' => [$phpMussel['ThisFileData']['size']]];
        } else {
        ...
        for ($phpMussel['ThisIter'] = 0, $phpMussel['SkipSerial'] = \true; $phpMussel['ThisIter'] < $phpMussel['ThisFilesCount']; $phpMussel['ThisIter']++) {
            ...
            if ($phpMussel['upload']['ThisError'] > 0) {
                if ($phpMussel['Config']['compatibility']['ignore_upload_errors'] || $phpMussel['upload']['ThisError'] > 8 || $phpMussel['upload']['ThisError'] === 5) {
                    continue;
                }
                $phpMussel['killdata'] .= \sprintf("---------UPLOAD-ERROR-%d---------:%d:%s\n", $phpMussel['upload']['ThisError'], $phpMussel['upload']['FilesData']['FileSet']['size'][$phpMussel['ThisIter']], $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']]);
                $phpMussel['whyflagged'] .= $phpMussel['L10N']->getString($phpMussel['upload']['ThisError'] === 3 || $phpMussel['upload']['ThisError'] === 4 ? 'upload_error_34' : 'upload_error_' . $phpMussel['upload']['ThisError']);
                if (($phpMussel['upload']['ThisError'] === 1 || $phpMussel['upload']['ThisError'] === 2) && $phpMussel['Config']['general']['delete_on_sight'] && \is_uploaded_file($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]) && \is_readable($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']])) {
                    \unlink($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]);
                }
                continue;
            }
            /** Protection against upload spoofing (1/2). */
            if (!$phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']] || !$phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]) {
            ...
            if (!\is_uploaded_file($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']])) {
                $phpMussel['killdata'] .= \sprintf("UNAUTHORISED-FILE-UPLOAD-NO-HASH:%d:%s\n", $phpMussel['upload']['FilesData']['FileSet']['size'][$phpMussel['ThisIter']], $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']]);
                $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), \sprintf('%s (%s)', $phpMussel['L10N']->getString('scan_unauthorised_upload'), $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']]));
                continue;
            }
            ...
            if ($phpMussel['Config']['general']['honeypot_mode']) {
                ...
                $phpMussel['ThisFileData'] = $phpMussel['ReadFile']($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']], $phpMussel['Config']['attack_specific']['scannable_threshold'] > 0 && $phpMussel['ThisFileSize'] > $phpMussel['ReadBytes']($phpMussel['Config']['attack_specific']['scannable_threshold']) ? $phpMussel['ReadBytes']($phpMussel['Config']['attack_specific']['scannable_threshold']) : $phpMussel['ThisFileSize'], \true);
                $phpMussel['killdata'] .= \sprintf("%s:%d:%s\n", \hash('sha256', $phpMussel['ThisFileData']), $phpMussel['upload']['FilesData']['FileSet']['size'][$phpMussel['ThisIter']], $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']]);
                $phpMussel['whyflagged'] .= $phpMussel['L10N']->getString('honeypot_event');
                if ($phpMussel['Config']['general']['quarantine_key'] && \strlen($phpMussel['ThisFileData']) < $phpMussel['ReadBytes']($phpMussel['Config']['general']['quarantine_max_filesize'])) {
                    $phpMussel['ThisFileQuarantinedAs'] = \sprintf('%s-%s', $phpMussel['Time'], \hash('md5', $phpMussel['Config']['general']['quarantine_key'] . \hash('crc32b', $phpMussel['ThisFileData']) . $phpMussel['Time']));
                    $phpMussel['Quarantine']($phpMussel['ThisFileData'], $phpMussel['Config']['general']['quarantine_key'], $_SERVER[$phpMussel['IPAddr']], $phpMussel['ThisFileQuarantinedAs']);
                ...
                if ($phpMussel['Config']['general']['delete_on_sight'] && \is_uploaded_file($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]) && \is_readable($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']])) {
                    \unlink($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]);
                }
                continue;
            }
            /** Process this block if the number of files being uploaded exceeds "max_uploads". */
            if ($phpMussel['upload']['count'] > $phpMussel['Config']['files']['max_uploads'] && $phpMussel['Config']['files']['max_uploads'] >= 1) {
                $phpMussel['killdata'] .= \str_repeat('-', 64) . ':' . $phpMussel['upload']['FilesData']['FileSet']['size'][$phpMussel['ThisIter']] . ':' . $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']] . "\n";
                $phpMussel['whyflagged'] .= \sprintf($phpMussel['L10N']->getString('_exclamation'), $phpMussel['L10N']->getString('upload_limit_exceeded') . ' (' . $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']] . ')');
                if ($phpMussel['Config']['general']['delete_on_sight'] && \is_uploaded_file($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]) && \is_readable($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']])) {
                    \unlink($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']]);
                }
                continue;
            ...
            /** Execute the scan! */
            try {
                $phpMussel['rb'] = $phpMussel['Scan']($phpMussel['upload']['FilesData']['FileSet']['tmp_name'][$phpMussel['ThisIter']], \true, \true, 0, $phpMussel['upload']['FilesData']['FileSet']['name'][$phpMussel['ThisIter']]);
            } catch (\Exception $e) {
    ...
    if ($phpMussel['Config']['general']['scan_cache_expiry'] > 0 && !empty($phpMussel['HashCache']['Data']) && \is_array($phpMussel['HashCache']['Data'])) {
        ...
        /** Update hash cache. */
        $phpMussel['HashCache']['Data'] = $phpMussel['SaveCache']('HashCache', $phpMussel['Time'] + $phpMussel['Config']['general']['scan_cache_expiry'], $phpMussel['HashCache']['Data']);
    ...
    $phpMussel['whyflagged'] = \trim($phpMussel['whyflagged']);
    ...
    if ($phpMussel['whyflagged']) {
        ...
        $phpMussel['TemplateData'] = \array_merge($phpMussel['Config']['template_data'], ['L10N-Lang-Attache' => $phpMussel['L10N-Lang-Attache'], 'GeneratedBy' => \sprintf($phpMussel['Client-L10N']->getString('generated_by'), '<div id="phpmusselversion" dir="ltr">' . $phpMussel['ScriptIdent'] . '</div>'), 'detected' => $phpMussel['whyflagged'], 'favicon' => $phpMussel['favicon'], 'xmlLang' => $phpMussel['Config']['general']['lang']]);
        ...
        if (!isset($phpMussel['InstanceCache']['template_file'])) {
            $phpMussel['InstanceCache']['template_file'] = !$phpMussel['Config']['template_data']['css_url'] ? 'template_' . $phpMussel['Config']['template_data']['theme'] . '.html' : 'template_custom.html';
        }
        /** Fallback for themes without default template files. */
        if ($phpMussel['Config']['template_data']['theme'] !== 'default' && !$phpMussel['Config']['template_data']['css_url'] && !\file_exists($phpMussel['Vault'] . $phpMussel['InstanceCache']['template_file'])) {
        ...
        if (!empty($phpMussel['killdata'])) {
            ...
            if ($phpMussel['PEData']) {
                $phpMussel['InstanceCache']['Handle']['Data'] .= \sprintf("== %s ==\n%s", $phpMussel['L10N']->getString('field_header_pe_reconstruction'), $phpMussel['PEData']);
            }
        ...
        $phpMussel['TemplateData']['pp'] = empty($phpMussel['Config']['legal']['privacy_policy']) ? '' : '<br /><a href="' . $phpMussel['Config']['legal']['privacy_policy'] . '">' . $phpMussel['L10N']->getString('PrivacyPolicy') . '</a>';
        /** Generate HTML output. */
        $phpMussel['HTML'] = $phpMussel['ParseVars']($phpMussel['TemplateData'], $phpMussel['ReadFile']($phpMussel['Vault'] . $phpMussel['InstanceCache']['template_file']));
        ...
        if (empty($phpMussel['Config']['general']['disable_webfonts'])) {
            ...
            $phpMussel['WebFontPos'] = ['Begin' => \strpos($phpMussel['HTML'], '<!-- WebFont Begin -->'), 'End' => \strpos($phpMussel['HTML'], '<!-- WebFont End -->')];
            if ($phpMussel['WebFontPos']['Begin'] !== \false && $phpMussel['WebFontPos']['End'] !== \false) {
                $phpMussel['HTML'] = \substr($phpMussel['HTML'], 0, $phpMussel['WebFontPos']['Begin']) . \substr($phpMussel['HTML'], $phpMussel['WebFontPos']['End'] + 20);
            }
