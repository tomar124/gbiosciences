 ...
 */
namespace AstraPrefixed\Phinx\Db\Plan;
...
use AstraPrefixed\Phinx\Db\Adapter\AdapterInterface;
use AstraPrefixed\Phinx\Db\Plan\Solver\ActionSplitter;
...
class Plan
{
    ...
    public function execute(AdapterInterface $executor)
    {
        ...
        collection($this->updatesSequence())->unfold()->each(function ($updates) use($executor) {
    ...
    {
        collection($this->inverseUpdatesSequence())->unfold()->each(function ($updates) use($executor) {
            $executor->executeActions($updates->getTable(), $updates->getActions());
        });
        foreach ($this->tableCreates as $newTable) {
            $executor->createTable($newTable->getTable(), $newTable->getColumns(), $newTable->getIndexes());
    ...
    {
        $moveActions = collection($this->tableMoves)->unfold(function ($move) {
        ...
        foreach ($this->columnRemoves as $columnRemove) {
            foreach ($columnRemove->getActions() as $action) {
                if ($action instanceof RemoveColumn) {
                    list($this->indexes) = $this->forgetDropIndex($action->getTable(), [$action->getColumn()->getName()], $this->indexes);
                }
            }
        }
        $this->tableUpdates = collection($this->tableUpdates)->unfold(new ActionSplitter(RenameColumn::class, ChangeColumn::class, function (RenameColumn $a, ChangeColumn $b) {
            return $a->getNewName() == $b->getColumnName();
        }))->toList();
        $this->constraints = collection($this->constraints)->map(function (AlterTable $alter) {
    ...
    protected function remapContraintAndIndexConflicts(AlterTable $alter)
    {
        ...
        collection($alter->getActions())->unfold(function ($action) {
            if (!$action instanceof DropForeignKey) {
                return [$action];
            }
            list($this->indexes, $dropIndexActions) = $this->forgetDropIndex($action->getTable(), $action->getForeignKey()->getColumns(), $this->indexes);
            $return = [$action];
    ...
    protected function forgetDropIndex(Table $table, array $columns, array $actions)
    {
        ...
        $indexes = collection($actions)->map(function ($alter) use($table, $columns, $dropIndexActions) {
            if ($alter->getTable()->getName() !== $table->getName()) {
                return $alter;
            }
            $newAlter = new AlterTable($table);
            collection($alter->getActions())->map(function ($action) use($columns) {
    ...
    protected function forgetRemoveColumn(Table $table, array $columns, array $actions)
    {
        ...
        $indexes = collection($actions)->map(function ($alter) use($table, $columns, $removeColumnActions) {
            if ($alter->getTable()->getName() !== $table->getName()) {
                return $alter;
            }
            $newAlter = new AlterTable($table);
    ...
    {
        collection($actions)->filter(function ($action) {
        ...
        collection($actions)->filter(function ($action) {
            return $action instanceof AddColumn || $action instanceof AddIndex;
    ...
    {
        collection($actions)->filter(function ($action) {
            return $action instanceof AddColumn || $action instanceof ChangeColumn || $action instanceof RemoveColumn || $action instanceof RenameColumn;
    ...
    {
        collection($actions)->filter(function ($action) {
            return $action instanceof DropTable || $action instanceof RenameTable || $action instanceof ChangePrimaryKey || $action instanceof ChangeComment;
        })->each(function ($action) {
            $table = $action->getTable();
            $name = $table->getName();
    ...
    {
        collection($actions)->filter(function ($action) {
            return $action instanceof AddIndex || $action instanceof DropIndex;
        })->reject(function ($action) {
    ...
    {
        collection($actions)->filter(function ($action) {
            return $action instanceof AddForeignKey || $action instanceof DropForeignKey;
        })->each(function ($action) {
            $table = $action->getTable();
            $name = $table->getName();
