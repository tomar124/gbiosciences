...
class Init extends Command
{
    ...
    {
        $this->setDescription('Initialize the application for Phinx')->addOption('--format', '-f', InputArgument::OPTIONAL, 'What format should we use to initialize?', 'yml')->addArgument('path', InputArgument::OPTIONAL, 'Which path should we initialize for Phinx?')->setHelp(\sprintf('%sInitializes the application for Phinx%s', \PHP_EOL, \PHP_EOL));
    }
    /**
    ...
    protected function resolvePath(InputInterface $input, $format)
    {
        ...
        if (!\in_array($format, ['yaml', 'yml', 'json', 'php'])) {
            throw new InvalidArgumentException(\sprintf('Invalid format "%s". Format must be either yaml, yml, json, or php.', $format));
        }
        // Fallback
        if (!$path) {
