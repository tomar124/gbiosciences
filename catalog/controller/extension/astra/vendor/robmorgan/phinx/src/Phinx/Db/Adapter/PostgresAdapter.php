...
use AstraPrefixed\Cake\Database\Driver\Postgres as PostgresDriver;
...
class PostgresAdapter extends PdoAdapter
{
    ...
    public function getColumns($tableName)
    {
        ...
        $sql = \sprintf('SELECT column_name, data_type, udt_name, is_identity, is_nullable,
             ...
             WHERE table_schema = %s AND table_name = %s', $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table']));
        $columnsInfo = $this->fetchAll($sql);
        foreach ($columnsInfo as $columnInfo) {
            $isUserDefined = \strtoupper(\trim($columnInfo['data_type'])) === 'USER-DEFINED';
            if ($isUserDefined) {
    ...
    protected function getRenameColumnInstructions($tableName, $columnName, $newColumnName)
    {
        ...
        $instructions = new AlterInstructions();
        $instructions->addPostStep(\sprintf('ALTER TABLE %s RENAME COLUMN %s TO %s', $tableName, $this->quoteColumnName($columnName), $this->quoteColumnName($newColumnName)));
        return $instructions;
    }
    /**
     * @inheritDoc
    ...
    {
        $instructions = new AlterInstructions();
        $sql = \sprintf('ALTER COLUMN %s TYPE %s', $this->quoteColumnName($columnName), $this->getColumnSqlDefinition($newColumn));
        ...
        $instructions->addAlter($sql);
        // process null
        $sql = \sprintf('ALTER COLUMN %s', $this->quoteColumnName($columnName));
        if ($newColumn->isNull()) {
            $sql .= ' DROP NOT NULL';
        } else {
            $sql .= ' SET NOT NULL';
        ...
        if ($newColumn->getDefault() !== null) {
            $instructions->addAlter(\sprintf('ALTER COLUMN %s SET %s', $this->quoteColumnName($columnName), $this->getDefaultValueDefinition($newColumn->getDefault(), $newColumn->getType())));
        } else {
            //drop default
            $instructions->addAlter(\sprintf('ALTER COLUMN %s DROP DEFAULT', $this->quoteColumnName($columnName)));
        }
        // rename column
        if ($columnName !== $newColumn->getName()) {
            $instructions->addPostStep(\sprintf('ALTER TABLE %s RENAME COLUMN %s TO %s', $this->quoteTableName($tableName), $this->quoteColumnName($columnName), $this->quoteColumnName($newColumn->getName())));
        }
    ...
    protected function getIndexSqlDefinition(Index $index, $tableName)
    {
        ...
        return \sprintf('CREATE %s INDEX %s ON %s (%s);', $index->getType() === Index::UNIQUE ? 'UNIQUE' : '', $this->quoteColumnName($indexName), $this->quoteTableName($tableName), \implode(',', \array_map([$this, 'quoteColumnName'], $index->getColumns())));
    }
    /**
