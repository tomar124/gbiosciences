...
use AstraPrefixed\Cake\Database\Driver\Mysql as MysqlDriver;
...
{
    protected $signedColumnTypes = ['integer' => \true, 'smallinteger' => \true, 'biginteger' => \true, 'float' => \true, 'decimal' => \true, 'double' => \true, 'boolean' => \true];
    const TEXT_TINY = 255;
    ...
    const INT_BIG = 1.8446744073709552E+19;
    const BIT = 64;
    ...
    public function hasTable($tableName)
    {
        ...
        $options = $this->getOptions();
        $exists = $this->fetchRow(\sprintf("SELECT TABLE_NAME\n            FROM INFORMATION_SCHEMA.TABLES\n            WHERE TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s'", $options['name'], $tableName));
        return !empty($exists);
    }
    /**
     * {@inheritDoc}
    ...
    public function createTable(Table $table, array $columns = [], array $indexes = [])
    {
        ...
        if (isset($options['id']) && \is_string($options['id'])) {
            ...
            $column = new Column();
            $column->setName($options['id'])->setType('integer')->setSigned(isset($options['signed']) ? $options['signed'] : \true)->setIdentity(\true);
    ...
    public function getColumns($tableName)
    {
        ...
        foreach ($rows as $columnInfo) {
            ...
            $column = new Column();
            $column->setName($columnInfo['Field'])->setNull($columnInfo['Null'] !== 'NO')->setDefault($columnInfo['Default'])->setType($phinxType['name'])->setSigned(\strpos($columnInfo['Type'], 'unsigned') === \false)->setLimit($phinxType['limit'])->setScale($phinxType['scale']);
    ...
    protected function getChangeColumnInstructions($tableName, $columnName, Column $newColumn)
    {
        ...
        $alter = \sprintf('CHANGE %s %s %s%s', $this->quoteColumnName($columnName), $this->quoteColumnName($newColumn->getName()), $this->getColumnSqlDefinition($newColumn), $after);
        return new AlterInstructions([$alter]);
    }
    /**
     * @inheritDoc
    ...
    {
        $foreignKeys = [];
        $rows = $this->fetchAll(\sprintf("SELECT\n              CONSTRAINT_NAME,\n              TABLE_NAME,\n              COLUMN_NAME,\n              REFERENCED_TABLE_NAME,\n              REFERENCED_COLUMN_NAME\n            FROM information_schema.KEY_COLUMN_USAGE\n            WHERE REFERENCED_TABLE_SCHEMA = DATABASE()\n              AND REFERENCED_TABLE_NAME IS NOT NULL\n              AND TABLE_NAME = '%s'\n            ORDER BY POSITION_IN_UNIQUE_CONSTRAINT", $tableName));
        foreach ($rows as $row) {
        ...
        switch ($type) {
            ...
            case static::PHINX_TYPE_INTEGER:
                if ($limit && $limit >= static::INT_TINY) {
                    ...
                    $limits = ['smallint' => 6, 'int' => 11, 'bigint' => 20];
                    foreach ($sizes as $name => $length) {
                        if ($limit >= $length) {
                            $def = ['name' => $name];
    ...
    public function getPhinxType($sqlTypeDef)
    {
        ...
        if (!\preg_match('/^([\\w]+)(\\(([\\d]+)*(,([\\d]+))*\\))*(.+)*$/', $sqlTypeDef, $matches)) {
            ...
            if ($type == static::PHINX_TYPE_ENUM || $type == static::PHINX_TYPE_SET) {
                $phinxType['values'] = \explode("','", \trim($matches[6], "()'"));
            }
            return $phinxType;
        }
    }
