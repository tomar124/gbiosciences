 ...
 */
namespace AstraPrefixed\Phinx\Migration;

use DateTime;
use InvalidArgumentException;
use AstraPrefixed\Phinx\Config\Config;
use AstraPrefixed\Phinx\Config\ConfigInterface;
use AstraPrefixed\Phinx\Config\NamespaceAwareInterface;
use AstraPrefixed\Phinx\Migration\Manager\Environment;
use AstraPrefixed\Phinx\Seed\AbstractSeed;
use AstraPrefixed\Phinx\Seed\SeedInterface;
use AstraPrefixed\Phinx\Util\Util;
...
class Manager
{
    ...
    public function printStatus($environment, $format = null)
    {
        ...
        if (\count($migrations)) {
            ...
            foreach ($sortedMigrations as $migration) {
                ...
                $output->writeln(\sprintf('%s %14.0f  %19s  %19s  <comment>%s</comment>', $status, $migration->getVersion(), $version ? $version['start_time'] : '', $version ? $version['end_time'] : '', $migration->getName()));
                if ($version && $version['breakpoint']) {
                    $output->writeln('         <error>BREAKPOINT SET</error>');
                }
                $finalMigrations[] = ['migration_status' => \trim(\strip_tags($status)), 'migration_id' => \sprintf('%14.0f', $migration->getVersion()), 'migration_name' => $migration->getName()];
    ...
    {
        $this->getOutput()->writeln(\sprintf('     <error>up</error>  %14.0f  %19s  %19s  <comment>%s</comment>  <error>** MISSING **</error>', $version['version'], $version['start_time'], $version['end_time'], \str_pad($version['migration_name'], $maxNameLength, ' ')));
        if ($version && $version['breakpoint']) {
    ...
    public function executeMigration($name, MigrationInterface $migration, $direction = MigrationInterface::UP, $fake = \false)
    {
        ...
        $this->getOutput()->writeln(' ==' . ' <info>' . $migration->getVersion() . ' ' . $migration->getName() . ':</info>' . ' <comment>' . ($direction === MigrationInterface::UP ? 'migrating' : 'reverting') . '</comment>');
        ...
        $this->getOutput()->writeln(' ==' . ' <info>' . $migration->getVersion() . ' ' . $migration->getName() . ':</info>' . ' <comment>' . ($direction === MigrationInterface::UP ? 'migrated' : 'reverted') . ' ' . \sprintf('%.4fs', $end - $start) . '</comment>');
    }
    /**
            ...
            foreach ($phpFiles as $filePath) {
                if (Util::isValidMigrationFileName(\basename($filePath))) {
                    ...
                    if (!$migration instanceof AbstractMigration) {
                        throw new InvalidArgumentException(\sprintf('AstraPrefixed\\The class "%s" in file "%s" must extend \\Phinx\\Migration\\AbstractMigration', $class, $filePath));
                    }
            ...
            foreach ($phpFiles as $filePath) {
                if (Util::isValidSeedFileName(\basename($filePath))) {
                    ...
                    if (!$seed instanceof AbstractSeed) {
                        throw new InvalidArgumentException(\sprintf('AstraPrefixed\\The class "%s" in file "%s" must extend \\Phinx\\Seed\\AbstractSeed', $class, $filePath));
                    }
                    $seeds[$class] = $seed;
                }
            }
