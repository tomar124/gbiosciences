...
class Config implements ConfigInterface, NamespaceAwareInterface
{
    ...
    {
        if (!\class_exists('AstraPrefixed\\Symfony\\Component\\Yaml\\Yaml', \true)) {
    ...
    public function getDefaultEnvironment()
    {
        ...
        if (isset($this->values['environments']['default_database'])) {
            ...
            throw new RuntimeException(\sprintf('The environment configuration for \'%s\' is missing', $this->values['environments']['default_database']));
        }
    ...
    protected function parseAgnosticDsn(array $options)
    {
        if (isset($options['dsn']) && \is_string($options['dsn'])) {
            $regex = '#^(?P<adapter>[^\\:]+)\\://(?:(?P<user>[^\\:@]+)(?:\\:(?P<pass>[^@]*))?@)?' . '(?P<host>[^\\:@/]+)(?:\\:(?P<port>[1-9]\\d*))?/(?P<name>[^\\?]+)(?:\\?(?P<query>.*))?$#';
            if (\preg_match($regex, \trim($options['dsn']), $parsedOptions)) {
                $additionalOpts = [];
