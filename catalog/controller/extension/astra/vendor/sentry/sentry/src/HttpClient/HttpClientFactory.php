...
use AstraPrefixed\GuzzleHttp\RequestOptions as GuzzleHttpClientOptions;
use AstraPrefixed\Http\Adapter\Guzzle6\Client as GuzzleHttpClient;
use AstraPrefixed\Http\Client\Common\Plugin\AuthenticationPlugin;
use AstraPrefixed\Http\Client\Common\Plugin\DecoderPlugin;
use AstraPrefixed\Http\Client\Common\Plugin\ErrorPlugin;
use AstraPrefixed\Http\Client\Common\Plugin\HeaderSetPlugin;
use AstraPrefixed\Http\Client\Common\Plugin\RetryPlugin;
use AstraPrefixed\Http\Client\Common\PluginClient;
use AstraPrefixed\Http\Client\Curl\Client as CurlHttpClient;
use AstraPrefixed\Http\Client\HttpAsyncClient as HttpAsyncClientInterface;
use AstraPrefixed\Http\Discovery\HttpAsyncClientDiscovery;
use AstraPrefixed\Http\Message\ResponseFactory as ResponseFactoryInterface;
use AstraPrefixed\Http\Message\StreamFactory as StreamFactoryInterface;
use AstraPrefixed\Http\Message\UriFactory as UriFactoryInterface;
use AstraPrefixed\Sentry\HttpClient\Authentication\SentryAuthentication;
use AstraPrefixed\Sentry\HttpClient\Plugin\GzipEncoderPlugin;
use AstraPrefixed\Sentry\Options;
use AstraPrefixed\Symfony\Component\HttpClient\HttpClient as SymfonyHttpClient;
use AstraPrefixed\Symfony\Component\HttpClient\HttplugClient as SymfonyHttplugClient;
/**
    ...
    public function create(Options $options) : HttpAsyncClientInterface
    {
        ...
        if (null === $httpClient) {
            if (\class_exists(SymfonyHttplugClient::class)) {
                ...
                $httpClient = new SymfonyHttplugClient(SymfonyHttpClient::create($symfonyConfig));
            } elseif (\class_exists(GuzzleHttpClient::class)) {
                /** @psalm-suppress UndefinedClass */
                $guzzleConfig = [GuzzleHttpClientOptions::TIMEOUT => self::DEFAULT_HTTP_TIMEOUT, GuzzleHttpClientOptions::CONNECT_TIMEOUT => self::DEFAULT_HTTP_CONNECT_TIMEOUT];
                if (null !== $options->getHttpProxy()) {
                    /** @psalm-suppress UndefinedClass */
        ...
        $httpClientPlugins = [new HeaderSetPlugin(['User-Agent' => $this->sdkIdentifier . '/' . $this->sdkVersion]), new AuthenticationPlugin(new SentryAuthentication($options, $this->sdkIdentifier, $this->sdkVersion)), new RetryPlugin(['retries' => $options->getSendAttempts()]), new ErrorPlugin()];
