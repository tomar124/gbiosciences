...
use AstraPrefixed\Sentry\Integration\ErrorListenerIntegration;
use AstraPrefixed\Sentry\Integration\ExceptionListenerIntegration;
use AstraPrefixed\Sentry\Integration\FatalErrorListenerIntegration;
use AstraPrefixed\Sentry\Integration\FrameContextifierIntegration;
use AstraPrefixed\Sentry\Integration\IntegrationInterface;
use AstraPrefixed\Sentry\Integration\RequestIntegration;
use AstraPrefixed\Sentry\Integration\TransactionIntegration;
use AstraPrefixed\Symfony\Component\OptionsResolver\Options as SymfonyOptions;
use AstraPrefixed\Symfony\Component\OptionsResolver\OptionsResolver;
/**
...
final class Options
{
    ...
    private function configureOptions(OptionsResolver $resolver) : void
    {
        $resolver->setDefaults(['integrations' => [], 'default_integrations' => \true, 'send_attempts' => 3, 'prefixes' => \explode(\PATH_SEPARATOR, \get_include_path() ?: ''), 'sample_rate' => 1, 'attach_stacktrace' => \false, 'context_lines' => 5, 'enable_compression' => \true, 'environment' => $_SERVER['SENTRY_ENVIRONMENT'] ?? null, 'project_root' => null, 'logger' => 'php', 'release' => $_SERVER['SENTRY_RELEASE'] ?? null, 'dsn' => $_SERVER['SENTRY_DSN'] ?? null, 'server_name' => \gethostname(), 'before_send' => static function (Event $event) : Event {
            return $event;
        }, 'tags' => [], 'error_types' => \E_ALL, 'max_breadcrumbs' => self::DEFAULT_MAX_BREADCRUMBS, 'before_breadcrumb' => static function (Breadcrumb $breadcrumb) : Breadcrumb {
            return $breadcrumb;
    ...
    private function getDefaultIntegrations() : array
    {
        ...
        if (null === $this->defaultIntegrations) {
            $this->defaultIntegrations = [new ExceptionListenerIntegration(), new ErrorListenerIntegration(null, \false), new FatalErrorListenerIntegration(), new RequestIntegration(), new TransactionIntegration(), new FrameContextifierIntegration()];
        }
