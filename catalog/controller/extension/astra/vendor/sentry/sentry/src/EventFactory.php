...
use AstraPrefixed\Sentry\Exception\EventCreationException;
...
final class EventFactory implements EventFactoryInterface
{
    ...
    public function createWithStacktrace(array $payload) : Event
    {
        if (!isset($payload['stacktrace']) || !$payload['stacktrace'] instanceof Stacktrace) {
            $payload['stacktrace'] = Stacktrace::createFromBacktrace($this->options, $this->serializer, $this->representationSerializer, \debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), __FILE__, __LINE__ - 6, \func_num_args() > 1 ? \func_get_arg(1) : \true);
        }
    ...
    private function addThrowableToEvent(Event $event, \Throwable $exception, bool $shouldReadSourceCodeExcerpts) : void
    {
        ...
        do {
            $exceptions[] = ['type' => \get_class($currentException), 'value' => $currentException->getMessage(), 'stacktrace' => Stacktrace::createFromBacktrace($this->options, $this->serializer, $this->representationSerializer, $currentException->getTrace(), $currentException->getFile(), $currentException->getLine(), $shouldReadSourceCodeExcerpts)];
