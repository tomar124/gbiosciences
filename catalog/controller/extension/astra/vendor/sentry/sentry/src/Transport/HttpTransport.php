...
use AstraPrefixed\GuzzleHttp\Promise\EachPromise;
use AstraPrefixed\GuzzleHttp\Promise\FulfilledPromise;
use AstraPrefixed\GuzzleHttp\Promise\PromiseInterface;
use AstraPrefixed\Http\Client\HttpAsyncClient as HttpAsyncClientInterface;
use AstraPrefixed\Http\Message\RequestFactory as RequestFactoryInterface;
use AstraPrefixed\Psr\Log\LoggerInterface;
use AstraPrefixed\Psr\Log\NullLogger;
use AstraPrefixed\Sentry\Dsn;
use AstraPrefixed\Sentry\Event;
use AstraPrefixed\Sentry\Options;
use AstraPrefixed\Sentry\Util\JSON;
/**
...
final class HttpTransport implements TransportInterface, ClosableTransportInterface
{
     ...
     */
    public function __construct(Options $options, HttpAsyncClientInterface $httpClient, RequestFactoryInterface $requestFactory, bool $delaySendingUntilShutdown = \true, bool $triggerDeprecation = \true, ?LoggerInterface $logger = null)
    {
    ...
    public function send(Event $event) : ?string
    {
        ...
        $request = $this->requestFactory->createRequest('POST', $dsn->getStoreApiEndpointUrl(), ['Content-Type' => 'application/json'], JSON::encode($event->toArray()));
    ...
    private function cleanupPendingRequests() : void
    {
        ...
        try {
            $eachPromise = new EachPromise($requestGenerator(), ['concurrency' => 30, 'rejected' => function (\Throwable $exception, int $requestIndex) : void {
